...
void sySchedulerUpdateViMode(u32 width, u32 height, u32 flags, s16 off_left, s16 off_right, s16 off_top, s16 off_bottom)
{
...
#ifdef PORT
    port_log("SSB64: sySchedulerUpdateViMode width=%u height=%u flags=0x%x off=L%d/R%d/T%d/B%d default_native=%dx%d in_bounds=%d\n",
             (unsigned)width, (unsigned)height, (unsigned)flags,
             (int)off_left, (int)off_right, (int)off_top, (int)off_bottom,
             (int)GS_SCREEN_WIDTH_DEFAULT, (int)GS_SCREEN_HEIGHT_DEFAULT,
             (int)is_res_in_bounds);
    ...
    port_log("SSB64:   → GfxSetNativeDimensions(%u, %u)\n",
