...
3. **Item-heavy** — item spawn/active hashes stable; `SYNCTEST_OK` through item phase (e.g. Saffron); no ghost items after rollback; optional `SSB64_NETPLAY_ITEM_HASH_TRACE=1` + `SSB64_NETPLAY_SNAPSHOT_ITEM_DIAG=1`. **Automatch:** set `SSB64_NETPLAY_AUTOMATCH_ITEMS=1` (default automatch is no-items).
4. **4-player / high delay** — ring depth 64+; global-min mismatch across remote slots + paced resim. **Lab (env-only):** host `SSB64_NETPLAY_REMOTE_SLOTS=1,2,3`; `SSB64_NETPLAY_ROLLBACK_CORRECTION_TUPLE_LOG=1`; inject mismatches on slots 1 and 3 → `CORRECTION_TUPLE` shows `source=timeline_global` with `mismatch=min(1,3)`; one resim span covers all slots; per-slot `ROLLBACK_SYNC` via `PendingEpisodeBySlot`.
...
- **Minimum correction tuple (2026-05-27):** first stick @~450–500 — matching `exec_mismatch`/`exec_target`; `CORRECTION_TUPLE` shows `source=wire` or `timeline_*` (not `scan` unless batch); resim span ≤ `(frontier - mismatch)` unless predicted rows require extension.
