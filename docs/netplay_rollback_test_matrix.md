...
| Remote analog authority (FSM) | `SSB64_NETPLAY_ROLLBACK_EPISODE_FSM=1` + optional `SSB64_NETPLAY_AUTHORITY_PUBLISH_LOG=1`; **@930** — no `analog_onset_predict`, `LOCAL_PUBLISH` on moving local peer; **@1700** — `REMOTE_PUBLISH source=wire_confirmed` on authority peer for remote human; no `patch_publish` zeroing wire analog without `REMOTE_PUBLISH`; **commit** — no `FRAME_COMMIT_TOKEN_MISMATCH`, no `FC_LOCAL_AUTH_MISMATCH` / `FC_REMOTE_AUTH_MISMATCH`; matching `inp_all` + `slot_span_digest`; no `PEER_SNAPSHOT_DIVERGE` at episode load |
