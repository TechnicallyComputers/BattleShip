...
| `ftKirbySpecialLwReconcileStoneAfterRollback` | After snapshot apply, if live `duration<=0` but blob `duration>0`, restore blob value; arm release suppress |
| Live + blob light hash | Fold `kirby.speciallw.duration` when status in SpecialLw..SpecialAirLwEnd (`netsync.c` + blob hash) |
...
- `ring_save_player` live_light matches blob_light for stone ticks (duration now in live fold).
