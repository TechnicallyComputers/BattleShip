...
When skew pacing **suppresses `scene_update()`**, taskman still runs **`scVSBattleFuncUpdateSkewPacingNetSlice`** ([`decomp/src/sc/sccommon/scvsbattle.c`](../decomp/src/sc/sccommon/scvsbattle.c)): **`syNetPeerUpdateBattleGate`** + (when live sim is held) **`syNetRollbackPumpCorrectionBeforeBattleSim`** + **`syNetPeerUpdate`**. Ingress was already pumped in **`syNetInputFuncRead`**. This omits **`ifCommonBattleUpdateInterfaceAll`** (**`gcRunAll`**), **`syNetReplayUpdate`**, and **`syNetRollbackAfterBattleUpdate`** — no live sim step completed for that task iteration, so no live snapshot. Pump on the hold path is required so deferred GGPO can BeginResim while the live cap is active (see [`docs/bugs/netplay_deferred_ggpo_pump_before_tick_commit_hold_2026-07-12.md`](bugs/netplay_deferred_ggpo_pump_before_tick_commit_hold_2026-07-12.md)).
