...
- **`SSB64_NETPLAY_BATTLE_GO_LOG`** — **`1`**: log `battle_go_apply` (sim tick when `ifCommonAnnounceGoSetStatus` runs), `world_detail` at GO, and `defer_resim_seal_wait` while rollback awaits baseline/seal rows (interface + live battle held). Pair with **`HASH_TRANSITION_LOG=1`** and **`SIM_STATE_TICK_INTERVAL=1`** around match GO (~tick 385–390 in DK/Link soak). [`netsync.c`](port/net/sys/netsync.c), [`netrollback.c`](port/net/sys/netrollback.c)
- **`SSB64_NETPLAY_HASH_TRANSITION_LOG`** — **`1`**: log `hash_transition` when live `figh`/`world`/`item`/`rng`/`anim`/`gch`/`eff` partitions change between sim ticks (first divergence bisect). [`netpeer.c`](port/net/sys/netpeer.c)
...
- **`SSB64_NETPLAY_SNAPSHOT_MODELPART_DIAG=1`** — Log `modelpart_ensure_parts` when rollback load materializes missing `FTParts` before `ftParamSetModelPartID` (intro Appear egg-hatch bisect). [`netrollbacksnapshot.c`](port/net/sys/netrollbacksnapshot.c)
...
- **`SSB64_NETPLAY_INTRO_ANCHOR_SIM_TRAIL=1`** — With intro anchor probe active, log `intro_anchor_sim_trail` per fighter immediately before/after the probe +1 sim (`phase=pre` vs ring@`load_tick`, `phase=post` vs ring@`probe_tick`): Appear/MPColl scalars (`entry_pos_y`, TransN/TopN/gobj Y, `ptr_gobj`/`ptr_topn`, `pos_diff_y`, j0/TransN `anim_frame`). [`netrollbacksnapshot.c`](port/net/sys/netrollbacksnapshot.c), [`netrollback.c`](port/net/sys/netrollback.c)
...
- **`SSB64_NETPLAY_VS_PUSH_FRAME_HZ`** — Overrides the decoupled VS wall-slot cap (`port/gameloop.cpp`). **Unset**: cap **on** at negotiated contract VI Hz (typically **60**) on **all** OS — fixed cross-OS pacing without per-platform env. Sim-led grid (`anchor + syNetInputGetTick() * gran`) is automatic when exec-sync anchor is latched. **`0`**: disable wall sleep (lab). Positive **1–480**: explicit Hz override (bisect).
