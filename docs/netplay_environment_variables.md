...
- **`SSB64_NETPLAY_FRAME_COMMIT_VALIDATION_TICKS`** — Sim ticks between NetSync validation + cross-peer frame-commit compare (default **120** when unset; clamp **30–120**). Overrides RTT-tier policy when set (both peers must use the same value). With auto session params: **`rtt_ms < 150` → 120**, **`rtt_ms >= 150` → 60** (WAN).
...
- **`SSB64_NETPLAY_STRICT_R_ABORT_FRAMES`** — Consecutive strict **`R`** stall evaluations on the same sim tick before `strict remote MISS stall abort` tears down VS (default **180**). Abort no longer unlocks publish for that frame (`admission_letter` **`A`**).
...
- **`SSB64_NETPLAY_SNAPSHOT_EFFECT_DIAG=1`** — Save `effect_count`/`truncated` for userdata effects (rollback ring). [`netrollbacksnapshot.c`](port/net/sys/netrollbacksnapshot.c); see [`bugs/netrollback_effect_snapshot_presence_phase1_2026-05-25.md`](bugs/netrollback_effect_snapshot_presence_phase1_2026-05-25.md).
- **`SSB64_NETPLAY_ROLLBACK_VERIFY_EFFECT_HASH=1`** — Include `hash_effect` in post-load `LOAD_HASH_DRIFT` verify (default off). [`netrollback.c`](port/net/sys/netrollback.c)
...
- **`SSB64_NETPLAY_ITEM_HASH_TRACE=1`** — Whenever `syNetSyncLogItemHashWalkTrace` runs (e.g. with **`SSB64_NETPLAY_RESIM_TICK_TRACE=1`**), emits `item_hash_walk` begin/steps/end: sorted active items with `kind`, procedural **`type`** (effective item “opcode”), per-step rollback fold and running hash. [`netsync.c`](port/net/sys/netsync.c)
...
- **`SSB64_MATCHMAKING_TURN_SERVER`** — [`mm_turn.c`](port/net/matchmaking/mm_turn.c). Coturn `host:port` for **TURN Allocate** on the automatch UDP socket (default **`216.154.76.149:3478`**). Relay address is registered as **`turn_endpoint`** on queue/heartbeat and exchanged as **`peer_turn`** in match JSON (requires matchmaking server with `turn_endpoint` support).
...
- **`SSB64_NETPLAY_TURN_DISABLE`** — Non-zero: skip TURN allocate and TURN bootstrap fallback (STUN/direct only).
- **`SSB64_MATCHMAKING_PUBLIC_ENDPOINT`**, **`SSB64_MATCHMAKING_BIND`**, **`SSB64_MATCHMAKING_LAN_ENDPOINT`** — [`scautomatch.c`](decomp/src/netplay/sc/sccommon/scautomatch.c). Queue join opens one UDP socket and registers **`udp_endpoint`** (STUN) plus optional **`lan_endpoint`**. While queued, heartbeats refresh endpoints on the server (STUN re-probe throttled). After match, bootstrap reuses the socket. When local and opponent STUN share the same WAN IPv4 (**same router, different LAN subnets**), the client tries **reflexive `peer` host:port first**; **`peer_lan` is LAN-first only** when it shares the subnet of your registered **`lan_endpoint`** (Windows `/16` adapter prefixes are clamped to `/24` for reachability). Set **`SSB64_MATCHMAKING_PEER_LAN=ip:port`** if the server omits opponent `peer_lan` on LAN tests.
