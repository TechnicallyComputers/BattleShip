...
**Build variants (`SSB64_NETMENU`):** Default **OFF** (“offline”) — stock VS flow, `port/stubs/net_port_glue_offline.c` satisfies netpeer symbols, **no libcurl**. **ON** (“netmenu” / netplay) — full `port/net/**` mirror, **`find_package(CURL)` on every host OS** (macOS Homebrew, Linux distro packages, Windows MSVC vcpkg or MinGW `mingw-w64-curl`). Packaging: add `-DSSB64_NETMENU=ON` or `--netplay` for netmenu builds (`package-linux.sh`, `package-macos.sh`, `package-mingw-windows.sh`); offline is the default for all three when `--netplay` is omitted.
