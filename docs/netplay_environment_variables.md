...
- **`SSB64_NETPLAY_SNAPSHOT_FIGHTER_FIELD_DIFF=1`** — On `LOAD_HASH_DRIFT` when `figh` drifts, log named field deltas (`status`, `motion`, `mot_atk`, `vel_air_y`, top joint Y, `pos_prev_y`, joints 0–3 translate, Fox `fox_speciallw_effect_gobj_id`). [`netrollbacksnapshot.c`](port/net/sys/netrollbacksnapshot.c)
- **`SSB64_NETPLAY_SNAPSHOT_RING_SAVE_DIAG=1`** — After each snapshot save, log `ring_save_diag`: stored `ring_figh` vs live full/light, `ring_anim`, `ring_eff` (detect ring≠live on same peer). [`netrollbacksnapshot.c`](port/net/sys/netrollbacksnapshot.c)
