...
- **`SSB64_MATCHMAKING_PUBLIC_ENDPOINT`**, **`SSB64_MATCHMAKING_BIND`**, **`SSB64_MATCHMAKING_LAN_ENDPOINT`** — [`scautomatch.c`](decomp/src/netplay/sc/sccommon/scautomatch.c). Queue join opens one UDP socket and registers **`udp_endpoint`** (STUN) plus optional **`lan_endpoint`**. While queued, periodic heartbeats may refresh both on the server. After match, the client re-STUNs on that socket (logs if reflexive changed vs queue registration) before bootstrap. Bootstrap reuses the socket (`reuse_existing_socket`); reflexive **`peer`** is tried first; **`peer_lan`** is deferred to the next staging tick when reflexive fails **and** local WAN IPv4 matches opponent **`peer`** (`mmHostportWanIpv4Equal`).
