...
**Linux vs Windows:** With **`SSB64_NETMENU`**, Windows builds link the same manual UDP P2P + bootstrap path as Linux (`SSB64_NETPLAY_*` bind/peer, delay-sync, phase lock). Matchmaking (`mm_*`), automatch HTTPS/CURL, and `mm_server_barrier.c` remain omitted or stubbed on Windows until a follow-up ports those pieces.
