...
**Precedence (input delay):** When `SSB64_NETPLAY_MATCH_INPUT_DELAY` is set (0–99), it defines the **linked** wire delay contract (floor/ceiling in netpeer) **and** strict extra slack in `netinput` (capped at **4**). It overrides `SSB64_NETPLAY_DELAY`, `SSB64_NETPLAY_STRICT_SLACK_FRAMES`, `SSB64_NET_DELAY_FRAMES`, and `SSB64_NETPLAY_INPUT_EXEC_DELAY_FRAMES` for those purposes. **Automatch:** `syNetPeer`’s **delay floor** (and committed `D`) start at `min(server input_delay, match_delay)` when match delay is set; the **ceiling** and final linked contract still use the match value once the host arms full `D` via `INPUT_DELAY_SYNC` after skew is within ±1 tick (guest commits on local **`sim + N`** ticks per **`SSB64_NETPLAY_DELAY_SYNC_COMMIT_LEAD_TICKS`** (default **2**) and then restores the floor to the match target). When unset, use those per-layer variables for lab tuning.
...
| **strict slack** | Extra frames **`sim + D + slack`** ceiling from **`g_NetInputDelayFrames`** / match-linked cap (**≤ 4** when match delay is set). | **`wire_cap`** in diag; **`syNetPeerGetStrictRequiredWireTick`** caps how far ahead on the wire strict admission looks. |
...
| Goal | Setting |
|------|--------|
| Linked match delay off | **Unset** `SSB64_NETPLAY_MATCH_INPUT_DELAY` (it forces **both** wire delay and strict slack; slack is capped at **4**). |
...
- **`SSB64_NETPLAY_STRICT_SLACK_FRAMES`** — Strict extra slack frames (0–4) used by strict required-wire readiness (`required_wire = sim + D + slack`). Ignored if match delay set.
