...
- **`SSB64_MATCHMAKING_PUBLIC_ENDPOINT`**, **`SSB64_MATCHMAKING_BIND`**, **`SSB64_MATCHMAKING_LAN_ENDPOINT`** — [`scautomatch.c`](decomp/src/netplay/sc/sccommon/scautomatch.c). Queue join opens one UDP socket and registers **`udp_endpoint`** (STUN) plus optional **`lan_endpoint`**. While queued, heartbeats refresh endpoints on the server (STUN re-probe throttled). After match, bootstrap reuses the socket. When local and opponent STUN share the same WAN IPv4 (**same router**), the client tries **`peer_lan` first** and **does not** reflexive-punch the shared public IP (hairpin failure). Set **`SSB64_MATCHMAKING_PEER_LAN=ip:port`** if the server omits opponent `peer_lan` on LAN tests.
