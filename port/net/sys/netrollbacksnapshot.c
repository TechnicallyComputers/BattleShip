...
#define SYNETRB_SNAP_GROUND_PUPUPU_LEGACY_PAYLOAD_LEN \
	((u16)offsetof(SYNetRbSnapGroundPupupu, lr_players))
...
static SYNetRbSnapshotSlot sSYNetRbLoadFailRestoreVerifySlot;
static sb32 sSYNetRbLoadFailRestoreVerifyValid;
...
static sb32 syNetRbSnapEffectIdIsLiveYoshiEggLayShell(u32 effect_gobj_id);
...
static void syNetRbSnapReNormalizeSpawnLrInBlobOverlay(SYNetRbSnapFighterBlob *blob);
static void syNetRbSnapPruneUnmaterializableModelpartCursorsInBlob(SYNetRbSnapFighterBlob *blob, const FTStruct *fp);
...
static void syNetRbSnapCaptureFighterCoupledIds(SYNetRbSnapFighterBlob *blob, const FTStruct *fp)
{
	                               ...
	                               (fp->status_id <= nFTCommonStatusGuardEnd)))
	{
		...
		if (((fp->fkind != nFTKindYoshi) || (fp->is_shield != FALSE)) &&
		    (syNetRbSnapFighterInYoshiEggLayScope(fp) == FALSE))
		{
			GObj *fighter_gobj;
			GObj *live_shield;
							       ...
							       s32 fkind, s32 status_id, sb32 is_shield)
{
	...
	{
		/*
		 * guard.effect_gobj aliases twister.tornado_gobj, tarucann.tarucann_gobj, and
		 * captureyoshi.effect_gobj at union offset 0x08. Never scrub through the guard slot while those
		 * statuses own the union.
		 */
		if ((status_id != nFTCommonStatusTwister) && (status_id != nFTCommonStatusTaruCann) &&
		    (status_id != nFTCommonStatusCaptureYoshi) && (status_id != nFTCommonStatusYoshiEgg))
		{
...
static void syNetRbSnapSanitizeCopyCaptainEffectAttach(const SYNetRbSnapshotSlot *slot);
...
static void syNetRbSnapPruneExcessSlotMatchedQuakes(const SYNetRbSnapshotSlot *slot);
static void syNetRbSnapEjectAllNonCanonicalEffectsForVerify(const SYNetRbSnapshotSlot *slot,
                                                            GObj * const *canonical_gobj_ptrs,
                                                            s32 canonical_gobj_count);
static void syNetRbSnapEnforceSlotAuthoritativeEffectSet(SYNetRbSnapshotSlot *slot);
...
static void syNetRbSnapNormalizeCapturedYoshiEggLayIdentity(SYNetRbSnapshotSlot *slot);
...
static sb32 syNetRbSnapLooseEffectIsShieldCandidate(const GObj *gobj, const EFStruct *ep);
static GObj *syNetRbSnapFindLooseShieldEffectForFighter(GObj *fighter_gobj, FTStruct *fp);
static s32 syNetRbSnapCountLooseShieldEffectsForPlayer(s32 player);
static GObj *syNetRbSnapResolveFighterShieldBubbleGobj(GObj *fighter_gobj, FTStruct *fp);
static sb32 syNetRbSnapFighterShieldBubbleVisible(GObj *fighter_gobj, FTStruct *fp);
...
static void syNetRbSnapCoupleYoshiShieldEffectEscapeCosmetic(GObj *fighter_gobj, FTStruct *fp, GObj *effect_gobj);
static sb32 syNetRbSnapFighterInYoshiShieldEscapePresentationScope(const FTStruct *fp);
static void syNetRbSnapReconcileYoshiShieldEscapePresentation(const SYNetRbSnapshotSlot *slot);
...
static GObj *syNetRbSnapFindLiveShieldEffectForFighter(const GObj *fighter_gobj);
static GObj *syNetRbSnapFindLiveShieldEffectByPlayerSlot(s32 player);
static GObj *syNetRbSnapFindLiveShieldEffectForPlayer(s32 player);
static GObj *syNetRbSnapTryAdoptLiveShieldEffectForFighter(GObj *fighter_gobj, FTStruct *fp);
#if defined(SSB64_NETMENU)
static sb32 syNetRbSnapAnyYoshiEggLayHatchReplayPending(void);
#endif
static sb32 syNetRbSnapPeerYoshiEggHatchStressActive(s32 self_player);
static sb32 syNetRbSnapEffectBlobIsShieldKind(const SYNetRbSnapEffectBlob *blob);
...
static sb32 syNetRbSnapLiveEffectIsYoshiEggLay(const GObj *gobj, const EFStruct *ep);
static sb32 syNetRbSnapLiveEffectIsYoshiEggEscape(const GObj *gobj, const EFStruct *ep);
...
static void syNetRbSnapPruneDuplicateLiveYoshiEggLayEffects(const SYNetRbSnapshotSlot *slot);
static sb32 syNetRbSnapShieldMintBlockedByEggHatchStress(FTStruct *fp, const char **reason_out);
#if defined(SSB64_NETMENU)
...
static GObj *syNetRbSnapResolveYoshiEggLayParentGobj(const SYNetRbSnapshotSlot *slot,
                                                     const SYNetRbSnapEffectBlob *blob);
static GObj *syNetRbSnapResolveYoshiEggEscapeParentGobj(const SYNetRbSnapshotSlot *slot,
                                                        const SYNetRbSnapEffectBlob *blob);
static s32 syNetRbSnapYoshiEggEscapePlayerFromEffectBlob(const SYNetRbSnapEffectBlob *blob);
static sb32 syNetRbSnapBlobInYoshiShieldEscapeScope(const SYNetRbSnapFighterBlob *blob);
static sb32 syNetRbSnapSlotHasYoshiEggEscapeBlobForPlayer(const SYNetRbSnapshotSlot *slot, s32 player);
static sb32 syNetRbSnapSlotAnyFighterInYoshiShieldEscapeScope(const SYNetRbSnapshotSlot *slot);
static sb32 syNetRbSnapSlotAnyFighterInGuardScope(const SYNetRbSnapshotSlot *slot);
static sb32 syNetRbSnapSlotVerifyJointAnimFragile(const SYNetRbSnapshotSlot *slot);
static sb32 syNetRbSnapSlotVerifyEffectCouplingFragile(const SYNetRbSnapshotSlot *slot);
static sb32 syNetRbSnapEffectRespawnKindGobjCollisionPriority(u8 respawn_kind);
static void syNetRbSnapSanitizeSlotEffectGobjIdCollisions(SYNetRbSnapshotSlot *slot);
...
static GObj *syNetRbSnapResolveLiveEffectGobjForBlobHash(const SYNetRbSnapshotSlot *slot,
                                                         const SYNetRbSnapEffectBlob *blob);
...
static sb32 syNetRbSnapBlobInYoshiAerialLandingFragileScope(const SYNetRbSnapFighterBlob *blob);
static sb32 syNetRbSnapshotSlotYoshiAerialLandingFragile(const SYNetRbSnapshotSlot *slot);
static sb32 syNetRbSnapSlotIsEmergencySentinel(const SYNetRbSnapshotSlot *slot);
static GObj *syNetRbSnapFindLiveYoshiEggEscapeEffectRelaxedForFighter(const GObj *fighter_gobj);
static sb32 syNetRbSnapEffectBlobAuthoritativeForSlot(const SYNetRbSnapshotSlot *slot,
                                                      const SYNetRbSnapEffectBlob *blob);
static void syNetRbSnapPruneStaleYoshiEggEscapeEffects(const SYNetRbSnapshotSlot *slot);
static void syNetRbSnapPruneDuplicateYoshiEggEscapeEffects(const SYNetRbSnapshotSlot *slot);
...
/* Vanilla ftManager spawn uses GObj root; rollback load/reapply rebinds TopN after joint restore. */
static Vec3f *syNetRbSnapFighterMPCollTranslatePtrGobj(GObj *fighter_gobj)
{
	DObj *root_dobj;
	...
	return &root_dobj->translate.vec.f;
}

static Vec3f *syNetRbSnapFighterMPCollTranslatePtrTopN(FTStruct *fp)
{
	if ((fp != NULL) && (fp->joints[nFTPartsJointTopN] != NULL))
	{
		return &fp->joints[nFTPartsJointTopN]->translate.vec.f;
	}
	return NULL;
}
...
static sb32 syNetRbSnapStatusInAppearPresentationScope(s32 fkind, s32 status_id);
...
void syNetRbSnapshotRebindFighterMPCollForAnchorProbe(void)
{
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		gobj_translate = syNetRbSnapFighterMPCollTranslatePtrGobj(fighter_gobj);
		fp->coll_data.p_translate = gobj_translate;
		fp->coll_data.p_lr = &fp->lr;
		fp->coll_data.p_map_coll = &fp->coll_data.map_coll;
	}
}
...
static sb32 syNetRbSnapFighterLiveSanityOk(GObj *fighter_gobj, FTStruct *fp, const SYNetRbSnapFighterBlob *blob,
					   const char **out_fail);
static sb32 syNetRbSnapFighterBlobSanityOk(const SYNetRbSnapFighterBlob *blob, const char **out_fail);
static sb32 syNetRbSnapFighterInIntroLoadFidelityScope(const FTStruct *fp);
...
static void syNetRbSnapCaptureFighter(SYNetRbSnapFighterBlob *blob, FTStruct *fp, GObj *fighter_gobj)
{
	...
	syNetRbSnapshotLogFighterBlobStatusTrail("capture", syNetInputGetTick(), fp->player, blob);
	{
		const char *blob_fail;

		blob_fail = NULL;
		if (syNetRbSnapFighterBlobSanityOk(blob, &blob_fail) == FALSE)
		{
			port_log(
			    "SSB64 NetRbSnapshot: capture_blob_sanity_fail tick=%u player=%d fkind=%d status=%d motion=%d reason=%s\n",
			    syNetInputGetTick(),
			    (int)fp->player,
			    (int)fp->fkind,
			    (int)fp->status_id,
			    (int)fp->motion_id,
			    (blob_fail != NULL) ? blob_fail : "unknown");
		}
	}
	...
	blob->entry_pos = fp->entry_pos;
	blob->entry_pos_captured = TRUE;
...
#if defined(SSB64_NETMENU)
	syNetRbSnapPruneUnmaterializableModelpartCursorsInBlob(blob, fp);
	syNetRbSnapReNormalizeSpawnLrInBlobOverlay(blob);
	syNetRbSnapRefreshFighterBlobSimScalarsFromLive(blob, fp);
#endif
}

/*
 * Re-pin GObj + per-joint anim cursors from the ring blob after presentation sync or
 * syNetplayCanonicalizeFighterSimState. Canonicalize and syNetRbSnapApplyDObjAnim both propagate
 * parent_gobj->anim_frame from arbitrary joints; capture stores gobj_anim_frame independently.
 */
static void syNetRbSnapRestoreFighterAnimScalarsFromBlob(GObj *fighter_gobj, FTStruct *fp,
							 const SYNetRbSnapFighterBlob *blob)
{
...
#if defined(SSB64_NETMENU)
		joint->anim_frame = syNetplayQuantizeAnimScalar(blob->joint_anim[ji].anim_frame);
		joint->anim_wait = syNetplayQuantizeAnimScalar(blob->joint_anim[ji].anim_wait);
		joint->anim_speed = syNetplayQuantizeAnimScalar(blob->joint_anim[ji].anim_speed);
#else
		joint->anim_frame = blob->joint_anim[ji].anim_frame;
		joint->anim_wait = blob->joint_anim[ji].anim_wait;
		joint->anim_speed = blob->joint_anim[ji].anim_speed;
#endif
	}
}

/*
 * Re-pin per-joint translate/rotate/scale from the ring blob after presentation sync or
 * syNetplayCanonicalizeFighterSimState. Figatree refresh and canonicalize can clobber TopN
 * facing/position (Appear intro Kirby off-screen, camera-facing yaw).
 */
static void syNetRbSnapRestoreFighterJointPoseFromBlob(FTStruct *fp, const SYNetRbSnapFighterBlob *blob)
{
	s32 ji;
	...
	for (ji = 0; ji < FTPARTS_JOINT_NUM_MAX; ji++)
	{
		...
		syNetplayQuantizeVec3f(&joint->rotate.vec.f);
		syNetplayQuantizeVec3f(&joint->scale.vec.f);
#endif
	}
}

static void syNetRbSnapRestoreFighterGobjTransformFromBlob(GObj *fighter_gobj,
							   const SYNetRbSnapFighterBlob *blob)
{
	DObj *root_dobj;

	if ((fighter_gobj == NULL) || (blob == NULL))
	{
		return;
	}
	root_dobj = DObjGetStruct(fighter_gobj);
	...
	root_dobj->translate.vec.f = blob->gobj_translate;
	root_dobj->rotate.vec.f = blob->gobj_rotate;
#if defined(SSB64_NETMENU)
...
static sb32 syNetRbSnapFighterInAppearPresentationScope(const FTStruct *fp)
{
	...
	return syNetRbSnapStatusInAppearPresentationScope(fp->fkind, fp->status_id);
}
...
#if defined(SSB64_NETMENU)
#define SYNETRB_SNAP_HIDDENPART_ANIM_MASK                                                      \
	(~(FTANIM_FLAG_SUBMOTION_SCRIPT | FTANIM_FLAG_ANIMJOINT | FTANIM_FLAG_TRANSLATE_SCALES | \
	   FTANIM_FLAG_SHIELDPOSE | FTANIM_FLAG_ANIMLOCKS))

static sb32 syNetRbSnapBlobInAppearPresentationScope(const SYNetRbSnapFighterBlob *blob);

static s32 syNetRbSnapHiddenPartRootJointId(FTStruct *fp, s32 hiddenpart_id)
{
	FTHiddenPart *hp_table;
	const FTHiddenPart *hiddenpart;

	if ((fp == NULL) || (fp->attr == NULL) || (hiddenpart_id < 0) || (hiddenpart_id >= 32))
	{
		return -1;
	}
	hp_table = (FTHiddenPart *)PORT_RESOLVE(fp->attr->hiddenparts);
	if (hp_table == NULL)
	{
		return -1;
	}
	hiddenpart = &hp_table[hiddenpart_id];
	if ((hiddenpart->root_joint_id < 0) || (hiddenpart->root_joint_id >= FTPARTS_JOINT_NUM_MAX))
	{
		return -1;
	}
	return hiddenpart->root_joint_id;
}

static sb32 syNetRbSnapFighterJointIsHiddenPartRoot(FTStruct *fp, s32 joint_id)
{
	FTHiddenPart *hp_table;
	s32 i;

	if ((fp == NULL) || (fp->attr == NULL) || (joint_id < 0))
	{
		return FALSE;
	}
	hp_table = (FTHiddenPart *)PORT_RESOLVE(fp->attr->hiddenparts);
	...
	for (i = 0; i < 32; i++)
	{
		const FTHiddenPart *hiddenpart = &hp_table[i];

		if (hiddenpart->root_joint_id != joint_id)
		{
			continue;
		}
		if ((hiddenpart->parent_joint_id >= 0) &&
		    (hiddenpart->parent_joint_id < FTPARTS_JOINT_NUM_MAX))
		{
			return TRUE;
		}
	}
...
/*
 * Mirror ftMainSetStatus hidden-part Add/Update/Eject when anim_desc hidden-joint bits change
 * (Yoshi egg hatch creates fp->joints[root] via ftMainUpdateHiddenPartID).
 */
static void syNetRbSnapReconcileFighterHiddenPartsFromAnimDesc(FTStruct *fp, FTAnimDesc from_desc,
							       FTAnimDesc to_desc)
{
	u32 anim_desc_bak;
	...
	anim_desc_bak = from_desc.word & SYNETRB_SNAP_HIDDENPART_ANIM_MASK;
	anim_desc_update = to_desc.word & SYNETRB_SNAP_HIDDENPART_ANIM_MASK;
	for (i = 0; ((anim_desc_bak != 0U) || (anim_desc_update != 0U)); i++, anim_desc_update <<= 1,
	    anim_desc_bak <<= 1)
	{
		if ((anim_desc_bak & (1U << 31)) == 0U)
		{
			if (anim_desc_update & (1U << 31))
			{
				ftMainUpdateHiddenPartID(fp, i);
			}
		}
		else if (anim_desc_update & (1U << 31))
		{
			s32 root_joint_id;

			/*
			 * ftMainAddHiddenPartID no-ops when root joint is NULL — use Update to create the
			 * hidden joint when both from/to bits are set but rollback never ran SetStatus.
			 */
			root_joint_id = syNetRbSnapHiddenPartRootJointId(fp, i);
			if ((root_joint_id >= 0) && (fp->joints[root_joint_id] == NULL))
			{
				ftMainUpdateHiddenPartID(fp, i);
			}
			else
		...
		{
			ftMainEjectHiddenPartID(fp, i);
		}
	}
}

/*
 * After rollback apply / figatree refresh, anim_desc already matches blob but hidden joints may
 * still be absent (SetStatus path never ran). Materialize any enabled hidden part whose root joint
 * slot is still NULL. `joint_valid` (when non-NULL) is the blob's capture-time presence table —
 * never materialize a joint the capture fold did not see, or the verify hash drifts (live fold
 * walks fp->joints[ji] != NULL per index).
 */
static void syNetRbSnapEnsureFighterHiddenPartsFromAnimDesc(FTStruct *fp, FTAnimDesc target_desc,
							    const u8 *joint_valid)
{
	FTHiddenPart *hp_table;
	u32 anim_bits;
	s32 i;
	...
	anim_bits = target_desc.word & SYNETRB_SNAP_HIDDENPART_ANIM_MASK;
	for (i = 0; anim_bits != 0U; i++, anim_bits <<= 1)
	{
		if (anim_bits & (1U << 31))
		{
			const FTHiddenPart *hiddenpart = &hp_table[i];
			...
			{
				if ((joint_valid != NULL) && (joint_valid[hiddenpart->root_joint_id] == FALSE))
				{
					continue;
				}
				ftMainUpdateHiddenPartID(fp, i);
...
/*
 * anim_desc hidden-joint bits can be clear while blob modelpart_status still records an active
 * cursor on the hidden root (egg hatch mid-Appear). Materialize from modelpart when parent exists.
 */
static void syNetRbSnapEnsureFighterHiddenPartsFromBlob(FTStruct *fp, const SYNetRbSnapFighterBlob *blob)
{
	FTHiddenPart *hp_table;
	s32 i;

	if ((fp == NULL) || (blob == NULL) || (fp->attr == NULL))
	{
		return;
	}
	syNetRbSnapEnsureFighterHiddenPartsFromAnimDesc(fp, blob->anim_desc, blob->joint_is_valid);
	...
	for (i = 0; i < 32; i++)
	{
		...
		root_joint_id = hiddenpart->root_joint_id;
		parent_joint_id = hiddenpart->parent_joint_id;
		if ((root_joint_id < nFTPartsJointCommonStart) || (root_joint_id >= FTPARTS_JOINT_NUM_MAX))
		{
			continue;
		}
		if ((parent_joint_id < 0) || (parent_joint_id >= FTPARTS_JOINT_NUM_MAX))
		{
			continue;
		}
		if (fp->joints[root_joint_id] != NULL)
		{
			continue;
		}
		/* Never materialize a joint the capture fold did not see — it drifts the verify hash. */
		if (blob->joint_is_valid[root_joint_id] == FALSE)
		{
			continue;
		}
		modelpart_index = root_joint_id - nFTPartsJointCommonStart;
		if (blob->modelpart_status[modelpart_index].modelpart_id_curr < 0)
		{
			continue;
		}
		if (fp->joints[parent_joint_id] == NULL)
		{
			continue;
		}
		ftMainUpdateHiddenPartID(fp, i);
...
/*
 * Enforce capture-time joint topology on hidden-part roots. The verify hash compares the live fold
 * (which walks fp->joints[ji] != NULL per index) against the fold captured at save time, so a hidden
 * joint materialized during load prep that did NOT exist at capture (blob->joint_is_valid == FALSE) —
 * or one the SetStatus hidden-part loop ejected that DID exist — drifts figh+anim on every walkback
 * tick even though every blob field diff passes. blob->joint_is_valid[] is the ground truth for what
 * the capture fold saw; eject/materialize hidden parts until live presence matches it.
 * Returns the number of topology changes so callers can re-pin blob pose/AObj state afterwards.
 */
static s32 syNetRbSnapReconcileFighterJointPresenceFromBlob(FTStruct *fp, const SYNetRbSnapFighterBlob *blob)
{
	FTHiddenPart *hp_table;
	s32 changed;
	s32 i;
	...
	for (i = 0; i < 32; i++)
	{
		...
		root_joint_id = hiddenpart->root_joint_id;
		if ((root_joint_id < 0) || (root_joint_id >= FTPARTS_JOINT_NUM_MAX))
		{
			continue;
		}
		live_present = (fp->joints[root_joint_id] != NULL) ? TRUE : FALSE;
		blob_present = (blob->joint_is_valid[root_joint_id] != FALSE) ? TRUE : FALSE;
		if (live_present == blob_present)
		{
			continue;
		}
		if (blob_present == FALSE)
		{
			ftMainEjectHiddenPartID(fp, i);
		}
		else
		{
			if ((hiddenpart->parent_joint_id < 0) ||
			    (hiddenpart->parent_joint_id >= FTPARTS_JOINT_NUM_MAX) ||
			    (fp->joints[hiddenpart->parent_joint_id] == NULL))
			{
				continue;
			}
			ftMainUpdateHiddenPartID(fp, i);
		}
		port_log("SSB64 NetRbSnapshot: intro_joint_presence player=%d fkind=%d hpid=%d root=%d action=%s "
		         "now_present=%d\n",
		         (int)fp->player, (int)fp->fkind, (int)i, (int)root_joint_id,
		         (blob_present == FALSE) ? "eject" : "materialize",
		         (fp->joints[root_joint_id] != NULL) ? 1 : 0);
		changed++;
	}
	return changed;
}

static s32 syNetRbSnapReconcileFighterJointPresenceFromSlot(const SYNetRbSnapshotSlot *slot)
{
	GObj *fighter_gobj;
	s32 changed_total;
...
static sb32 syNetRbSnapBlobInIntroLoadFidelityScope(const SYNetRbSnapFighterBlob *blob)
{
	if (blob == NULL)
	{
		return FALSE;
	...
	if ((blob->status_id == nFTCommonStatusEntry) || (blob->status_id == nFTCommonStatusEntryNull))
	{
		return TRUE;
	}
	return FALSE;
...
static void syNetRbSnapSyncFighterHiddenPartsFromBlob(FTStruct *fp, const SYNetRbSnapFighterBlob *blob)
{
	if ((fp == NULL) || (blob == NULL))
	{
		return;
	...
	if ((syNetRbSnapFighterInIntroLoadFidelityScope(fp) == FALSE) &&
	    (syNetRbSnapBlobInIntroLoadFidelityScope(blob) == FALSE))
	{
		return;
	}
	syNetRbSnapEnsureFighterHiddenPartsFromBlob(fp, blob);
}

static void syNetRbSnapSyncFighterHiddenPartsForSlot(const SYNetRbSnapshotSlot *slot)
{
	GObj *fighter_gobj;
...
#if defined(SSB64_NETMENU)
				/*
				 * Figatree-only DObjs can carry modelpart cursors without an fp->joints[] slot.
				 * Hidden-part roots must still be materialized before sanity passes when the capture
				 * fold recorded the joint (blob joint_is_valid); stale cursors on absent joints are
				 * ignored to match the verify hash fold.
				 */
				if (syNetRbSnapFighterJointIsHiddenPartRoot(fp, joint_id) == FALSE)
				{
					continue;
				}
				if ((blob != NULL) && (blob->is_valid != FALSE) &&
				    (blob->joint_is_valid[joint_id] == FALSE))
				{
					continue;
				}
#endif
...
static sb32 syNetRbSnapStatusInAppearPresentationScope(s32 fkind, s32 status_id)
{
	if (syNetRbSnapInferAppearSpawnLrFromStatusId(fkind, status_id) != 0)
	{
		return TRUE;
	}
	switch (fkind)
	...
	case nFTKindNCaptain:
		return ((status_id >= nFTCaptainStatusAppearRStart) && (status_id <= nFTCaptainStatusAppearLEnd)) ? TRUE
		                                                                                                  : FALSE;
	case nFTKindNess:
	case nFTKindNNess:
		return ((status_id >= nFTNessStatusAppearRStart) && (status_id <= nFTNessStatusAppearLEnd)) ? TRUE
		                                                                                            : FALSE;
	case nFTKindBoss:
		return (status_id == nFTBossStatusAppear) ? TRUE : FALSE;
	default:
		return FALSE;
	}
}

sb32 syNetRbSnapshotStatusInAppearPresentationScope(s32 fkind, s32 status_id)
{
	return syNetRbSnapStatusInAppearPresentationScope(fkind, status_id);
}

static const char *syNetRbSnapAppearFacingLabelFromStatusId(s32 fkind, s32 status_id)
{
	s32 spawn_lr;

	spawn_lr = syNetRbSnapInferAppearSpawnLrFromStatusId(fkind, status_id);
	if (spawn_lr == 1)
	{
		return "AppearR";
	}
...
static void syNetRbSnapReNormalizeSpawnLrInBlobOverlay(SYNetRbSnapFighterBlob *blob)
{
	...
	if ((blob == NULL) || (blob->spawn_lr_captured == FALSE))
	{
		return;
	}
	blob_sv = (union FTStatusVars *)blob->status_vars;
	syNetRbSnapNormalizeEntryLrInOverlay(&blob_sv->common.entry, (s32)blob->spawn_lr);
}

/*
 * Capture-time mirror of load-time modelpart prune: do not persist cursors on joints the fold
 * cannot materialize (no fp->joints[] stub or no FTParts at save). Keeps blob modelpart_status
 * honest for load verify and post-gate cosmetic DL push.
 */
static void syNetRbSnapPruneUnmaterializableModelpartCursorsInBlob(SYNetRbSnapFighterBlob *blob, const FTStruct *fp)
{
	s32 ji;
	...
	for (ji = 0; ji < modelpart_count; ji++)
	{
		...
		if (blob->modelpart_status[ji].modelpart_id_curr < 0)
		{
			continue;
		}
		joint_id = ji + nFTPartsJointCommonStart;
		if (blob->joint_is_valid[joint_id] == FALSE)
		{
			blob->modelpart_status[ji].modelpart_id_curr = -1;
			continue;
		}
		joint = fp->joints[joint_id];
		if (joint == NULL)
...
static sb32 syNetRbSnapBlobInAppearPresentationScope(const SYNetRbSnapFighterBlob *blob)
{
	...
	return syNetRbSnapStatusInAppearPresentationScope(blob->fkind, blob->status_id);
}

static void syNetRbSnapInferFighterEntryPosFromAppearJoints(FTStruct *fp)
{
	DObj *topn_joint;
	DObj *transn_joint;
	...
	fp->entry_pos.y = topn_joint->translate.vec.f.y - transn_joint->translate.vec.f.y;
	if (ftStatusVarsEntry(fp)->is_rotate != FALSE)
	{
		fp->entry_pos.x = topn_joint->translate.vec.f.x + transn_joint->translate.vec.f.x;
		fp->entry_pos.z = topn_joint->translate.vec.f.z + transn_joint->translate.vec.f.z;
	}
	else
	{
		fp->entry_pos.x = topn_joint->translate.vec.f.x - transn_joint->translate.vec.f.x;
...
#if defined(SSB64_NETMENU)
	syNetplayQuantizeVec3f(&fp->entry_pos);
#endif
}
...
static void syNetRbSnapRestoreFighterEntryPosFromBlob(FTStruct *fp, const SYNetRbSnapFighterBlob *blob)
{
	...
	if (blob->entry_pos_captured != FALSE)
	{
		fp->entry_pos = blob->entry_pos;
#if defined(SSB64_NETMENU)
		syNetplayQuantizeVec3f(&fp->entry_pos);
#endif
		return;
	...
	if ((syNetRbSnapBlobInAppearPresentationScope(blob) != FALSE) ||
	    (blob->status_id == nFTCommonStatusEntry) || (blob->status_id == nFTCommonStatusEntryNull))
	{
		syNetRbSnapInferFighterEntryPosFromAppearJoints(fp);
	}
}

static void syNetRbSnapRestoreFighterPostCanonicalizeFromBlob(GObj *fighter_gobj, FTStruct *fp,
							      const SYNetRbSnapFighterBlob *blob)
{
	if ((fighter_gobj == NULL) || (fp == NULL) || (blob == NULL))
	{
...
#if defined(SSB64_NETMENU)
static sb32 syNetRbSnapModelPartDiagEnabled(void)
{
	...
	env = getenv("SSB64_NETPLAY_SNAPSHOT_MODELPART_DIAG");
	s_cached = ((env != NULL) && (env[0] != '\0') && (atoi(env) != 0)) ? 1 : 0;
	return (s_cached != 0) ? TRUE : FALSE;
}

static sb32 syNetRbSnapAppearPresentationDiagEnabled(void)
{
	static int s_cached = -999;
	const char *env;
	...
	env = getenv("SSB64_NETPLAY_SNAPSHOT_APPEAR_PRESENTATION_DIAG");
	s_cached = ((env != NULL) && (env[0] != '\0') && (atoi(env) != 0)) ? 1 : 0;
	return (s_cached != 0) ? TRUE : FALSE;
}
...
static void syNetRbSnapLogAppearPresentationDiag(const SYNetRbSnapshotSlot *slot, u32 tick, const char *phase)
{
	...
	for (pidx = 0; pidx < GMCOMMON_PLAYERS_MAX; pidx++)
	{
		...
		joint1 = fp->joints[1];
		topn_ry_live = (topn != NULL) ? topn->rotate.vec.f.y : 0.0F;
		...
		spawn_lr_sidecar = (blob->spawn_lr_captured != FALSE) ? (s32)blob->spawn_lr : 0;
		port_log(
		    "SSB64 NetRbSnapshot: appear_presentation_diag phase=%s tick=%u player=%d fkind=%d status=%d "
		    "appear_facing=%s lr=%d entry_lr=%d infer_lr=%d spawn_lr_captured=%d spawn_lr_sidecar=%d "
		    "entry_is_rotate=%d entry_pos_y_live=0x%08X entry_pos_y_blob=0x%08X "
		    ...
		    phase_label, tick, (int)pidx, (int)fp->fkind, (int)fp->status_id,
		    syNetRbSnapAppearFacingLabelFromStatusId(fp->fkind, fp->status_id), (int)fp->lr,
		    (int)ftStatusVarsEntry(fp)->lr, (int)infer_lr, (int)blob->spawn_lr_captured,
		    (int)spawn_lr_sidecar, (int)ftStatusVarsEntry(fp)->is_rotate,
		    ...
		    syNetRbSnapF32DiagBits(topn_ry_blob), syNetRbSnapF32DiagBits(joint1_ry_live),
		    syNetRbSnapF32DiagBits(joint1_ry_blob));
		for (ji = 0; ji < modelpart_count; ji++)
		{
			...
			live_mp = fp->modelpart_status[ji].modelpart_id_curr;
			blob_mp = blob->modelpart_status[ji].modelpart_id_curr;
...
/*
 * Figatree refresh walks joints expecting user_data.p FTParts (lbCommonAddFighterPartsFigatree).
 * After rollback load/reapply some joints exist but lost FTParts — allocate before modelpart push.
 */
static void syNetRbSnapEnsureFighterJointParts(FTStruct *fp, s32 joint_id)
{
	DObj *joint;
	FTParts *parts;
	FTAttributes *attr;
	...
	if ((joint_id < 0) || (joint_id >= FTPARTS_JOINT_NUM_MAX))
	{
		return;
	}
	joint = fp->joints[joint_id];
	...
	parts = ftManagerGetNextPartsAlloc();
	joint->user_data.p = parts;
	...
	{
		parts->flags =
		    commonparts_container->commonparts[fp->detail_curr - nFTPartsDetailStart].flags;
	}
	else
	{
		parts->flags = 0;
	}
	parts->joint_id = joint_id;
	parts->gobj = NULL;
	...
	if (fp->costume != 0)
	{
		...
		if ((accesspart != NULL) && (joint_id == accesspart->joint_id))
		{
			parts->gobj =
			    gcMakeGObjSPAfter(nGCCommonKindFighterParts, NULL, nGCCommonLinkIDFighterParts,
					      GOBJ_PRIORITY_DEFAULT);
			gcAddDObjForGObj(parts->gobj, FTACCESSPART_GET_DL(accesspart));
			lbCommonAddMObjForFighterPartsDObj(DObjGetStruct(parts->gobj),
							   FTACCESSPART_GET_MOBJSUBS(accesspart),
							   FTACCESSPART_GET_COSTUME_MATANIM_JOINTS(accesspart), NULL,
							   fp->costume);
		}
	}

#if defined(SSB64_NETMENU)
	if (syNetRbSnapModelPartDiagEnabled() != FALSE)
	{
		port_log("SSB64 NetRbSnapshot: modelpart_ensure_parts player=%d joint=%d\n", fp->player,
			 joint_id);
	}
#endif
}
...
/*
 * ftMainRefreshFigatreeVisual does not replay motion SetModelPartID events. Push blob
 * modelpart/texturepart cursors to the live figatree after joint re-stamp.
 *
 * ftParamSetModelPartID calls ftGetParts(joint) before its joint NULL check — skip NULL joints.
 * Materialize FTParts on live joints first (intro Appear egg-hatch DLs; was asymmetric skip).
 * Call Set* before memcpy: live cursors may already match blob after ApplyFighter, but figatree
 * DLs were clobbered; force a cursor mismatch so DL/MObj replay runs.
 */
static sb32 syNetRbSnapFighterHasTexturePartsContainer(const FTStruct *fp)
{
	if ((fp == NULL) || (fp->attr == NULL))
	{
		return FALSE;
	}
	return (fp->attr->textureparts_container != NULL) ? TRUE : FALSE;
}

static void syNetRbSnapApplyFighterModelPartsFromBlob(GObj *fighter_gobj, FTStruct *fp,
						    const SYNetRbSnapFighterBlob *blob)
{
	s32 ji;
	s32 modelpart_count;
	...
	{
		s32 joint_id = ji + nFTPartsJointCommonStart;
		s32 modelpart_id = blob->modelpart_status[ji].modelpart_id_curr;

		if (modelpart_id < 0)
		{
			continue;
		}
		if ((fp->joints[joint_id] == NULL) || (ftGetParts(fp->joints[joint_id]) == NULL))
		{
			continue;
		}
	...
	if (syNetRbSnapFighterHasTexturePartsContainer(fp) != FALSE)
	{
...
static void syNetRbSnapEnsureAllFighterJointParts(FTStruct *fp)
{
	s32 ji;
	...
	for (ji = 0; ji < FTPARTS_JOINT_NUM_MAX; ji++)
	{
		syNetRbSnapEnsureFighterJointParts(fp, ji);
	}
}

#if defined(SSB64_NETMENU)
/*
 * Tree-only DObj under TopN->child with no fp->joints[] entry — still visited by
 * lbCommonAddFighterPartsFigatree. Minimal FTParts so figatree bind does not SIGSEGV.
 */
static void syNetRbSnapEnsureFigatreeSubtreePartsOnDObj(FTStruct *fp, DObj *dobj)
{
	FTParts *parts;
	FTAttributes *attr;
	FTCommonPartContainer *commonparts_container;

	if ((fp == NULL) || (dobj == NULL) || (dobj->user_data.p != NULL))
	{
		return;
	}
	...
	commonparts_container =
	    (attr != NULL) ? (FTCommonPartContainer *)PORT_RESOLVE(attr->commonparts_container) : NULL;
	if (commonparts_container != NULL)
	{
		parts->flags =
		    commonparts_container->commonparts[fp->detail_curr - nFTPartsDetailStart].flags;
	...
	if (syNetRbSnapModelPartDiagEnabled() != FALSE)
	{
		port_log("SSB64 NetRbSnapshot: modelpart_ensure_figatree_subtree player=%d dobj=%p\n",
			 fp->player, (void *)dobj);
	}
}

/*
 * lbCommonAddFighterPartsFigatree walks every DObj under TopN->child; fp->joints[] may omit
 * nodes that still appear in the figatree walk — materialize FTParts before Sync.
 */
static void syNetRbSnapEnsureFigatreeSubtreeParts(FTStruct *fp)
{
	DObj *root_dobj;
	DObj *current_dobj;
	s32 ji;
	...
	current_dobj = root_dobj;
	while (current_dobj != NULL)
	{
		if (current_dobj->user_data.p == NULL)
		{
			s32 joint_id_hint = -1;

			for (ji = 0; ji < FTPARTS_JOINT_NUM_MAX; ji++)
			{
				if (fp->joints[ji] == current_dobj)
				{
					joint_id_hint = ji;
					break;
				}
			...
			if (joint_id_hint >= 0)
			{
				syNetRbSnapEnsureFighterJointParts(fp, joint_id_hint);
			}
			else
			{
				syNetRbSnapEnsureFigatreeSubtreePartsOnDObj(fp, current_dobj);
			}
		}
...
static void syNetRbSnapEnsureAllFighterJointPartsForSlot(const SYNetRbSnapshotSlot *slot)
{
	GObj *fighter_gobj;
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		if (slot->fighters[slot_index].is_valid == FALSE)
		{
			continue;
		}
...
static void syNetRbSnapApplyFighterJointPoseAndAnimFromBlob(FTStruct *fp, GObj *fighter_gobj,
							    const SYNetRbSnapFighterBlob *blob)
{
	s32 ji;
	...
	for (ji = 0; ji < FTPARTS_JOINT_NUM_MAX; ji++)
	{
		if ((blob->joint_is_valid[ji] == FALSE) || (fp->joints[ji] == NULL))
		{
			continue;
		}
		{
...
static void syNetRbSnapApplyFighter(const SYNetRbSnapFighterBlob *blob, FTStruct *fp, GObj *fighter_gobj)
{
...
#if defined(SSB64_NETMENU)
	if ((syNetRbSnapFighterInIntroLoadFidelityScope(fp) != FALSE) &&
	    (syNetRbSnapFighterInGrabThrowSynctestFragileScope(fp) == FALSE))
	{
		syNetRbSnapReconcileFighterHiddenPartsFromAnimDesc(fp, fp->anim_desc, blob->anim_desc);
	}
#endif
	...
	syNetRbSnapshotLogFighterBlobStatusTrail("apply_after", syNetInputGetTick(), fp->player, blob);
	{
		...
		{
			port_log(
			    "SSB64 NetRbSnapshot: apply_blob_sanity_fail tick=%u player=%d fkind=%d status=%d motion=%d reason=%s\n",
			    syNetInputGetTick(),
			    (int)fp->player,
			    (int)fp->fkind,
			    (int)blob->status_id,
		...
		if (syNetRbSnapFighterLiveSanityOk(fighter_gobj, fp, blob, &live_fail) == FALSE)
		{
			port_log(
			    "SSB64 NetRbSnapshot: apply_live_sanity_fail tick=%u player=%d fkind=%d status=%d motion=%d reason=%s\n",
			    syNetInputGetTick(),
			    (int)fp->player,
			    (int)fp->fkind,
			    (int)fp->status_id,
			    (int)fp->motion_id,
			    (live_fail != NULL) ? live_fail : "unknown");
		}
	}
}
...
static sb32 syNetRbSnapFighterStatusTrailEnabled(void)
{
	...
	e = getenv("SSB64_NETPLAY_FIGHTER_STATUS_TRAIL");
	if ((e != NULL) && (e[0] != '\0'))
	{
		s_env_cache = (atoi(e) != 0) ? 1 : 0;
	}
	else if (syNetRbSnapFighterDiagEnabled() != FALSE)
	{
		s_env_cache = 1;
	}
	else
					   ...
					   const char **out_fail)
{
	...
	if (fp->status_id < 0)
	{
		if (out_fail != NULL)
		{
			*out_fail = "status_negative";
		}
		return FALSE;
	}
	if ((fighter_gobj != NULL) && (DObjGetStruct(fighter_gobj) == NULL))
	{
		if (out_fail != NULL)
		{
			*out_fail = "null_dobj";
	...
	if ((fp->stock_count > 0) && (fp->status_id < nFTCommonStatusDeadDown) &&
	    (fp->status_id > nFTCommonStatusDeadUpFall) && (fp->status_id != nFTCommonStatusSleep) &&
	    (fp->status_id != nFTCommonStatusEntry) && (fp->proc_update == NULL) && (fp->proc_physics == NULL) &&
	...
	if (syNetRbSnapFighterAppearPresentationOk(fighter_gobj, fp, blob, out_fail) == FALSE)
	{
		return FALSE;
	}
	return TRUE;
...
static sb32 syNetRbSnapFighterBlobSanityOk(const SYNetRbSnapFighterBlob *blob, const char **out_fail)
{
	...
	if (blob->motion_id < 0)
	{
		if (out_fail != NULL)
		{
			*out_fail = "motion_negative";
		}
		return FALSE;
	}
	if (blob->status_id >= FTSTAT_CHARDATA_START)
	{
		if (out_fail != NULL)
		{
			*out_fail = "status_out_of_range";
		}
		return FALSE;
	}
	return TRUE;
...
void syNetRbSnapshotLogFighterBlobStatusTrail(const char *tag, u32 tick, s32 player,
					      const void *blob_void)
{
	const SYNetRbSnapFighterBlob *blob = (const SYNetRbSnapFighterBlob *)blob_void;

	if ((syNetRbSnapFighterStatusTrailEnabled() == FALSE) || (tag == NULL) || (blob == NULL))
	{
		return;
	}
	port_log(
	    "SSB64 NetRbSnapshot: status_trail tag=%s tick=%u phase=blob player=%d fkind=%d status=%d motion=%d shield=%d valid=%d\n",
	    tag,
	    tick,
	    (int)player,
	    (int)blob->fkind,
...
void syNetRbSnapshotLogFighterStatusTrail(const char *tag, u32 tick)
{
	...
	if (syNetRbSnapFighterStatusTrailEnabled() == FALSE)
	{
		return;
	}
	slot = syNetRbSnapshotSlotForTick(tick);
	if ((tag != NULL) && (strcmp(tag, "load_fail_restore") == 0) && (sSYNetRbLoadFailRestoreVerifyValid != FALSE))
	{
		slot = &sSYNetRbLoadFailRestoreVerifySlot;
	}
	for (fighter_gobj = gGCCommonLinks[nGCCommonLinkIDFighter]; fighter_gobj != NULL;
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		blob = NULL;
		slot_index = fp->player;
		if ((slot != NULL) && (slot->is_valid != FALSE) && (slot_index >= 0) &&
		    (slot_index < GMCOMMON_PLAYERS_MAX))
		{
			if (((tag != NULL) && (strcmp(tag, "load_fail_restore") == 0) &&
			     (slot == &sSYNetRbLoadFailRestoreVerifySlot)) ||
			    (slot->tick == tick))
			{
				blob = &slot->fighters[slot_index];
		...
		port_log(
		    "SSB64 NetRbSnapshot: status_trail tag=%s tick=%u phase=live player=%d fkind=%d status=%d motion=%d shield=%d attr=%p data=%p proc_upd=%p proc_phy=%p proc_int=%p sanity=%d fail=%s\n",
		    (tag != NULL) ? tag : "?",
		    tick,
		    (int)fp->player,
		    (int)fp->fkind,
		    ...
		    (void *)fp->data,
		    (void *)fp->proc_update,
		    (void *)fp->proc_physics,
		    (void *)fp->proc_interrupt,
		    (syNetRbSnapFighterLiveSanityOk(fighter_gobj, fp, blob, &fail_reason) != FALSE) ? 1 : 0,
		    (fail_reason != NULL) ? fail_reason : "ok");
	}
}

sb32 syNetRbSnapshotVerifyLiveFightersSanity(u32 tick, const char *tag)
{
	GObj *fighter_gobj;
	sb32 ok;
	const SYNetRbSnapshotSlot *slot;
	sb32 use_load_fail_restore_slot;

	ok = TRUE;
	use_load_fail_restore_slot = FALSE;
	slot = syNetRbSnapshotSlotForTick(tick);
	if ((tag != NULL) && (strcmp(tag, "load_fail_restore") == 0) && (sSYNetRbLoadFailRestoreVerifyValid != FALSE))
	{
		slot = &sSYNetRbLoadFailRestoreVerifySlot;
		use_load_fail_restore_slot = TRUE;
	}
	for (fighter_gobj = gGCCommonLinks[nGCCommonLinkIDFighter]; fighter_gobj != NULL;
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		if (fp != NULL)
		{
			...
			if ((slot != NULL) && (slot->is_valid != FALSE) && (slot_index >= 0) &&
			    (slot_index < GMCOMMON_PLAYERS_MAX))
			{
				if ((use_load_fail_restore_slot != FALSE) || (slot->tick == tick))
				{
					blob = &slot->fighters[slot_index];
					if (blob->is_valid == FALSE)
					{
		...
		if (syNetRbSnapFighterLiveSanityOk(fighter_gobj, fp, blob, &fail_reason) == FALSE)
		{
			ok = FALSE;
			port_log(
			    "SSB64 NetRbSnapshot: fighter_sanity_fail tag=%s tick=%u player=%d fkind=%d status=%d motion=%d reason=%s\n",
			    (tag != NULL) ? tag : "?",
			    tick,
			    (fp != NULL) ? (int)fp->player : -1,
			    (fp != NULL) ? (int)fp->fkind : -1,
			    (fp != NULL) ? (int)fp->status_id : -1,
			    (fp != NULL) ? (int)fp->motion_id : -1,
			    (fail_reason != NULL) ? fail_reason : "unknown");
		}
	}
	if (use_load_fail_restore_slot != FALSE)
	{
		sSYNetRbLoadFailRestoreVerifyValid = FALSE;
	}
	return ok;
}

sb32 syNetRbSnapshotAnyLiveFighterInIntroLoadFidelityScope(void)
{
	GObj *fighter_gobj;
...
sb32 syNetRbSnapshotVerifyAppearPresentationIntegrity(u32 tick)
{
	GObj *fighter_gobj;
	sb32 ok;
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		fail_reason = NULL;
		if (syNetRbSnapFighterAppearPresentationOk(fighter_gobj, fp, NULL, &fail_reason) == FALSE)
		{
			ok = FALSE;
			port_log(
			    "SSB64 NetRbSnapshot: appear_presentation_fail tag=verify tick=%u player=%d fkind=%d status=%d reason=%s\n",
			    tick,
			    (fp != NULL) ? (int)fp->player : -1,
			    (fp != NULL) ? (int)fp->fkind : -1,
			    (fp != NULL) ? (int)fp->status_id : -1,
...
static void syNetRbSnapRebindFighterStatusProcs(GObj *fighter_gobj, FTStruct *fp)
{
	...
	else if (syNetRbSnapFighterIsInPKFireSpecialNStatus(fp) != FALSE)
	{
		if (fp->fkind == nFTKindKirby)
		{
					       ...
					       const SYNetRbSnapFighterBlob *blob)
{
	...
	if ((fighter_gobj != NULL) && (syNetRbSnapFighterIsInPKFireSpecialNStatus(fp) != FALSE))
	{
		syNetRbSnapRebindFighterStatusProcs(fighter_gobj, fp);
...
static u32 syNetRbSnapHashFighterBlobLight(const SYNetRbSnapFighterBlob *blob)
{
	    ...
	    (blob->status_id <= nFTKirbyStatusSpecialAirLwEnd))
	{
		...
		h = syNetRbSnapFnvAccumulateU32(h, (u32)blob->damage_resist);
	}
	if ((blob->status_id >= nFTCommonStatusDeadDown) && (blob->status_id <= nFTCommonStatusDeadUpFall))
	{
...
static u32 syNetRbSnapHashF32LiveFold(f32 value)
{
#if defined(SSB64_NETMENU)
	value = syNetplayQuantizeF32(value);
#endif
...
static void syNetRbSnapLogFieldDiffFoldF32(const char *tag, u32 tick, s32 player, const char *field, f32 live_value,
                                             f32 blob_value)
{
	u32 live_bits;
	u32 blob_bits;

	live_bits = syNetRbSnapHashF32LiveFold(live_value);
	blob_bits = syNetRbSnapHashF32ForFold(blob_value);
	syNetRbSnapLogFieldDiffScalar(tag, tick, player, field, live_bits, blob_bits);
}

/*
 * Layer-2 load_drift bisect: light/full hash oracle fields and full-fold joint participation
 * that layer-1 diag-bit TRS checks can miss (p_translate gate, TopN xz, fold-only joints, …).
 */
static void syNetRbSnapLogFighterFieldDiffSecondLayer(const char *tag, u32 tick, s32 player, FTStruct *fp,
                                                      const SYNetRbSnapFighterBlob *blob, u32 live_light,
                                                      u32 blob_light, u32 live_full, u32 blob_full)
{
	s32 ji;
	unsigned long long live_fold_mask;
	unsigned long long blob_fold_mask;
	unsigned long long live_only_mask;
	unsigned long long blob_only_mask;
	u32 live_p_translate;
	u32 blob_p_translate;

	if ((fp == NULL) || (blob == NULL) || (tag == NULL))
	{
		return;
	}
	if ((live_light == blob_light) && (live_full == blob_full))
	{
		return;
	}
	live_fold_mask = 0ULL;
	...
	for (ji = 0; ji < FTPARTS_JOINT_NUM_MAX; ji++)
	{
		if (fp->joints[ji] != NULL)
		{
			live_fold_mask |= (1ULL << ji);
		}
		if (blob->joint_is_valid[ji] != FALSE)
		{
			blob_fold_mask |= (1ULL << ji);
		}
	}
	live_only_mask = live_fold_mask & ~blob_fold_mask;
	blob_only_mask = blob_fold_mask & ~live_fold_mask;
	if (live_only_mask != 0ULL)
	{
		port_log(
		    "SSB64 NetRbSnapshot: fighter_field_diff tag=%s tick=%u player=%d field=fold_joint_live_only "
		    "live=0x%010llX blob=0x%010llX\n",
		    tag, tick, (int)player, live_only_mask, 0ULL);
	}
	if (blob_only_mask != 0ULL)
	{
		port_log(
		    "SSB64 NetRbSnapshot: fighter_field_diff tag=%s tick=%u player=%d field=fold_joint_blob_only "
		    "live=0x%010llX blob=0x%010llX\n",
		    tag, tick, (int)player, 0ULL, blob_only_mask);
	}
	syNetRbSnapLogFieldDiffScalar(tag, tick, player, "fold_percent_damage", (u32)fp->percent_damage,
	                              (u32)blob->percent_damage);
	syNetRbSnapLogFieldDiffScalar(tag, tick, player, "fold_stock_count", (u32)fp->stock_count,
	                              (u32)blob->stock_count);
	syNetRbSnapLogFieldDiffScalar(tag, tick, player, "fold_lr", (u32)fp->lr, (u32)blob->lr);
	...
	syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_vel_ground_x", fp->physics.vel_ground.x,
	                               blob->physics.vel_ground.x);
	syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_vel_ground_z", fp->physics.vel_ground.z,
	                               blob->physics.vel_ground.z);
	syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_vel_damage_ground", fp->physics.vel_damage_ground,
	                               blob->physics.vel_damage_ground);
	...
	if ((fp->joints[nFTPartsJointTopN] != NULL) || (blob->joint_is_valid[nFTPartsJointTopN] != FALSE))
	{
		...
		topn_tx_live = (fp->joints[nFTPartsJointTopN] != NULL)
		                   ? fp->joints[nFTPartsJointTopN]->translate.vec.f.x
		                   : 0.0F;
		topn_ty_live = (fp->joints[nFTPartsJointTopN] != NULL)
		                   ? fp->joints[nFTPartsJointTopN]->translate.vec.f.y
		                   : 0.0F;
		topn_tz_live = (fp->joints[nFTPartsJointTopN] != NULL)
		                   ? fp->joints[nFTPartsJointTopN]->translate.vec.f.z
		                   : 0.0F;
		syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_topn_tx", topn_tx_live,
		                               blob->joint_translate[nFTPartsJointTopN].x);
		syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_topn_ty", topn_ty_live,
		                               blob->joint_translate[nFTPartsJointTopN].y);
		syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_topn_tz", topn_tz_live,
		                               blob->joint_translate[nFTPartsJointTopN].z);
	}
	syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_vel_damage_air_x", fp->physics.vel_damage_air.x,
	                               blob->physics.vel_damage_air.x);
	syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_vel_damage_air_y", fp->physics.vel_damage_air.y,
	                               blob->physics.vel_damage_air.y);
	syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_vel_damage_air_z", fp->physics.vel_damage_air.z,
	...
	syNetRbSnapLogFieldDiffScalar(tag, tick, player, "fold_tap_stick_y", (u32)fp->tap_stick_y, (u32)blob->tap_stick_y);
	syNetRbSnapLogFieldDiffScalar(tag, tick, player, "fold_hold_stick_x", (u32)fp->hold_stick_x,
	                              (u32)blob->hold_stick_x);
	syNetRbSnapLogFieldDiffScalar(tag, tick, player, "fold_hold_stick_y", (u32)fp->hold_stick_y,
	                              (u32)blob->hold_stick_y);
	syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_coll_pos_diff_x", fp->coll_data.pos_diff.x,
	                               blob->coll.pos_diff.x);
	syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_coll_pos_diff_y", fp->coll_data.pos_diff.y,
	                               blob->coll.pos_diff.y);
	syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_coll_pos_diff_z", fp->coll_data.pos_diff.z,
	                               blob->coll.pos_diff.z);
	live_p_translate = (fp->coll_data.p_translate != NULL) ? 1U : 0U;
	blob_p_translate = (blob->coll_p_translate_valid != FALSE) ? 1U : 0U;
	syNetRbSnapLogFieldDiffScalar(tag, tick, player, "fold_p_translate_gate", live_p_translate, blob_p_translate);
	if ((fp->coll_data.p_translate != NULL) || (blob->coll_p_translate_valid != FALSE))
	{
		f32 ptr_x_live;
		f32 ptr_y_live;
		f32 ptr_z_live;

		ptr_x_live = (fp->coll_data.p_translate != NULL) ? fp->coll_data.p_translate->x : 0.0F;
		ptr_y_live = (fp->coll_data.p_translate != NULL) ? fp->coll_data.p_translate->y : 0.0F;
		ptr_z_live = (fp->coll_data.p_translate != NULL) ? fp->coll_data.p_translate->z : 0.0F;
		syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_p_translate_x", ptr_x_live, blob->gobj_translate.x);
		syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_p_translate_y", ptr_y_live, blob->gobj_translate.y);
		syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, "fold_p_translate_z", ptr_z_live, blob->gobj_translate.z);
	}
	...
	for (ji = 0; ji < FTPARTS_JOINT_NUM_MAX; ji++)
	{
		...
		if ((fp->joints[ji] == NULL) || (blob->joint_is_valid[ji] == FALSE))
		{
			continue;
		}
		joint = fp->joints[ji];
		snprintf(field_name, sizeof(field_name), "fold_j%d_tx", (int)ji);
		syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, field_name, joint->translate.vec.f.x,
		                               blob->joint_translate[ji].x);
		snprintf(field_name, sizeof(field_name), "fold_j%d_ty", (int)ji);
		syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, field_name, joint->translate.vec.f.y,
		                               blob->joint_translate[ji].y);
		snprintf(field_name, sizeof(field_name), "fold_j%d_tz", (int)ji);
		syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, field_name, joint->translate.vec.f.z,
		                               blob->joint_translate[ji].z);
		snprintf(field_name, sizeof(field_name), "fold_j%d_rx", (int)ji);
		syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, field_name, joint->rotate.vec.f.x,
		                               blob->joint_rotate[ji].x);
		snprintf(field_name, sizeof(field_name), "fold_j%d_ry", (int)ji);
		syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, field_name, joint->rotate.vec.f.y,
		                               blob->joint_rotate[ji].y);
		snprintf(field_name, sizeof(field_name), "fold_j%d_rz", (int)ji);
		syNetRbSnapLogFieldDiffFoldF32(tag, tick, player, field_name, joint->rotate.vec.f.z,
		                               blob->joint_rotate[ji].z);
	}
}
...
void syNetRbSnapshotLogRingBlobStatusTrailAtTick(const char *tag, u32 tick)
{
	...
	for (si = 0; si < GMCOMMON_PLAYERS_MAX; si++)
	{
		if (slot->fighters[si].is_valid != FALSE)
		{
			syNetRbSnapshotLogFighterBlobStatusTrail(tag, tick, si, &slot->fighters[si]);
		}
	}
}
...
void syNetRbSnapshotLogFighterFieldDiffAtTick(u32 tick, const char *tag)
{
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		blob = &slot->fighters[slot_index];
		{
			...
			blob_light = syNetRbSnapHashFighterBlobLight(blob);
			blob_full = syNetRbSnapHashFighterBlobFull(blob);
			blob_anim = syNetRbSnapHashFighterBlobAnim(blob);
			light_ok = (live_light == blob_light) ? 1U : 0U;
			full_ok = (live_full == blob_full) ? 1U : 0U;
			anim_ok = (live_anim == blob_anim) ? 1U : 0U;
			port_log(
			    "SSB64 NetRbSnapshot: fighter_field_diff tag=%s tick=%u player=%d live_fhash_light=0x%08X "
			    "live_fhash_full=0x%08X live_anim_hash=0x%08X blob_fhash_light=0x%08X blob_fhash_full=0x%08X "
			    "blob_anim_hash=0x%08X light_ok=%u full_ok=%u anim_ok=%u status=%d motion=%d\n",
			    reason, tick, (int)slot_index, live_light, live_full, live_anim, blob_light, blob_full,
		...
		if (((fp->fkind == nFTKindKirby) && (fp->status_id >= nFTKirbyStatusSpecialLwStart) &&
		    ...
		    ((blob->fkind == nFTKindKirby) && (blob->status_id >= nFTKirbyStatusSpecialLwStart) &&
		     (blob->status_id <= nFTKirbyStatusSpecialAirLwEnd)))
		{
			syNetRbSnapLogFieldDiffScalar(reason, tick, slot_index, "damage_resist", (u32)fp->damage_resist,
			                              (u32)blob->damage_resist);
		}
		if (((syNetRbSnapFighterInGuardScope(fp) != FALSE) || (syNetRbSnapBlobInGuardScope(blob) != FALSE)) &&
		    (syNetRbSnapFighterInYoshiEggLayScope(fp) == FALSE) &&
		    (syNetRbSnapBlobInYoshiEggLayScope(blob) == FALSE))
		{
		...
		syNetRbSnapLogFieldDiffScalar(reason, tick, slot_index, "entry_pos_x",
		                              syNetRbSnapF32DiagBits(fp->entry_pos.x),
		                              syNetRbSnapF32DiagBits(blob->entry_pos.x));
		syNetRbSnapLogFieldDiffScalar(reason, tick, slot_index, "entry_pos_y",
		                              syNetRbSnapF32DiagBits(fp->entry_pos.y),
		                              syNetRbSnapF32DiagBits(blob->entry_pos.y));
		syNetRbSnapLogFieldDiffScalar(reason, tick, slot_index, "entry_pos_z",
		                              syNetRbSnapF32DiagBits(fp->entry_pos.z),
		                              syNetRbSnapF32DiagBits(blob->entry_pos.z));
		if (fp->joints[nFTPartsJointTopN] != NULL)
		{
			syNetRbSnapLogFieldDiffScalar(
		...
		/*
		 * Joint topology diff: the live figh/anim folds walk fp->joints[ji] != NULL per index, so a
		 * joint present live but absent at capture (or vice versa) shifts the merged hash while every
		 * blob field diff above stays silent (those skip joint_is_valid == FALSE slots). Surface the
		 * presence mask plus per-joint AObj chain length / anim cursor drift for joints valid on both
		 * sides.
		 */
		{
			unsigned long long live_mask;
			unsigned long long blob_mask;
			...
			if (live_mask != blob_mask)
			{
				port_log(
				    "SSB64 NetRbSnapshot: fighter_field_diff tag=%s tick=%u player=%d "
				    "field=joint_presence live=0x%010llX blob=0x%010llX\n",
				    reason, tick, (int)slot_index, live_mask, blob_mask);
			}
			for (ji = 0; ji < FTPARTS_JOINT_NUM_MAX; ji++)
			{
				DObj *joint;
				...
				joint = fp->joints[ji];
				if ((joint == NULL) || (blob->joint_is_valid[ji] == FALSE))
				{
					continue;
				}
				chain_total = 0U;
				...
				snprintf(field_name, sizeof(field_name), "j%d_chain_total", (int)ji);
				syNetRbSnapLogFieldDiffScalar(reason, tick, slot_index, field_name, chain_total,
				                              (u32)blob->joint_anim[ji].aobj_chain_total);
				snprintf(field_name, sizeof(field_name), "j%d_anim_frame", (int)ji);
				syNetRbSnapLogFieldDiffScalar(reason, tick, slot_index, field_name,
				                              syNetRbSnapF32DiagBits(joint->anim_frame),
				                              syNetRbSnapF32DiagBits(blob->joint_anim[ji].anim_frame));
				snprintf(field_name, sizeof(field_name), "j%d_anim_wait", (int)ji);
				syNetRbSnapLogFieldDiffScalar(reason, tick, slot_index, field_name,
				                              syNetRbSnapF32DiagBits(joint->anim_wait),
				                              syNetRbSnapF32DiagBits(blob->joint_anim[ji].anim_wait));
				/*
				 * Full figh fold hashes every present joint's translate + rotate (xyz). Scalars above
				 * only cover TopN Y; log all joint_is_valid TRS deltas so load_drift bisect can name ji.
				 */
				snprintf(field_name, sizeof(field_name), "j%d_tx", (int)ji);
				syNetRbSnapLogFieldDiffScalar(
				    reason, tick, slot_index, field_name,
				    syNetRbSnapF32DiagBits(joint->translate.vec.f.x),
				    syNetRbSnapF32DiagBits(blob->joint_translate[ji].x));
				snprintf(field_name, sizeof(field_name), "j%d_ty", (int)ji);
				syNetRbSnapLogFieldDiffScalar(
				    reason, tick, slot_index, field_name,
				    syNetRbSnapF32DiagBits(joint->translate.vec.f.y),
				    syNetRbSnapF32DiagBits(blob->joint_translate[ji].y));
				snprintf(field_name, sizeof(field_name), "j%d_tz", (int)ji);
				syNetRbSnapLogFieldDiffScalar(
				    reason, tick, slot_index, field_name,
				    syNetRbSnapF32DiagBits(joint->translate.vec.f.z),
				    syNetRbSnapF32DiagBits(blob->joint_translate[ji].z));
				snprintf(field_name, sizeof(field_name), "j%d_rx", (int)ji);
				syNetRbSnapLogFieldDiffScalar(
				    reason, tick, slot_index, field_name,
				    syNetRbSnapF32DiagBits(joint->rotate.vec.f.x),
				    syNetRbSnapF32DiagBits(blob->joint_rotate[ji].x));
				snprintf(field_name, sizeof(field_name), "j%d_ry", (int)ji);
				syNetRbSnapLogFieldDiffScalar(
				    reason, tick, slot_index, field_name,
				    syNetRbSnapF32DiagBits(joint->rotate.vec.f.y),
				    syNetRbSnapF32DiagBits(blob->joint_rotate[ji].y));
				snprintf(field_name, sizeof(field_name), "j%d_rz", (int)ji);
				syNetRbSnapLogFieldDiffScalar(
				    reason, tick, slot_index, field_name,
				    syNetRbSnapF32DiagBits(joint->rotate.vec.f.z),
...
void syNetRbSnapshotLogGuardShieldLoadDriftDiag(u32 tick, u32 live_f, u32 slot_f, u32 live_a, u32 slot_a)
{
	GObj *fighter_gobj;
	SYNetRbSnapshotSlot *slot;
	sb32 figh_mismatch;
	...
	if ((live_f == slot_f) && (live_a == slot_a))
	{
		return;
	}
	figh_mismatch = (live_f != slot_f) ? TRUE : FALSE;
	anim_mismatch = (live_a != slot_a) ? TRUE : FALSE;
	slot = syNetRbSnapshotSlotForTick(tick);
	for (fighter_gobj = gGCCommonLinks[nGCCommonLinkIDFighter]; fighter_gobj != NULL;
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		if ((fp->shield_health <= 0) && (fp->is_shield == FALSE))
		{
			continue;
		}
		slot_index = fp->player;
		...
		live_anim = syNetSyncHashFighterSlotAnim(fp, fighter_gobj);
		port_log(
		    "SSB64 NetRbSnapshot: guard_shield_load_drift tick=%u player=%d fkind=%d status=%d motion=%d "
		    "shield=%d is_shield=%d figh_mismatch=%d anim_mismatch=%d fhash_light=0x%08X live_anim=0x%08X "
		    "blob_shield=%d blob_is_shield=%d\n",
		    tick,
		    (int)slot_index,
		    (int)fp->fkind,
		    (int)fp->status_id,
		    ...
		    live_light,
		    live_anim,
		    (blob != NULL) ? (int)blob->shield_health : -1,
		    (blob != NULL) ? (int)(blob->is_shield != FALSE) : -1);
	}
}
#endif /* PORT */
...
static sb32 syNetRbSnapEffectGobjListed(const GObj * const *out, s32 count, const GObj *gobj)
{
	s32 i;
	...
	for (link_pass = 0; link_pass < 2; link_pass++)
	{
		...
		for (gobj = link_head; gobj != NULL; gobj = gobj->link_next)
		{
			...
			if (ep != NULL)
			{
				...
				if (syNetRbSnapEffectGobjListed((const GObj * const *)out, count, gobj) != FALSE)
				{
					continue;
				}
				if (count < max)
...
/* efManagerCaptainFalconPunchMakeEffect joint indices (see decomp/src/ef/efmanager.c). */
static s32 syNetRbSnapInferCaptainFalconPunchJointIndex(const FTStruct *fp)
{
	if (fp == NULL)
	{
		return -1;
...
static sb32 syNetRbSnapFighterInCaptainFalconPunchEffectScope(const FTStruct *fp)
{
	if (fp == NULL)
	{
		return FALSE;
	}
	if (fp->status_id == nFTKirbyStatusCopyCaptainSpecialN)
	{
		return TRUE;
	}
	if (((fp->fkind == nFTKindCaptain) || (fp->fkind == nFTKindNCaptain)) &&
	    (fp->status_id == nFTCaptainStatusSpecialN))
	{
		return TRUE;
	}
	return FALSE;
...
static sb32 syNetRbSnapEffectBlobIsUserdataJoint(const SYNetRbSnapEffectBlob *blob)
{
	...
	return ((blob->respawn_kind == SYNETRB_EFFECT_RESPAWN_USERDATA_JOINT) ||
	        ((blob->snap_flags & SYNETRB_EFFECT_SNAP_USERDATA_JOINT) != 0U)) ?
	           TRUE :
	           FALSE;
}

static GObj *syNetRbSnapFindLiveUserdataJointEffectForFighter(const GObj *fighter_gobj)
{
	s32 pass;
...
static sb32 syNetRbSnapSlotListsUserdataJointAttachForFighterPlayer(const SYNetRbSnapshotSlot *slot, s32 player)
{
	...
	for (ei = 0; ei < slot->effect_count; ei++)
	{
		...
		if (syNetRbSnapEffectBlobIsUserdataJoint(blob) == FALSE)
		{
			continue;
		}
                                                       ...
                                                       const SYNetRbSnapEffectBlob *blob)
{
	...
	{
		joint_idx = syNetRbSnapInferCaptainFalconPunchJointIndex(fp);
	}
	if ((joint_idx < 0) || (joint_idx >= FTPARTS_JOINT_NUM_MAX) || (fp->joints[joint_idx] == NULL))
	{
		return;
                                                           ...
                                                           const SYNetRbSnapEffectBlob *blob)
{
	...
	if ((slot == NULL) || (syNetRbSnapEffectBlobIsUserdataJoint(blob) == FALSE))
	{
		return NULL;
	}
	for (pass = 0; pass < 2; pass++)
                                                       ...
                                                       const SYNetRbSnapEffectBlob *blob)
{
	...
	if (slot != NULL)
	{
		...
		{
			pi = syNetRbSnapPlayerForFighterGobjId(slot, blob->fighter_gobj_id);
			if (pi >= 0)
			{
...
/*
 * Slot-authoritative userdata-joint restore (Kirby CopyCaptain Falcon Punch flame, etc.): adopt or
 * respawn every captured joint-attached FX, rebind user_data.p, and stamp blob pose. Without a
 * respawn kind, particle reset + orphan prune leave is_effect_attach stuck and the vanilla one-shot
 * flag0 gate cannot re-fire the flame mid-punch.
 */
static void syNetRbSnapEnsureUserdataJointEffectsFromSlot(const SYNetRbSnapshotSlot *slot)
{
	s32 ei;
...
/*
 * Unstick Kirby CopyCaptain when the slot expects a joint-attached flame but live reconcile did not
 * restore one — otherwise is_effect_attach blocks ftKirbyCopyCaptainSpecialNUpdateEffect forever.
 */
static void syNetRbSnapSanitizeCopyCaptainEffectAttach(const SYNetRbSnapshotSlot *slot)
{
	GObj *fighter_gobj;
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		fp = ftGetStruct(fighter_gobj);
		if ((fp == NULL) || (fp->status_id != nFTKirbyStatusCopyCaptainSpecialN))
		{
			continue;
		}
		pi = fp->player;
		...
		if (syNetRbSnapLiveFighterHasUserdataJointAttachEffect(fighter_gobj) != FALSE)
		{
			continue;
		}
		if (syNetRbSnapSlotListsUserdataJointAttachForFighterPlayer(slot, pi) != FALSE)
		{
			fp->is_effect_attach = FALSE;
		}
		else if (fp->is_effect_attach != FALSE)
		{
			fp->is_effect_attach = FALSE;
		}
	}
...
/*
 * Slot lists at most one quake per blob, but verify-only repair passes (EnsureQuake + reconcile +
 * TryRepair twice) can stack live quakes that all match the same blob identity. Keep one canonical
 * GObj per slot quake blob (prefer ring gobj_id, else first live match) and eject the rest.
 */
static void syNetRbSnapPruneDuplicateQuakeEffects(const SYNetRbSnapshotSlot *slot)
{
	s32 ei;
	...
	for (ei = 0; ei < slot->effect_count; ei++)
	{
		...
		keep_gobj = gcFindGObjByID(blob->gobj_id);
		if ((keep_gobj == NULL) || (efGetStruct(keep_gobj) == NULL) ||
		    (syNetRbSnapLiveEffectMatchesBlob(slot, blob, keep_gobj, efGetStruct(keep_gobj)) == FALSE))
		{
			keep_gobj = syNetRbSnapFindLiveQuakeEffectForBlob(slot, blob);
		}
		for (pass = 0; pass < 2; pass++)
		{
			GObj *link_head;
...
/*
 * Global quake quota: slot lists N quake blobs but verify repair can stack >N live quakes that all
 * match blob identity (save 1 → verify 5+ identical folds). Keep the first N slot-matched quakes
 * in link order and eject the rest after per-blob dedupe.
 */
static void syNetRbSnapPruneExcessSlotMatchedQuakes(const SYNetRbSnapshotSlot *slot)
{
	s32 quake_quota;
	...
	for (pass = 0; pass < 2; pass++)
	{
		...
		for (gobj = link_head; gobj != NULL; gobj = next)
		{
			...
			if (syNetRbSnapLiveEffectMatchesAnyBlobInSlot(slot, gobj, ep) == FALSE)
			{
				continue;
			}
			if (kept < quake_quota)
                                                      ...
                                                      const u32 *reconciled_ids, s32 reconciled_count)
{
	...
	for (pass = 0; pass < 2; pass++)
	{
		...
		for (gobj = link_head; gobj != NULL; gobj = next)
		{
			...
			eject = FALSE;
			if ((syNetRbSnapLiveEffectIsQuake(gobj, ep) != FALSE) &&
			    ((DObjGetStruct(gobj) == NULL) || (gobj->obj == NULL)))
			{
				eject = TRUE;
			}
			else if ((ep != NULL) && (ep->fighter_gobj == NULL))
			{
				if (syNetRbSnapLiveEffectIsQuake(gobj, ep) != FALSE)
				{
					eject = TRUE;
...
u8 syNetRbSnapEffectRespawnKindFromLive(const GObj *gobj, const EFStruct *ep)
{
	...
	{
		return SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE;
	}
	...
	if (syNetRbSnapLiveEffectIsUserdataJointAttach(gobj, ep) != FALSE)
	{
		return SYNETRB_EFFECT_RESPAWN_USERDATA_JOINT;
	}
	return SYNETRB_EFFECT_RESPAWN_NONE;
}
...
static sb32 syNetRbSnapEffectIdIsLiveYoshiEggLayShell(u32 effect_gobj_id)
{
	...
	gobj = gcFindGObjByID(effect_gobj_id);
	if (gobj == NULL)
	{
		return FALSE;
	}
	...
	if ((ep->fighter_gobj == NULL) || ((fp = ftGetStruct(ep->fighter_gobj)) == NULL))
	{
		return FALSE;
	}
	return (fp->status_id == nFTCommonStatusYoshiEgg) ? TRUE : FALSE;
}
...
static sb32 syNetRbSnapLiveGuardClaimsEffectId(u32 effect_gobj_id)
{
	...
	if ((effect_gobj_id == 0U) || (syNetRbSnapEffectIdIsLiveYoshiEggLayShell(effect_gobj_id) != FALSE))
	{
		return FALSE;
	}
	for (fighter_gobj = gGCCommonLinks[nGCCommonLinkIDFighter]; fighter_gobj != NULL;
...
#if defined(SSB64_NETMENU)
		if ((s_syNetRbSnapRepairStageVerifyOnly != FALSE) &&
		    (syNetRbSnapBlobInYoshiShieldEscapeScope(blob) != FALSE))
		{
			continue;
		}
#endif
...
static u32 syNetRbSnapFoldGroundPayloadHashForRollback(const SYNetRbSnapGroundBlob *ground)
{
	...
	case nGRKindSector:
		...
		break;
	case nGRKindYoster:
		if (n >= (u32)sizeof(SYNetRbSnapGroundYoster))
		{
			SYNetRbSnapGroundYoster *yo = (SYNetRbSnapGroundYoster *)payload_scratch;
			s32 ci;

			/* map_head is a live pointer for restore only — exclude from cross-peer digest. */
			yo->map_head = 0;
			for (ci = 0; ci < 3; ci++)
			{
				SYNetRbSnapGroundYosterCloud *sc = &yo->clouds[ci];

				sc->altitude = syNetplayQuantizeF32ForRollbackHash(sc->altitude);
				sc->pressure = syNetplayQuantizeF32ForRollbackHash(sc->pressure);
				sc->translate.x = syNetplayQuantizeF32ForRollbackHash(sc->translate.x);
				sc->translate.y = syNetplayQuantizeF32ForRollbackHash(sc->translate.y);
				sc->translate.z = syNetplayQuantizeF32ForRollbackHash(sc->translate.z);
			}
		}
		else if (n >= (u32)SYNETRB_SNAP_GROUND_YOSTER_LEGACY_PAYLOAD_LEN)
		{
			SYNetRbSnapGroundYosterCloudLegacy *legacy = (SYNetRbSnapGroundYosterCloudLegacy *)payload_scratch;
			s32 ci;

			for (ci = 0; ci < 3; ci++)
			{
				legacy[ci].altitude = syNetplayQuantizeF32ForRollbackHash(legacy[ci].altitude);
				legacy[ci].pressure = syNetplayQuantizeF32ForRollbackHash(legacy[ci].pressure);
			}
		}
		break;
	default:
...
static void syNetRbSnapCaptureGround(SYNetRbSnapshotSlot *slot)
{
	...
	case nGRKindPupupu:
	{
		...
		dst->lr_players = src->lr_players;
		dst->flowers_back_status = src->flowers_back_status;
		dst->flowers_front_status = src->flowers_front_status;
		dst->flowers_back_wait = src->flowers_back_wait;
		dst->flowers_front_wait = src->flowers_front_wait;
		dst->whispy_mouth_texture = src->whispy_mouth_texture;
...
#ifdef PORT
static sb32 syNetRbSnapPupupuWhispyRepairDiagEnabled(void);
#endif

static void syNetRbSnapResyncPupupuWhispyFlowerPresentation(GRCommonGroundVarsPupupu *pu, sb32 has_extended_blob)
{
	sb32 flowers_in_wind;
	...
	flowers_in_wind =
	    (pu->flowers_front_status >= (u8)nGRPupupuFlowerStatusWindStart) &&
	    (pu->flowers_front_status <= (u8)nGRPupupuFlowerStatusWindLoopEnd);
	if ((has_extended_blob != FALSE) && (flowers_in_wind != FALSE))
	{
		return;
	}
	pu->flowers_back_status = (u8)nGRPupupuFlowerStatusWindStart;
	pu->flowers_front_status = (u8)nGRPupupuFlowerStatusWindStart;
	pu->flowers_back_wait = 15U;
	pu->flowers_front_wait = 22U;
	pu->whispy_mouth_texture = -1;
	pu->whispy_eyes_texture = -1;
#ifdef PORT
	if (syNetRbSnapPupupuWhispyRepairDiagEnabled() != FALSE)
	{
		port_log(
		    "SSB64 WhispyRepair: flower_reseed extended=%d flowers_f=%u flowers_b=%u lr=%d\n",
		    (int)has_extended_blob,
...
static void syNetRbSnapApplyGround(const SYNetRbSnapshotSlot *slot)
{
	...
	case nGRKindSector:
		...
		break;
	case nGRKindPupupu:
		if (ground->payload_len >= SYNETRB_SNAP_GROUND_PUPUPU_LEGACY_PAYLOAD_LEN)
		{
			...
			dst->whispy_eyes_status = src->whispy_eyes_status;
			dst->whispy_mouth_status = src->whispy_mouth_status;
			has_extended_blob = (ground->payload_len >= (u16)sizeof(SYNetRbSnapGroundPupupu)) ? TRUE : FALSE;
			if (has_extended_blob != FALSE)
			{
				dst->lr_players = src->lr_players;
				dst->flowers_back_status = src->flowers_back_status;
...
static void syNetRbSnapshotLogMapHashSaveCompactDiag(u32 tick, u32 hash_map, const SYNetRbSnapGroundBlob *ground)
{
	u32 kin;
	u32 ground_fold;
	...
	kin = syNetSyncHashMapCollisionKinematicsForRollback();
	ground_fold = ((ground != NULL) && (ground->payload_len > 0U))
	                  ? syNetRbSnapFoldGroundPayloadHashForRollback(ground)
	                  : 2166136261U;
	port_log(
...
static sb32 syNetRbSnapBlobInYoshiShieldEscapeScope(const SYNetRbSnapFighterBlob *blob)
{
	...
	return ((blob->status_id == nFTCommonStatusEscapeF) || (blob->status_id == nFTCommonStatusEscapeB)) ? TRUE
	                                                                                                   : FALSE;
}
...
static sb32 syNetRbSnapSlotHasYoshiEggEscapeBlobForPlayer(const SYNetRbSnapshotSlot *slot, s32 player)
{
	...
	for (ei = 0; ei < slot->effect_count; ei++)
	{
		...
		if (blob->respawn_kind != SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE)
		{
			continue;
		}
		if (syNetRbSnapYoshiEggEscapePlayerFromEffectBlob(blob) == player)
		{
			return TRUE;
		}
	}
...
/*
 * Snapshot apply: refuse to respawn/reconcile effect blobs that disagree with the loaded fighter slot
 * (guard @ T vs escape shell leaked from forward sim @ T+1 — synctest tick 1587 class).
 */
static sb32 syNetRbSnapEffectBlobAuthoritativeForSlot(const SYNetRbSnapshotSlot *slot,
                                                      const SYNetRbSnapEffectBlob *blob)
{
	...
	if (blob->respawn_kind == SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE)
	{
		player = syNetRbSnapYoshiEggEscapePlayerFromEffectBlob(blob);
		if (player < 0)
		{
			return FALSE;
		}
		fb = &slot->fighters[player];
		if ((fb->is_valid != FALSE) && (syNetRbSnapBlobInYoshiShieldEscapeScope(fb) != FALSE))
		{
			return TRUE;
		}
		return syNetRbSnapSlotHasYoshiEggEscapeBlobForPlayer(slot, player);
	}
	if (syNetRbSnapEffectBlobIsShieldKind(blob) != FALSE)
	{
		player = syNetRbSnapShieldPlayerFromEffectBlob(blob);
		if (player < 0)
		{
			return FALSE;
		}
		...
		if (syNetRbSnapBlobInYoshiShieldEscapeScope(fb) != FALSE)
		{
			return FALSE;
		}
		if (syNetRbSnapSlotHasYoshiEggEscapeBlobForPlayer(slot, player) != FALSE)
		{
			return FALSE;
		}
		return syNetRbSnapBlobInGuardScope(fb);
	}
	return TRUE;
}
...
static sb32 syNetRbSnapSlotAnyFighterInYoshiShieldEscapeScope(const SYNetRbSnapshotSlot *slot)
{
	...
	{
		if ((slot->fighters[pi].is_valid != FALSE) &&
		    (syNetRbSnapBlobInYoshiShieldEscapeScope(&slot->fighters[pi]) != FALSE))
		{
			return TRUE;
		}
	}
...
static sb32 syNetRbSnapSlotAnyFighterInGuardScope(const SYNetRbSnapshotSlot *slot)
{
	...
	{
		if ((slot->fighters[pi].is_valid != FALSE) &&
		    (syNetRbSnapBlobInGuardScope(&slot->fighters[pi]) != FALSE))
		{
			return TRUE;
		}
	}
...
/*
 * Synctest verify-only: escape-roll and sustained guard slots hash joint anim from the ring blob;
 * figatree / joint re-stamp after effect repair clobbers figh/anim (tick 1058).
 */
static sb32 syNetRbSnapSlotVerifyJointAnimFragile(const SYNetRbSnapshotSlot *slot)
{
	if (syNetRbSnapSlotAnyFighterInYoshiShieldEscapeScope(slot) != FALSE)
	{
		return TRUE;
	}
	...
	return syNetRbSnapSlotAnyFighterInGuardScope(slot);
}

/* Escape-roll only: skip finalize/patch that mints or couples shield presentation (tick 2318). */
static sb32 syNetRbSnapSlotVerifyEffectCouplingFragile(const SYNetRbSnapshotSlot *slot)
{
	return syNetRbSnapSlotAnyFighterInYoshiShieldEscapeScope(slot);
}
...
static sb32 syNetRbSnapEffectRespawnKindGobjCollisionPriority(u8 respawn_kind)
{
	switch (respawn_kind)
	{
	case SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE:
		return 100;
...
/*
 * Rebirth halo and camera quake are independent effects that can share a stale pool gobj_id during
...
static sb32 syNetRbSnapEffectGobjIdCollisionAllowsCoexist(u8 kind_a, u8 kind_b)
{
	...
	if (((kind_a == SYNETRB_EFFECT_RESPAWN_QUAKE) &&
	     ((kind_b == SYNETRB_EFFECT_RESPAWN_SHIELD) || (kind_b == SYNETRB_EFFECT_RESPAWN_YOSHI_SHIELD))) ||
	    ((kind_b == SYNETRB_EFFECT_RESPAWN_QUAKE) &&
	     ((kind_a == SYNETRB_EFFECT_RESPAWN_SHIELD) || (kind_a == SYNETRB_EFFECT_RESPAWN_YOSHI_SHIELD))))
	{
		return TRUE;
	}
	return FALSE;
...
static sb32 syNetRbSnapshotSlotAnyFighterRebirthScope(const SYNetRbSnapshotSlot *slot)
{
	...
	for (pidx = 0; pidx < GMCOMMON_PLAYERS_MAX; pidx++)
	{
		...
		if ((fb->is_valid != FALSE) && (fb->status_id >= nFTCommonStatusRebirthDown) &&
		    (fb->status_id <= nFTCommonStatusRebirthWait))
		{
			return TRUE;
		}
	}
...
static sb32 syNetRbSnapSlotHasQuakeEffectBlob(const SYNetRbSnapshotSlot *slot)
{
	...
	for (ei = 0; ei < slot->effect_count; ei++)
	{
		if ((slot->effects[ei].is_valid != FALSE) &&
		    (slot->effects[ei].respawn_kind == SYNETRB_EFFECT_RESPAWN_QUAKE))
		{
			return TRUE;
		}
	}
                                                         ...
                                                         const SYNetRbSnapEffectBlob *blob)
{
	...
	{
		return syNetRbSnapFindLiveQuakeEffectForBlob(slot, blob);
	}
	gobj = gcFindGObjByID(blob->gobj_id);
	if ((gobj != NULL) &&
	    (syNetRbSnapLiveEffectMatchesBlob(slot, blob, gobj, efGetStruct(gobj)) != FALSE))
	{
		return gobj;
	}
	for (pass = 0; pass < 2; pass++)
...
static void syNetRbSnapCompactSlotEffects(SYNetRbSnapshotSlot *slot)
{
	...
	write = 0;
	for (read = 0; read < slot->effect_count; read++)
	{
		if (slot->effects[read].is_valid == FALSE)
		{
			continue;
		}
		if (write != read)
		{
			slot->effects[write] = slot->effects[read];
		}
		write++;
	}
	slot->effect_count = write;
}

/*
 * Ring save can stack escape/quake/shield blobs on one stale gobj_id (tick 1571 SIGSEGV class).
 * Keep the highest-priority respawn kind per id.
 */
static void syNetRbSnapSanitizeSlotEffectGobjIdCollisions(SYNetRbSnapshotSlot *slot)
{
	s32 i;
	...
	for (i = 0; i < slot->effect_count; i++)
	{
		SYNetRbSnapEffectBlob *blob_i;

		blob_i = &slot->effects[i];
		if ((blob_i->is_valid == FALSE) || (blob_i->gobj_id == 0U))
		{
			continue;
		}
		for (j = i + 1; j < slot->effect_count; j++)
		{
			SYNetRbSnapEffectBlob *blob_j;

			blob_j = &slot->effects[j];
			if ((blob_j->is_valid == FALSE) || (blob_j->gobj_id != blob_i->gobj_id))
			{
				continue;
			}
			if (syNetRbSnapEffectGobjIdCollisionAllowsCoexist(blob_i->respawn_kind, blob_j->respawn_kind) !=
			    FALSE)
			{
				continue;
			}
			if (syNetRbSnapEffectRespawnKindGobjCollisionPriority(blob_i->respawn_kind) >=
			    syNetRbSnapEffectRespawnKindGobjCollisionPriority(blob_j->respawn_kind))
			{
				blob_j->is_valid = FALSE;
			}
			else
			{
				blob_i->is_valid = FALSE;
				break;
			}
		}
	}
	syNetRbSnapCompactSlotEffects(slot);
}

/*
 * Ring eff hash must fold only slot-authoritative effect blobs (post-sanitize), not every live GObj when
 * gobj_id collisions were deduped at capture (tick 633 eff_fold_diag count=2 vs effect_count=1).
 */
static u32 syNetRbSnapHashCanonicalSlotEffects(const SYNetRbSnapshotSlot *slot)
{
	...
	for (i = 0; i < slot->effect_count; i++)
	{
		...
		gobj = syNetRbSnapResolveLiveEffectGobjForBlobHash(slot, blob);
		if (gobj == NULL)
		{
			continue;
		}
		if (syNetRbSnapEffectGobjListed((const GObj * const *)sorted, count, gobj) != FALSE)
		{
			continue;
		}
	...
	for (i = 1; i < count; i++)
	{
		...
		key_id = key_gobj->id;
		j = i;
		while ((j > 0) && (sorted[j - 1]->id > key_id))
		{
			sorted[j] = sorted[j - 1];
			j--;
		}
	...
	for (i = 0; i < count; i++)
	{
		...
		{
			hash ^= fold;
			hash = syNetRbSnapFnvAccumulateU32(hash, 0xA5A5A5A5U);
		}
	}
	return hash;
}
                                                                ...
                                                                const SYNetRbSnapEffectBlob *shield_blob)
{
	...
	if ((slot == NULL) || (shield_blob == NULL) || (shield_blob->gobj_id == 0U))
	{
		return FALSE;
	}
	for (ei = 0; ei < slot->effect_count; ei++)
	{
		...
		eb = &slot->effects[ei];
		if ((eb->is_valid == FALSE) || (eb->gobj_id != shield_blob->gobj_id))
		{
			continue;
		}
		if (eb->respawn_kind == SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE)
		{
			return TRUE;
		}
	}
...
static sb32 syNetRbSnapBlobInCliffMotionSynctestFragileScope(const SYNetRbSnapFighterBlob *blob)
{
	...
	if ((blob->status_id >= nFTCommonStatusCliffCatch) &&
	    (blob->status_id <= nFTCommonStatusCliffEscapeSlow2))
	{
		return TRUE;
	}
	return FALSE;
...
static sb32 syNetRbSnapBlobInYoshiAerialLandingFragileScope(const SYNetRbSnapFighterBlob *blob)
{
	...
	if ((blob == NULL) || (blob->is_valid == FALSE) || (blob->fkind != nFTKindYoshi))
	{
		return FALSE;
	}
	status = blob->status_id;
...
static sb32 syNetRbSnapshotSlotYoshiAerialLandingFragile(const SYNetRbSnapshotSlot *slot)
{
	...
	for (pidx = 0; pidx < GMCOMMON_PLAYERS_MAX; pidx++)
	{
		if (syNetRbSnapBlobInYoshiAerialLandingFragileScope(&slot->fighters[pidx]) != FALSE)
		{
			return TRUE;
		}
	}
...
static sb32 syNetRbSnapSlotIsEmergencySentinel(const SYNetRbSnapshotSlot *slot)
{
	return ((slot != NULL) && (slot->tick == 0xFFFFFFFFU)) ? TRUE : FALSE;
}
...
static sb32 syNetRbSnapFighterGuardEffectUnionOwned(const FTStruct *fp)
{
	...
	/*
	 * guard.effect_gobj aliases twister.tornado_gobj, tarucann.tarucann_gobj, and captureyoshi.effect_gobj
	 * at union offset 0x08.
	 */
...
/*
 * Vanilla may allocate a shield GObj before proc wiring completes; syNetRbSnapLiveEffectIsShield
 * rejects those until EffectGObjHasUpdateProc matches, which made ensure mint a duplicate.
 */
static sb32 syNetRbSnapLooseEffectIsShieldCandidate(const GObj *gobj, const EFStruct *ep)
{
...
static GObj *syNetRbSnapFindLooseShieldEffectForFighter(GObj *fighter_gobj, FTStruct *fp)
{
	...
	for (pass = 0; pass < 2; pass++)
	{
		     ...
		     gobj != NULL; gobj = gobj->link_next)
		{
			...
			if (syNetRbSnapLooseEffectIsShieldCandidate(gobj, ep) == FALSE)
			{
				continue;
			}
...
static GObj *syNetRbSnapResolveFighterShieldBubbleGobj(GObj *fighter_gobj, FTStruct *fp)
{
	...
	shield_gobj = syNetRbSnapFindLiveShieldEffectByPlayerSlot(fp->player);
	if (shield_gobj != NULL)
	{
		return shield_gobj;
	}
	if (syNetRbSnapFighterInYoshiShieldEscapePresentationScope(fp) != FALSE)
	{
		return NULL;
	}
	if (syNetRbSnapFighterGuardEffectUnionOwned(fp) != FALSE)
	{
		...
		{
			guard_ep = efGetStruct(guard_coupled);
			if ((guard_ep != NULL) &&
			    (syNetRbSnapLooseEffectIsShieldCandidate(guard_coupled, guard_ep) != FALSE) &&
	...
	return syNetRbSnapFindLooseShieldEffectForFighter(fighter_gobj, fp);
}

static sb32 syNetRbSnapFighterShieldBubbleVisible(GObj *fighter_gobj, FTStruct *fp)
{
	return (syNetRbSnapResolveFighterShieldBubbleGobj(fighter_gobj, fp) != NULL) ? TRUE : FALSE;
}
...
static sb32 syNetRbSnapPeerYoshiEggHatchStressActive(s32 self_player)
{
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		if ((fp != NULL) && (fp->player != self_player) && (fp->status_id == nFTCommonStatusYoshiEgg))
		{
			return TRUE;
		}
	}
#if defined(SSB64_NETMENU)
	if (syNetRbSnapAnyYoshiEggLayHatchReplayPending() != FALSE)
	{
		return TRUE;
	}
#endif
...
/*
 * Block minting a fresh shield bubble only during hatch-replay deferral, or when this slot already
 * owns a live bubble while a peer is rolling in YoshiEgg (duplicate mint / id-1011 churn). First
 * bubble while Z is held near a peer egg must still spawn same-frame — blocking that caused the
 * one-tick Yoshi shield gap at egg overlap (bubble=0 with is_shield=1).
 */
static sb32 syNetRbSnapShieldMintBlockedByEggHatchStress(FTStruct *fp, const char **reason_out)
{
	GObj *fighter_gobj;
	...
	if (syNetRbSnapAnyYoshiEggLayHatchReplayPending() != FALSE)
	{
		if (reason_out != NULL)
		{
			*reason_out = "hatch_replay_pending";
		}
		return TRUE;
	}
#endif
	...
	for (fighter_gobj = gGCCommonLinks[nGCCommonLinkIDFighter]; fighter_gobj != NULL;
	     fighter_gobj = fighter_gobj->link_next)
	{
		FTStruct *peer_fp = ftGetStruct(fighter_gobj);

		if ((peer_fp != NULL) && (peer_fp->player != fp->player) &&
		    (peer_fp->status_id == nFTCommonStatusYoshiEgg))
		{
			if (reason_out != NULL)
			{
				*reason_out = "peer_egg_existing_bubble";
			}
			return TRUE;
		}
	}
...
static GObj *syNetRbSnapTryAdoptLiveShieldEffectForFighter(GObj *fighter_gobj, FTStruct *fp)
{
	...
	shield_gobj = syNetRbSnapResolveFighterShieldBubbleGobj(fighter_gobj, fp);
	if (shield_gobj == NULL)
	{
		return NULL;
	}
	shield_ep = efGetStruct(shield_gobj);
	if ((shield_ep != NULL) && (shield_ep->fighter_gobj != fighter_gobj))
	{
		shield_ep->fighter_gobj = fighter_gobj;
		shield_ep->effect_vars.shield.player = fp->player;
	}
	if (fp->fkind == nFTKindYoshi)
	{
		syNetRbSnapCoupleYoshiShieldEffect(fighter_gobj, fp, shield_gobj);
	}
	else
	{
		syNetRbSnapAuditLiveShieldEffectOwner(shield_gobj, fp);
...
static sb32 syNetRbSnapFighterInYoshiShieldEscapePresentationScope(const FTStruct *fp)
{
	if ((fp == NULL) || (fp->fkind != nFTKindYoshi) || (fp->shield_health == 0))
	{
		return FALSE;
	}
	if ((fp->status_id != nFTCommonStatusEscapeF) && (fp->status_id != nFTCommonStatusEscapeB))
	{
		return FALSE;
	}
	/*
	 * Shield-origin roll (EscapeF/B from Guard): vanilla clears is_shield but the egg shell stays visible.
	 * Dash escape uses itemthrow_buffer_tics=0 and does not hold Z — do not show the Z egg shell there.
	 * Tail of shield roll: itemthrow_buffer_tics drains to 0 while still in EscapeF/B — keep scope while the
	 * cosmetic shell remains coupled (is_effect_attach).
	 */
	...
	return (syNetRbSnapFighterShieldInputHeld(fp) != FALSE) ? TRUE : FALSE;
}
...
static void syNetRbSnapApplyYoshiShieldPresentation(GObj *fighter_gobj, FTStruct *fp)
{
	...
	else if (syNetRbSnapFighterInYoshiShieldEscapePresentationScope(fp) != FALSE)
	{
		ftParamHideModelPartAll(fighter_gobj);
	}
}
...
/*
 * Escape (EscapeF/B) aliases the guard/escape status_vars union. The egg shell is a pure cosmetic during
 * the roll, so ownership is tracked through the effect list (shield.player / ep->fighter_gobj) only —
 * never ftStatusVarsGuard() and never fp->is_shield. Writing the guard overlay or is_shield here would
 * stomp live escape state (itemthrow_buffer_tics, etc.) and re-assert shield sim during the vulnerable
 * roll window. is_effect_attach stays set so the shell tracks Yoshi's joint and tears down on the next
 * non-PRESERVE_EFFECT status change (escape -> Wait), matching vanilla cleanup.
 */
static void syNetRbSnapCoupleYoshiShieldEffectEscapeCosmetic(GObj *fighter_gobj, FTStruct *fp, GObj *effect_gobj)
{
	EFStruct *ep;

	if ((fighter_gobj == NULL) || (fp == NULL) || (effect_gobj == NULL) || (fp->fkind != nFTKindYoshi))
	{
		return;
	}
	ep = efGetStruct(effect_gobj);
	...
	{
		ep->fighter_gobj = fighter_gobj;
		if (syNetRbSnapLiveEffectIsShield(effect_gobj, ep) != FALSE)
		{
			syNetRbSnapAuditLiveShieldEffectOwner(effect_gobj, fp);
		}
		else if (syNetRbSnapLiveEffectIsYoshiEggEscape(effect_gobj, ep) != FALSE)
		{
			DObj *dobj;
...
static void syNetRbSnapReconcileYoshiShieldEscapePresentation(const SYNetRbSnapshotSlot *slot)
{
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		FTStruct *fp;
		GObj *effect_gobj;
		const char *hatch_stress_reason;
		...
		{
			s32 pi_esc;
			const SYNetRbSnapFighterBlob *fb_esc;

			pi_esc = fp->player;
			if ((pi_esc >= 0) && (pi_esc < GMCOMMON_PLAYERS_MAX))
			{
				fb_esc = &slot->fighters[pi_esc];
				if ((fb_esc->is_valid != FALSE) &&
				    (syNetRbSnapBlobInYoshiShieldEscapeScope(fb_esc) == FALSE) &&
				    (syNetRbSnapSlotHasYoshiEggEscapeBlobForPlayer(slot, pi_esc) == FALSE))
				{
					continue;
				}
			}
		}
		syNetRbSnapEjectStaleYoshiEggLayShellsForFighter(fighter_gobj, NULL);
		/*
		 * Effect-list resolve only — syNetRbSnapResolveFighterShieldBubbleGobj falls through to a
		 * ftStatusVarsGuard(fp)->effect_gobj read, which is a union stomp while status is EscapeF/B.
		 * Vanilla shield escape spawns nEFKindYoshiEggEscape (not the guard bubble); prefer that shell.
		 */
		effect_gobj = syNetRbSnapFindLiveYoshiEggEscapeEffectForFighter(fighter_gobj);
		if (effect_gobj == NULL)
		{
			effect_gobj = syNetRbSnapFindLiveYoshiEggEscapeEffectRelaxedForFighter(fighter_gobj);
		}
		if (effect_gobj == NULL)
		{
			if (syNetRbSnapShieldMintBlockedByEggHatchStress(fp, &hatch_stress_reason) != FALSE)
			{
				continue;
			}
			if (s_syNetRbSnapRepairStageVerifyOnly != FALSE)
			...
			effect_gobj = efManagerYoshiEggEscapeMakeEffect(fighter_gobj);
		}
		if (effect_gobj != NULL)
		{
			if (s_syNetRbSnapRepairStageVerifyOnly == FALSE)
			{
				syNetRbSnapCoupleYoshiShieldEffectEscapeCosmetic(fighter_gobj, fp, effect_gobj);
			}
			syNetRbSnapEjectStaleYoshiEggLayShellsForFighter(fighter_gobj, effect_gobj);
		}
	}
	(void)slot;
}
...
static GObj *syNetRbSnapTryEnsureLiveShieldEffectForFighter(GObj *fighter_gobj, const SYNetRbSnapshotSlot *slot)
{
	...
	}
	{
		...
		resolved = syNetRbSnapResolveFighterShieldBubbleGobj(fighter_gobj, fp);
		if (resolved != NULL)
		{
			adopted = syNetRbSnapTryAdoptLiveShieldEffectForFighter(fighter_gobj, fp);
			return (adopted != NULL) ? adopted : resolved;
		}
	}
	{
		GObj *guard_coupled;
		...
		{
			guard_ep = efGetStruct(guard_coupled);
			if ((guard_ep == NULL) || (syNetRbSnapLooseEffectIsShieldCandidate(guard_coupled, guard_ep) == FALSE))
			{
				/*
				 * Stale guard->effect_gobj can alias a recycled egg-lay shell (shared pool id 1011)
				 * or another non-shield effect after victim hatch / reconcile churn.
				 */
				ftStatusVarsGuard(fp)->effect_gobj = NULL;
				if (fp->is_effect_attach != FALSE)
				{
			...
			else if (syNetRbSnapShieldPlayerFromEffectVars(guard_ep) != fp->player)
			{
				ftStatusVarsGuard(fp)->effect_gobj = NULL;
			}
		}
	...
	/*
	 * Adopt-only when any shield candidate exists for this slot (including proc-not-yet-wired vanilla
	 * spawns). Never stack a rollback mint on top of an existing bubble.
	 */
	if (syNetRbSnapCountLooseShieldEffectsForPlayer(fp->player) > 0)
	{
		GObj *slot_shield;

		slot_shield = syNetRbSnapFindLooseShieldEffectForFighter(fighter_gobj, fp);
		if (slot_shield == NULL)
		{
			slot_shield = syNetRbSnapFindLiveShieldEffectByPlayerSlot(fp->player);
		}
		if (slot_shield != NULL)
		{
			if (fp->fkind == nFTKindYoshi)
			{
				syNetRbSnapCoupleYoshiShieldEffect(fighter_gobj, fp, slot_shield);
			}
			else
			{
				syNetRbSnapAuditLiveShieldEffectOwner(slot_shield, fp);
				ftStatusVarsGuard(fp)->effect_gobj = slot_shield;
				fp->is_effect_attach = TRUE;
				fp->is_shield = TRUE;
			}
			return slot_shield;
		...
		if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
		{
			port_log(
			    ...
			    (int)syNetRbSnapFighterShieldInputHeldAuthoritative(fp),
			    (int)ftStatusVarsGuard(fp)->is_release, (int)ftStatusVarsGuard(fp)->release_lag,
			    (int)syNetRbSnapCountLooseShieldEffectsForPlayer(fp->player));
		}
		return NULL;
	}
	{
		...
		if (syNetRbSnapShieldMintBlockedByEggHatchStress(fp, &hatch_stress_reason) != FALSE)
		{
			if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
			{
				port_log(
				    ...
				    "reason=%s shield_health=%d z_auth=%d is_release=%d release_lag=%d\n",
				    (unsigned int)syNetRbSnapGuardShieldDiagTick(slot), (int)fp->player, (int)fp->status_id,
				    (hatch_stress_reason != NULL) ? hatch_stress_reason : "egg_hatch_stress",
				    (int)fp->shield_health, (int)syNetRbSnapFighterShieldInputHeldAuthoritative(fp),
				    (int)ftStatusVarsGuard(fp)->is_release, (int)ftStatusVarsGuard(fp)->release_lag);
			}
			return NULL;
		}
...
static u32 syNetRbSnapGuardEffectIdFromBlob(const SYNetRbSnapFighterBlob *blob)
{
	if ((syNetRbSnapBlobInGuardScope(blob) == FALSE) || (syNetRbSnapBlobInYoshiEggLayScope(blob) != FALSE))
	{
		return 0U;
	}
	/*
...
static s32 syNetRbSnapYoshiEggEscapePlayerFromEffectBlob(const SYNetRbSnapEffectBlob *blob)
{
	if ((blob == NULL) || (blob->respawn_kind != SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE))
	{
		return -1;
	}
	if (blob->quake_magnitude < GMCOMMON_PLAYERS_MAX)
	{
		return (s32)blob->quake_magnitude;
	}
	return -1;
}
                                                        ...
                                                        const SYNetRbSnapEffectBlob *blob)
{
	...
	/*
	 * Never resolve via syNetRbSnapPlayerForFighterGobjId(blob->fighter_gobj_id): all fighters share
	 * kind id 1000 and the first slot match is often Kirby. Authoritative identity is the captured
	 * player index (quake_magnitude), slot status, or live escape presentation scope.
	 */
	escape_player = syNetRbSnapYoshiEggEscapePlayerFromEffectBlob(blob);
	if (escape_player >= 0)
	{
		fighter_gobj = syNetRbSnapResolveFighterGobjByPlayer((s8)escape_player);
		fp = (fighter_gobj != NULL) ? ftGetStruct(fighter_gobj) : NULL;
		if ((fp != NULL) && (fp->fkind == nFTKindYoshi))
		{
			return fighter_gobj;
	...
	if (slot != NULL)
	{
		...
		for (pi = 0; pi < GMCOMMON_PLAYERS_MAX; pi++)
		{
			...
			fb = &slot->fighters[pi];
			if ((fb->is_valid == FALSE) || (fb->fkind != nFTKindYoshi))
			{
				continue;
			}
			if ((fb->status_id != nFTCommonStatusEscapeF) && (fb->status_id != nFTCommonStatusEscapeB))
			{
				continue;
			}
			fighter_gobj = syNetRbSnapResolveFighterGobjByPlayer((s8)pi);
		...
		for (pi = 0; pi < GMCOMMON_PLAYERS_MAX; pi++)
		{
			...
			fp = ftGetStruct(fighter_gobj);
			if ((fp != NULL) && (fp->fkind == nFTKindYoshi) &&
			    (syNetRbSnapFighterInYoshiShieldEscapePresentationScope(fp) != FALSE))
			{
				return fighter_gobj;
			}
		}
	}
	if ((blob->fighter_gobj_id != 0U) && (blob->fighter_gobj_id != (u32)nGCCommonKindFighter))
	{
		fighter_gobj = gcFindGObjByID(blob->fighter_gobj_id);
		fp = (fighter_gobj != NULL) ? ftGetStruct(fighter_gobj) : NULL;
		if ((fp != NULL) && (fp->fkind == nFTKindYoshi))
...
static FTStruct *syNetRbSnapResolveShieldOwnerFpFromEffect(const EFStruct *ep)
{
	s32 shield_player;
	GObj *owner_gobj;
	...
	if (shield_player >= 0)
	{
		...
		{
			return ftGetStruct(owner_gobj);
		}
	}
	if (ep->fighter_gobj != NULL)
	{
		return ftGetStruct(ep->fighter_gobj);
	}
	return NULL;
}

static sb32 syNetRbSnapLiveShieldExistsForBlob(const SYNetRbSnapEffectBlob *blob)
{
	s32 shield_player;
	...
	if (syNetRbSnapFindLiveShieldEffectForPlayer(shield_player) != NULL)
	{
		return TRUE;
	}
	return (syNetRbSnapCountLiveShieldEffectsForPlayer(shield_player) > 0) ? TRUE : FALSE;
}
...
static GObj *syNetRbSnapTryRespawnEffectFromBlob(const SYNetRbSnapshotSlot *slot, const SYNetRbSnapEffectBlob *blob)
{
	...
	if ((slot != NULL) && (syNetRbSnapEffectBlobAuthoritativeForSlot(slot, blob) == FALSE))
	{
		return NULL;
	}
	{
	...
	}
	{
		...
		/*
		 * One live bubble per player slot — never stack a second mint when reconcile/enforce/synctest
		 * restore revisit the same shield blob (Yoshi escape roll synctest triple-respawn class).
		 */
		if (syNetRbSnapLiveShieldExistsForBlob(blob) != FALSE)
		{
			return NULL;
		}
	}
	...
	else if (blob->respawn_kind == SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE)
	{
		fighter_gobj = syNetRbSnapResolveYoshiEggEscapeParentGobj(slot, blob);
	}
	else if (blob->respawn_kind == SYNETRB_EFFECT_RESPAWN_USERDATA_JOINT)
	{
		fighter_gobj = syNetRbSnapResolveUserdataJointParentGobj(slot, blob);
	}
	else if ((blob->fighter_gobj_id != 0U) && (blob->fighter_gobj_id != (u32)nGCCommonKindFighter))
	{
		fighter_gobj = gcFindGObjByID(blob->fighter_gobj_id);
		if ((fighter_gobj != NULL) && (ftGetStruct(fighter_gobj) == NULL))
		{
	...
	case SYNETRB_EFFECT_RESPAWN_SHIELD:
		if (fighter_gobj != NULL)
		{
			...
			fp_shield = ftGetStruct(fighter_gobj);
			if ((fp_shield != NULL) &&
			    (syNetRbSnapFighterInYoshiShieldEscapePresentationScope(fp_shield) != FALSE))
			{
				return syNetRbSnapFindLiveYoshiEggEscapeEffectForFighter(fighter_gobj);
			}
			...
			if ((slot != NULL) && (syNetRbSnapSlotEffectBlobConflictsWithShieldRespawn(slot, blob) != FALSE))
			{
				return NULL;
			}
			if ((fp_shield != NULL) && (fp_shield->fkind == nFTKindYoshi))
			{
				if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
				{
					port_log(
				...
				if (effect_gobj != NULL)
				{
					if (syNetRbSnapFighterInYoshiShieldEscapePresentationScope(fp_shield) != FALSE)
					{
						syNetRbSnapCoupleYoshiShieldEffectEscapeCosmetic(fighter_gobj, fp_shield, effect_gobj);
					}
					else
					{
						syNetRbSnapCoupleYoshiShieldEffect(fighter_gobj, fp_shield, effect_gobj);
					}
				}
				if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
				{
	...
	case SYNETRB_EFFECT_RESPAWN_YOSHI_SHIELD:
		if (fighter_gobj != NULL)
		{
			FTStruct *fp_yoshi_shield;
			GObj *existing_shield;
			s32 shield_player;

			fp_yoshi_shield = ftGetStruct(fighter_gobj);
			if ((fp_yoshi_shield != NULL) &&
			    (syNetRbSnapFighterInYoshiShieldEscapePresentationScope(fp_yoshi_shield) != FALSE))
			{
				return syNetRbSnapFindLiveYoshiEggEscapeEffectForFighter(fighter_gobj);
			}
			existing_shield = syNetRbSnapFindLiveShieldEffectForFighter(fighter_gobj);
			...
			/*
			 * Verify-only hash closure: adopt/patch live bubble via ApplyEffectBlobToGObj — never mint or
			 * CoupleYoshiShieldEffect during synctest probe (guard@T figh stomp @1299 class).
			 */
			if (s_syNetRbSnapRepairStageVerifyOnly != FALSE)
			{
				return NULL;
			...
			if (effect_gobj != NULL)
			{
				...
				{
					/*
					 * Owner mid-roll: cosmetic couple only (no guard overlay / is_shield) so the egg
					 * shell stays visible without stomping escape status_vars.
					 */
					if (syNetRbSnapFighterInYoshiShieldEscapePresentationScope(fp_yoshi) != FALSE)
					{
						syNetRbSnapCoupleYoshiShieldEffectEscapeCosmetic(fighter_gobj, fp_yoshi, effect_gobj);
					}
					else
					{
	...
	case SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE:
		...
		{
			FTStruct *fp_escape;
			GObj *existing_escape;

			fp_escape = ftGetStruct(fighter_gobj);
			if ((fp_escape == NULL) || (fp_escape->fkind != nFTKindYoshi))
			{
				break;
			}
			existing_escape = syNetRbSnapFindLiveYoshiEggEscapeEffectForFighter(fighter_gobj);
			if (existing_escape == NULL)
			{
				existing_escape = syNetRbSnapFindLiveYoshiEggEscapeEffectRelaxedForFighter(fighter_gobj);
			}
			if (existing_escape != NULL)
			{
				return existing_escape;
			}
			/*
			 * Verify-only hash closure (Phase 9): mint the egg shell when adopt paths miss so eff fold
			 * matches the slot blob. ApplyEffectBlobToGObj skips is_effect_attach / hide-model during
			 * verify — presentation stomp that caused tick 2318 is not replayed here.
			 */
			if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
			{
				port_log(
				    "SSB64 NetRbSnapshot: effect_respawn kind=YOSHI_EGG_ESCAPE blob_gobj_id=%u fighter_gobj_id=%u\n",
				    blob->gobj_id, blob->fighter_gobj_id);
			}
			effect_gobj = efManagerYoshiEggEscapeMakeEffect(fighter_gobj);
			if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
			{
				port_log("SSB64 NetRbSnapshot: effect_respawn kind=YOSHI_EGG_ESCAPE result=%s new_gobj_id=%u\n",
				         (effect_gobj != NULL) ? "ok" : "fail",
				         (effect_gobj != NULL) ? syNetRbSnapGobjId(effect_gobj) : 0U);
			}
			return effect_gobj;
		...
		break;
	case SYNETRB_EFFECT_RESPAWN_USERDATA_JOINT:
		if (fighter_gobj != NULL)
		{
			if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
			{
				port_log(
				    "SSB64 NetRbSnapshot: effect_respawn kind=USERDATA_JOINT blob_gobj_id=%u fighter_gobj_id=%u "
				    "resolved_parent=%u joint_idx=%u\n",
				    blob->gobj_id, blob->fighter_gobj_id, (unsigned int)fighter_gobj->id,
                                              ...
                                              const SYNetRbSnapEffectBlob *blob)
{
	...
	if ((gobj == NULL) && (blob->respawn_kind == SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE) && (slot != NULL))
	{
		GObj *escape_parent;

		escape_parent = syNetRbSnapResolveYoshiEggEscapeParentGobj(slot, blob);
		if (escape_parent != NULL)
		{
			gobj = syNetRbSnapFindLiveYoshiEggEscapeEffectRelaxedForFighter(escape_parent);
			if (gobj == NULL)
			{
				gobj = syNetRbSnapFindLiveYoshiEggEscapeEffectForFighter(escape_parent);
			}
		}
	}
	if (gobj == NULL)
	...
	else if ((syNetRbSnapEffectBlobIsShieldKind(blob) != FALSE) && (fighter_gobj == NULL))
	{
		s32 shield_player;
	...
	if ((fighter_gobj == NULL) && (blob->respawn_kind == SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE) &&
	    (ep->fighter_gobj != NULL) && (ftGetStruct(ep->fighter_gobj) != NULL))
	{
		fighter_gobj = ep->fighter_gobj;
	}
	ep->fighter_gobj = fighter_gobj;
	...
	if ((syNetRbSnapLiveEffectIsShield(gobj, ep) != FALSE) && (fighter_gobj != NULL))
	{
		...
		if ((fp_sh != NULL) && (fp_sh->joints[nFTPartsJointYRotN] != NULL))
		{
			...
			syNetRbSnapAuditLiveShieldEffectOwner(gobj, fp_sh);
			if ((s_syNetRbSnapRepairStageVerifyOnly == FALSE) &&
			    (syNetRbSnapFighterInYoshiShieldEscapePresentationScope(fp_sh) != FALSE))
			{
				fp_sh->is_effect_attach = TRUE;
				ftParamHideModelPartAll(fighter_gobj);
			}
		}
	}
	if ((syNetRbSnapLiveEffectIsYoshiEggEscape(gobj, ep) != FALSE) && (fighter_gobj != NULL))
	{
		FTStruct *fp_esc;
		DObj *dobj_esc;

		fp_esc = ftGetStruct(fighter_gobj);
		dobj_esc = DObjGetStruct(gobj);
		if ((fp_esc != NULL) && (dobj_esc != NULL) && (fp_esc->joints[5] != NULL))
		{
			dobj_esc->user_data.p = fp_esc->joints[5];
			if ((s_syNetRbSnapRepairStageVerifyOnly == FALSE) &&
			    (syNetRbSnapFighterInYoshiShieldEscapePresentationScope(fp_esc) != FALSE))
			{
				fp_esc->is_effect_attach = TRUE;
				ftParamHideModelPartAll(fighter_gobj);
			}
		}
	}
...
static GObj *syNetRbSnapFindLiveShieldEffectForPlayer(s32 player)
{
	...
	return syNetRbSnapFindLiveShieldEffectByPlayerSlot(player);
}
...
{
	GObj *seen[SYNETRB_SNAPSHOT_MAX_EFFECTS];
	s32 pass;
	s32 count;
	...
	for (pass = 0; pass < 2; pass++)
	{
		     ...
		     gobj != NULL; gobj = gobj->link_next)
		{
			...
			{
				if (syNetRbSnapEffectGobjListed((const GObj * const *)seen, count, gobj) != FALSE)
				{
					continue;
				}
				seen[count++] = gobj;
...
static GObj *syNetRbSnapPickCanonicalYoshiEggLayForFighter(GObj *fighter_gobj, FTStruct *fp)
{
	...
	coupled = syNetRbSnapResolveCoupledGobj(ftStatusVarsCaptureYoshi(fp)->effect_gobj);
	...
	for (pass = 0; pass < 2; pass++)
	{
		     ...
		     gobj != NULL; gobj = gobj->link_next)
		{
			...
			if ((best == NULL) || (gobj->anim_frame >= best_anim))
			{
				best = gobj;
				best_anim = gobj->anim_frame;
			}
		}
	}
	return best;
...
static void syNetRbSnapPruneDuplicateLiveYoshiEggLayEffects(const SYNetRbSnapshotSlot *slot)
{
	...
	for (pass = 0; pass < 2; pass++)
	{
		...
		for (gobj = link_head; gobj != NULL; gobj = next)
		{
			...
			if ((syNetRbSnapLiveEffectIsYoshiEggLay(gobj, ep) == FALSE) || (ep->fighter_gobj == NULL))
			{
				continue;
			}
			fp = ftGetStruct(ep->fighter_gobj);
			fighter_gobj = ep->fighter_gobj;
			if ((fp == NULL) || (fp->status_id != nFTCommonStatusYoshiEgg) ||
			    (syNetRbSnapCountLiveYoshiEggLayEffectsForFighter(fighter_gobj) <= 1))
			{
				continue;
			}
			keep_gobj = syNetRbSnapPickCanonicalYoshiEggLayForFighter(fighter_gobj, fp);
			if ((keep_gobj == NULL) || (keep_gobj == gobj))
			{
				continue;
			}
			...
			{
				port_log(
				    "SSB64 NetRbSnapshot: yoshi_egg_lay_prune tick=%u path=eject reason=live_duplicate "
				    "player=%d effect_gobj_id=%u keep_gobj_id=%u\n",
				    (unsigned int)((slot != NULL) ? slot->tick : syNetInputGetTick()), (int)fp->player,
				    (unsigned int)gobj->id, syNetRbSnapGobjId(keep_gobj));
			}
			syNetRbSnapEjectGObj(gobj);
		}
	}
...
sb32 syNetRbSnapRepairStageIsVerifyOnly(void)
{
	return s_syNetRbSnapRepairStageVerifyOnly;
}
#endif
...
#if defined(PORT) && defined(SSB64_NETMENU)
static sb32 syNetRbSnapPupupuWhispyXfNeedsRespawn(const LBTransform *xf)
{
	if (xf == NULL)
	{
		return TRUE;
...
static sb32 syNetRbSnapPupupuWhispyXfParticleBroken(const LBTransform *xf, s32 particle_link)
{
	f32 max_size;
	s32 drawable;
	s32 structs;
...
static sb32 syNetRbSnapPupupuWhispyXfNeedsRespawnForLink(const LBTransform *xf, s32 particle_link)
{
	if (syNetRbSnapPupupuWhispyXfNeedsRespawn(xf) != FALSE)
	{
		return TRUE;
	}
	return syNetRbSnapPupupuWhispyXfParticleBroken(xf, particle_link);
}

static sb32 syNetRbSnapPupupuWhispyParticleEffectDrawable(const LBTransform *xf, s32 particle_link)
{
	if (xf == NULL)
	{
		return FALSE;
...
/*
 * Whispy leaves/dust are LBParticles (not effect-link GObjs). Rollback load wipes all particles;
 * stale leaves_xf/dust_xf must be cleared on the first repair after reset. Later passes only gap-fill
 * so finalize/late repair does not orphan live structs or double-spawn every load.
 */
static void syNetRbSnapEnsurePupupuWhispyParticlesLive(sb32 clear_stale_xf)
{
	GRCommonGroundVarsPupupu *pu = &gGRCommonStruct.pupupu;
	sb32 spawned_leaves = FALSE;
	sb32 spawned_dust = FALSE;
	sb32 needs_leaves;
	sb32 needs_dust;
#if defined(SSB64_NETMENU)
	s32 particle_link = (pu->particle_bank_id | LBPARTICLE_MASK_GENLINK(0)) >> 3;
	s32 dust_particle_link = (pu->particle_bank_id | LBPARTICLE_MASK_GENLINK(1)) >> 3;
	...
	s32 dust_structs = 0;
	s32 leaves_generators = 0;
	s32 dust_generators = 0;
	u32 particle_struct_skip_flags = 0U;
	u32 particle_gen_skip_flags = 0U;

	if (gEFParticleStructsGObj != NULL)
	{
		particle_struct_skip_flags = gEFParticleStructsGObj->flags;
	}
	if (gEFParticleGeneratorsGObj != NULL)
	{
		particle_gen_skip_flags = gEFParticleGeneratorsGObj->flags;
	}
#endif
...
#if defined(PORT) && defined(SSB64_NETMENU)
	needs_leaves = syNetRbSnapPupupuWhispyXfNeedsRespawnForLink(pu->leaves_xf, particle_link);
	if ((needs_leaves != FALSE) &&
	    (syNetRbSnapPupupuWhispyParticleEffectDrawable(pu->leaves_xf, particle_link) != FALSE))
	{
		needs_leaves = FALSE;
	}
	needs_dust = FALSE;
	...
	{
		needs_dust = syNetRbSnapPupupuWhispyXfNeedsRespawnForLink(pu->dust_xf, dust_particle_link);
		if ((needs_dust != FALSE) &&
		    (syNetRbSnapPupupuWhispyParticleEffectDrawable(pu->dust_xf, dust_particle_link) != FALSE))
		{
			needs_dust = FALSE;
		}
	}
#else
	needs_leaves = (pu->leaves_xf == NULL) ? TRUE : FALSE;
	needs_dust = (pu->dust_xf == NULL) ? TRUE : FALSE;
#endif
	if (needs_leaves != FALSE)
	{
#if defined(SSB64_NETMENU)
		...
		spawned_leaves = (pu->leaves_xf != NULL) ? TRUE : FALSE;
	}
	if (needs_dust != FALSE)
	{
#if defined(SSB64_NETMENU)
		...
		spawned_dust = (pu->dust_xf != NULL) ? TRUE : FALSE;
	}
#if defined(SSB64_NETMENU)
	if ((pu->leaves_xf != NULL) && (pu->dust_xf != NULL) && (pu->leaves_xf == pu->dust_xf) &&
	    (syNetRbSnapPupupuWhispyRepairDiagEnabled() != FALSE))
	{
		port_log(
		    "SSB64 WhispyRepair: xf_alias tick=%u clear_stale=%d leaves_xf=%p dust_xf=%p spawned_leaves=%d "
		    "spawned_dust=%d\n",
		    syNetInputGetTick(),
		    (int)clear_stale_xf,
		    (void *)pu->leaves_xf,
	...
	if ((spawned_leaves != FALSE) || (spawned_dust != FALSE))
	{
		grPupupuWhispyWarmupLiveEffects();
	}
	if (pu->leaves_xf != NULL)
	{
		leaves_max_size =
		    lbParticleGetMaxDrawSizeForGeneratorID(pu->leaves_xf->generator_id, particle_link);
		leaves_drawable =
		    lbParticleCountDrawableForGeneratorID(pu->leaves_xf->generator_id, particle_link);
		leaves_structs =
		    lbParticleCountStructsForGeneratorID(pu->leaves_xf->generator_id, particle_link);
		leaves_generators =
		    lbParticleCountQueuedGeneratorsForID(pu->leaves_xf->generator_id);
	}
	if (pu->dust_xf != NULL)
	{
		dust_max_size =
		    lbParticleGetMaxDrawSizeForGeneratorID(pu->dust_xf->generator_id, dust_particle_link);
		dust_drawable =
		    lbParticleCountDrawableForGeneratorID(pu->dust_xf->generator_id, dust_particle_link);
		dust_structs =
		    lbParticleCountStructsForGeneratorID(pu->dust_xf->generator_id, dust_particle_link);
		dust_generators =
		    lbParticleCountQueuedGeneratorsForID(pu->dust_xf->generator_id);
	}
#endif
#ifdef PORT
	if (syNetRbSnapPupupuWhispyRepairDiagEnabled() != FALSE)
	{
		port_log(
		    "SSB64 WhispyRepair: status=%u clear_stale=%d bank_id=%d link=%d flowers_f=%u flowers_b=%u "
		    "mouth_tex=%d eyes_tex=%d lr=%d needs_leaves=%d needs_dust=%d leaves_xf=%p dust_xf=%p "
		    "leaves_alloc=%d dust_alloc=%d spawned_leaves=%d spawned_dust=%d "
		    "leaves_max_size=%f dust_max_size=%f leaves_drawable=%d dust_drawable=%d "
		    "leaves_structs=%d dust_structs=%d leaves_gn=%d dust_gn=%d "
		    ...
		    leaves_generators,
		    dust_generators,
		    (unsigned int)particle_struct_skip_flags,
		    (unsigned int)particle_gen_skip_flags);
#else
		    -1,
		    -1,
		    (int)spawned_leaves,
...
static void syNetRbSnapClearPupupuWhispyStaleParticleXf(void)
{
	GRCommonGroundVarsPupupu *pu = &gGRCommonStruct.pupupu;
	s32 leaves_link;
	s32 dust_link;
	...
	leaves_link = (pu->particle_bank_id | LBPARTICLE_MASK_GENLINK(0)) >> 3;
	dust_link = (pu->particle_bank_id | LBPARTICLE_MASK_GENLINK(1)) >> 3;
	if (pu->leaves_xf != NULL)
	{
		if (lbParticleTransformIsAllocated(pu->leaves_xf) == FALSE)
		{
			pu->leaves_xf = NULL;
		}
		else if (syNetRbSnapPupupuWhispyXfNeedsRespawnForLink(pu->leaves_xf, leaves_link) != FALSE)
		{
			grPupupuWhispyEjectLeavesEffect();
		}
	}
	if (pu->dust_xf != NULL)
	{
		if (lbParticleTransformIsAllocated(pu->dust_xf) == FALSE)
		{
			pu->dust_xf = NULL;
		}
		else if (syNetRbSnapPupupuWhispyXfNeedsRespawnForLink(pu->dust_xf, dust_link) != FALSE)
		{
			grPupupuWhispyEjectDustEffect();
		}
	}
...
static void syNetRbSnapEnsurePupupuWhispyAfterParticleReset(const SYNetRbSnapshotSlot *slot)
{
	...
	if (slot->ground.payload_len < SYNETRB_SNAP_GROUND_PUPUPU_LEGACY_PAYLOAD_LEN)
	{
		return;
	}
	syNetRbSnapClearPupupuWhispyStaleParticleXf();
}

sb32 syNetRbSnapPupupuWhispySlotIsBlow(u32 tick)
{
	SYNetRbSnapshotSlot *slot;
	const SYNetRbSnapGroundPupupu *src;

	slot = syNetRbSnapshotSlotForTick(tick);
	if ((slot == NULL) || (slot->is_valid == FALSE) || (slot->tick != tick) || (slot->ground_captured == FALSE))
	{
		return FALSE;
	}
	if (slot->ground.gkind != nGRKindPupupu)
	{
		return FALSE;
	}
	if (slot->ground.payload_len < SYNETRB_SNAP_GROUND_PUPUPU_LEGACY_PAYLOAD_LEN)
	...
	src = (const SYNetRbSnapGroundPupupu *)slot->ground.payload;
	return (src->whispy_status == (u8)nGRPupupuWhispyWindStatusBlow) ? TRUE : FALSE;
}

void syNetRbSnapRepairPupupuWhispyPresentationAfterLoad(u32 tick, const char *reason)
{
#if defined(PORT) && defined(SSB64_NETMENU)
	GRCommonGroundVarsPupupu *pu;
	s32 leaves_drawable = -1;
	s32 dust_drawable = -1;
	sb32 xf_alias = FALSE;
	...
	if (pu->whispy_status != (u8)nGRPupupuWhispyWindStatusBlow)
	{
		/* Cosmetic LBParticles are not snapshotted; eject orphans when wind is not active. */
		grPupupuWhispyEjectLeavesEffect();
		grPupupuWhispyEjectDustEffect();
		if ((syNetRbSnapPupupuWhispyRepairDiagEnabled() != FALSE) && (reason != NULL))
		{
			port_log(
			    "SSB64 WhispyRepair: post_verify tick=%u reason=%s action=eject_not_blowing whispy_status=%u "
			    "flowers_f=%u flowers_b=%u\n",
			    tick,
			    reason,
			    (u32)pu->whispy_status,
			    (u32)pu->flowers_front_status,
	...
	if (pu->leaves_xf != NULL)
	{
		...
		leaves_drawable = lbParticleCountDrawableForGeneratorID(pu->leaves_xf->generator_id, particle_link);
	}
	if (pu->dust_xf != NULL)
	{
		s32 dust_particle_link_diag = (pu->particle_bank_id | LBPARTICLE_MASK_GENLINK(1)) >> 3;

		dust_drawable = lbParticleCountDrawableForGeneratorID(pu->dust_xf->generator_id,
		                                                      dust_particle_link_diag);
	}
	if (syNetRbSnapPupupuWhispyRepairDiagEnabled() != FALSE)
	{
		port_log(
		    "SSB64 WhispyRepair: post_verify tick=%u reason=%s flowers_f=%u flowers_b=%u mouth_tex=%d "
		    "eyes_tex=%d leaves_drawable=%d dust_drawable=%d xf_alias=%d leaves_xf=%p dust_xf=%p\n",
		    tick,
	...
	if ((xf_alias != FALSE) && (syNetRbSnapPupupuWhispyRepairDiagEnabled() != FALSE))
	{
		port_log(
		    "SSB64 WhispyRepair: xf_alias tick=%u leaves_xf=%p dust_xf=%p leaves_alloc=%d dust_alloc=%d\n",
		    tick,
		    (void *)pu->leaves_xf,
		    (void *)pu->dust_xf,
		    (int)lbParticleTransformIsAllocated(pu->leaves_xf),
...
static void syNetRbSnapGapFillPupupuWhispyParticles(const SYNetRbSnapshotSlot *slot)
{
	if ((slot == NULL) || (slot->ground_captured == FALSE) || (slot->ground.gkind != nGRKindPupupu))
	{
		return;
...
#ifdef PORT
		/*
		 * Synctest verify-only loads must not gap-fill Whispy LBParticles or eject stale xf on the
		 * live pool — emergency restore does not fully reverse it (Yamabuki gate / Hyrule twister
		 * pattern). Ground blob fields still come from syNetRbSnapApplyGround.
		 */
		if (s_syNetRbSnapRepairStageVerifyOnly != FALSE)
		{
			/* syNetRbSnapResetParticlesForRollback already nulls xfs; keep verify-only path read-only. */
			if (syNetRbSnapPupupuWhispyRepairDiagEnabled() != FALSE)
			{
				port_log("SSB64 NetRbSnapshot: pupupu_whispy_repair tick=%u reason=verify_skip_all\n",
				         (unsigned int)slot->tick);
			}
			break;
		}
#endif
		if ((pupupu_clear_stale_xf != FALSE) && (syNetRbSnapSlotIsEmergencySentinel(slot) == FALSE))
		{
			syNetRbSnapEnsurePupupuWhispyAfterParticleReset(slot);
		}
		else
		{
			syNetRbSnapGapFillPupupuWhispyParticles(slot);
		}
		break;
	case nGRKindYoster:
...
{
	syNetRbSnapRepairStageAfterParticleResetInternal(slot, TRUE, TRUE);
}
                                             ...
                                             GObj *gobj, EFStruct *ep)
{
	...
	if (syNetRbSnapEffectBlobIsUserdataJoint(blob) != FALSE)
	{
		...
		fp_live = (ep->fighter_gobj != NULL) ? ftGetStruct(ep->fighter_gobj) : NULL;
		if (blob->quake_magnitude != 0xFFU)
		{
			s32 live_joint;

			dobj_uj = DObjGetStruct(gobj);
			if ((fp_live == NULL) || (dobj_uj == NULL))
			{
				return FALSE;
			}
			live_joint = syNetRbSnapFindFighterJointIndex(fp_live, dobj_uj->user_data.p);
			if (live_joint < 0)
			{
				live_joint = syNetRbSnapInferCaptainFalconPunchJointIndex(fp_live);
			}
			if (live_joint != (s32)blob->quake_magnitude)
			{
				return FALSE;
			}
		}
...
static void syNetRbSnapReapplyEffectBlobsFromSlot(const SYNetRbSnapshotSlot *slot)
{
	...
	for (ei = 0; ei < slot->effect_count; ei++)
	{
		...
		if (syNetRbSnapEffectBlobIsShieldKind(blob) != FALSE)
		{
			...
			if (match == NULL)
			{
				match = syNetRbSnapFindLiveShieldEffectForFighter(
				    syNetRbSnapResolveFighterGobjByPlayer((s8)shield_player));
			}
			if (match != NULL)
			{
		...
		if (gobj != NULL)
		{
			...
			ep_by_id = efGetStruct(gobj);
			if ((syNetRbSnapEffectHiddenFromRollback(gobj, ep_by_id) == FALSE) &&
			    (syNetRbSnapLiveEffectMatchesBlob(slot, blob, gobj, ep_by_id) != FALSE))
			{
				(void)syNetRbSnapApplyEffectBlobToGObj(slot, gobj, blob);
				continue;
			}
                                                            ...
                                                            s32 reconciled_gobj_count)
{
	...
	if (syNetRbSnapEffectBlobIsShieldKind(blob) != FALSE)
	{
		...
		if (gobj != NULL)
		{
			...
			/*
			 * Per-player shield identity: adopt the live bubble even when ring gobj_id or guard coupling
			 * fields are stale (escape roll keeps is_shield=0 while the shell persists).
			 */
			return gobj;
		}
		return NULL;
	...
	if ((gobj != NULL) && (syNetRbSnapLiveEffectIsShield(gobj, ep) != FALSE) &&
	    (blob->respawn_kind != SYNETRB_EFFECT_RESPAWN_SHIELD) &&
	    (blob->respawn_kind != SYNETRB_EFFECT_RESPAWN_YOSHI_SHIELD))
	{
		gobj = NULL;
		ep = NULL;
	}
	if ((gobj != NULL) && (syNetRbSnapLiveEffectIsQuake(gobj, ep) != FALSE) &&
	    (blob->respawn_kind != SYNETRB_EFFECT_RESPAWN_QUAKE))
	{
		gobj = NULL;
		ep = NULL;
	...
	if (blob->respawn_kind == SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE)
	{
		...
		if (fighter_gobj != NULL)
		{
			gobj = syNetRbSnapFindLiveYoshiEggEscapeEffectForFighter(fighter_gobj);
			if (gobj == NULL)
			{
				gobj = syNetRbSnapFindLiveYoshiEggEscapeEffectRelaxedForFighter(fighter_gobj);
			}
			if ((gobj != NULL) &&
			    (syNetRbSnapReconciledEffectGobjPtrListed(reconciled_gobj_ptrs, reconciled_gobj_count,
			                                              gobj) == FALSE))
			{
				return gobj;
			}
...
#if defined(SSB64_NETMENU)
		if ((live_shield == NULL) && (s_syNetRbSnapRepairStageVerifyOnly != FALSE))
		{
			const SYNetRbSnapEffectBlob *shield_blob;
			...
			if (shield_blob != NULL)
			{
				live_shield = syNetRbSnapTryRespawnEffectFromBlob(slot, shield_blob);
			}
		}
#endif
		if (live_shield == NULL)
...
static sb32 syNetRbSnapLiveEffectIsYoshiEggEscape(const GObj *gobj, const EFStruct *ep)
{
	...
	if ((ep->proc_update == efManagerYoshiEggLayProcUpdate) &&
	    (syNetRbSnapEffectGObjHasUpdateProc(gobj, efManagerYoshiEggLayProcUpdate) != FALSE))
	{
		return FALSE;
	}
	fp = ftGetStruct(ep->fighter_gobj);
	...
	if (((GObj *)gobj)->proc_display != efManagerYoshiShieldProcDisplay)
	{
		return FALSE;
	}
	if (syNetRbSnapEffectGObjHasUpdateProc(gobj, efManagerShieldProcUpdate) != FALSE)
	{
		return FALSE;
	}
	if (syNetRbSnapEffectGObjHasUpdateProc(gobj, efManagerYoshiEggLayProcUpdate) != FALSE)
	{
		return FALSE;
	}
	if ((ep->proc_update != NULL) && (ep->proc_update != efManagerNoEjectProcUpdate))
	{
		return FALSE;
	}
	dobj = DObjGetStruct((GObj *)gobj);
	...
	return ((fp->joints[5] != NULL) && (dobj->user_data.p == fp->joints[5])) ? TRUE : FALSE;
}
...
static GObj *syNetRbSnapFindLiveYoshiEggEscapeEffectForFighter(const GObj *fighter_gobj)
{
	...
	for (pass = 0; pass < 2; pass++)
	{
		     ...
		     gobj != NULL; gobj = gobj->link_next)
		{
			...
			if ((syNetRbSnapLiveEffectIsYoshiEggEscape(gobj, ep) == FALSE) || (ep->fighter_gobj != fighter_gobj))
			{
				continue;
			}
			return gobj;
...
/*
 * Verify/adopt path: strict LiveEffectIsYoshiEggEscape requires joints[5] userdata match, which can fail
 * after rollback load before cosmetic couple runs — missing the live shell forces MakeEffect (tick 2318).
 */
static GObj *syNetRbSnapFindLiveYoshiEggEscapeEffectRelaxedForFighter(const GObj *fighter_gobj)
{
	FTStruct *fp;
...
static void syNetRbSnapPruneStaleYoshiEggEscapeEffects(const SYNetRbSnapshotSlot *slot)
{
	...
	for (pass = 0; pass < 2; pass++)
	{
		...
		for (gobj = link_head; gobj != NULL; gobj = next)
		{
			...
			{
				port_log(
				    "SSB64 NetRbSnapshot: yoshi_egg_escape_prune tick=%u player=%d status=%d "
				    "reason=slot_not_escape effect_gobj_id=%u\n",
				    (unsigned int)slot->tick, (int)player, (int)fp->status_id, (unsigned int)gobj->id);
			}
			syNetRbSnapEjectGObj(gobj);
		}
	}
...
static void syNetRbSnapPruneDuplicateYoshiEggEscapeEffects(const SYNetRbSnapshotSlot *slot)
{
	...
	for (pi = 0; pi < GMCOMMON_PLAYERS_MAX; pi++)
	{
		...
		if (syNetRbSnapSlotHasYoshiEggEscapeBlobForPlayer(slot, pi) == FALSE)
		{
			continue;
		}
		fighter_gobj = syNetRbSnapResolveFighterGobjByPlayer((s8)pi);
		...
		keep_gobj = syNetRbSnapFindLiveYoshiEggEscapeEffectForFighter(fighter_gobj);
		for (pass = 0; pass < 2; pass++)
		{
			GObj *link_head;
			GObj *gobj;
...
static void syNetRbSnapEjectStaleYoshiEggLayShellsForFighter(GObj *fighter_gobj, GObj *keep_gobj)
{
	...
	for (pass = 0; pass < 2; pass++)
	{
		     ...
		     gobj != NULL; gobj = next)
		{
			...
			next = gobj->link_next;
			if ((gobj == keep_gobj) || (syNetRbSnapShieldEffectMatchesKeep(gobj, keep_gobj) != FALSE))
			{
				continue;
			}
			ep = efGetStruct(gobj);
			...
			{
				port_log(
				    "SSB64 NetRbSnapshot: yoshi_egg_lay_prune tick=%u path=eject reason=escape_stale_orphan "
				    "player=%d effect_gobj_id=%u status=%d\n",
...
static sb32 syNetRbSnapStartYoshiEggLayHatchCosmeticLive(GObj *fighter_gobj, const Vec3f *pos)
{
	...
	{
		syNetRbSnapClearCaptureYoshiEffectPointerIfMatch(fp, effect_gobj);
		gcEjectGObj(effect_gobj);
		effect_gobj = efManagerYoshiEggLayMakeEffect(fighter_gobj);
		if (effect_gobj == NULL)
		{
...
static GObj *syNetRbSnapReplayYoshiEggLayHatchShell(GObj *fighter_gobj, const Vec3f *pos)
{
	...
	if (effect_gobj != NULL)
	{
		    ...
		    (ep->effect_vars.yoshi_egg_lay.index == 1) && (effect_gobj->anim_frame > 0.0F))
		{
			...
			if (fp != NULL)
			{
				syNetRbSnapArmYoshiEggLayHatchShellParticles(fp->player, pos);
			}
			return effect_gobj;
		}
	}
...
static void syNetRbSnapSanitizeGuardEffectGobj(FTStruct *fp)
{
	...
	if ((fp == NULL) || (syNetRbSnapFighterInYoshiEggLayScope(fp) != FALSE) ||
	    (syNetRbSnapFighterGuardEffectUnionOwned(fp) == FALSE))
	{
		return;
	}
	...
	ep = efGetStruct(effect_gobj);
	if ((ep == NULL) || (syNetRbSnapLiveEffectIsShield(effect_gobj, ep) == FALSE) ||
	    (syNetRbSnapLiveEffectIsYoshiEggLay(effect_gobj, ep) != FALSE))
	{
		ftStatusVarsGuard(fp)->effect_gobj = NULL;
		if (fp->is_effect_attach != FALSE)
		{
...
void syNetRbSnapSanitizeCaptureYoshiEffectGobj(FTStruct *fp)
{
	...
	if (fp->status_id == nFTCommonStatusYoshiEgg)
	{
		...
		{
			/*
			 * Clearing the coupling without ejecting orphans the shell on the effect link; the next
			 * ftCommonYoshiEggMakeEffect tick mints a second egg (effect_count 1→2 @ first overlap).
			 */
			syNetRbSnapClearCaptureYoshiEffectPointerIfMatch(fp, effect_gobj);
			syNetRbSnapEjectGObj(effect_gobj);
			if (fp->is_effect_attach != FALSE)
			{
				fp->is_effect_attach = FALSE;
			}
...
static void syNetRbSnapEnsureYoshiEggLayEffectsFromSlot(const SYNetRbSnapshotSlot *slot)
{
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		if (syNetRbSnapCountLiveYoshiEggLayEffectsForFighter(fighter_gobj) > 0)
		{
			eg = syNetRbSnapPickCanonicalYoshiEggLayForFighter(fighter_gobj, fp);
			if (eg != NULL)
			{
				ftStatusVarsCaptureYoshi(fp)->effect_gobj = eg;
				fp->is_effect_attach = TRUE;
				...
				{
					port_log(
					    "SSB64 NetRbSnapshot: yoshi_egg_lay_ensure path=owner_exists player=%d "
					    "fighter_gobj_id=%u effect_gobj_id=%u live_count=%d\n",
					    (int)pi, (unsigned int)fighter_gobj->id, (unsigned int)eg->id,
					    (int)syNetRbSnapCountLiveYoshiEggLayEffectsForFighter(fighter_gobj));
				}
				continue;
			}
		}
...
sb32 syNetRbSnapTryAdoptLiveYoshiEggLayEffectForFighter(GObj *fighter_gobj)
{
	FTStruct *fp;
	GObj *live;
	...
	if (syNetRbSnapYoshiEggLayEffectOwnedByFighter(ftStatusVarsCaptureYoshi(fp)->effect_gobj, fighter_gobj) !=
	    FALSE)
	{
		fp->is_effect_attach = TRUE;
		return TRUE;
	...
	if (live == NULL)
	{
		live = syNetRbSnapPickCanonicalYoshiEggLayForFighter(fighter_gobj, fp);
	}
	if (live == NULL)
	{
		return FALSE;
...
GObj *syNetRbSnapTryAdoptLiveYoshiShieldForEscapeEnd(GObj *fighter_gobj)
{
	FTStruct *fp;
	GObj *escape_gobj;
	GObj *shield_gobj;
	...
	syNetRbSnapEjectStaleYoshiEggLayShellsForFighter(fighter_gobj, NULL);
	escape_gobj = syNetRbSnapFindLiveYoshiEggEscapeEffectForFighter(fighter_gobj);
	if (escape_gobj != NULL)
	{
		syNetRbSnapClearFighterEffectPointerIfMatch(fp, escape_gobj);
		if (fp->is_effect_attach != FALSE)
		{
			fp->is_effect_attach = FALSE;
		}
		syNetRbSnapEjectGObj(escape_gobj);
	}
	shield_gobj = syNetRbSnapFindLiveShieldEffectForFighter(fighter_gobj);
	if (shield_gobj == NULL)
	{
		...
		for (gobj = link_head; gobj != NULL; gobj = next)
		{
			...
			pi = fp->player;
			{
				...
				if (keep_gobj != NULL)
				{
					...
					if ((gobj->id != 0U) && (gobj->id == syNetRbSnapGobjId(keep_gobj)))
					{
						if (gobj != keep_gobj)
						{
							if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
							{
								port_log(
								    "SSB64 NetRbSnapshot: yoshi_egg_lay_prune tick=%u path=eject "
								    "reason=owner_duplicate_id_collision player=%d effect_gobj_id=%u "
								    "keep_gobj_id=%u\n",
								    (unsigned int)((slot != NULL) ? slot->tick : 0U), (int)pi,
								    (unsigned int)gobj->id, syNetRbSnapGobjId(keep_gobj));
							}
							syNetRbSnapClearCaptureYoshiEffectPointerIfMatch(fp, gobj);
							syNetRbSnapEjectGObj(gobj);
...
static void syNetRbSnapEnsureShieldEffectsFromSlot(const SYNetRbSnapshotSlot *slot)
{
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		 ...
		 */
		if ((s_syNetRbSnapRepairStageVerifyOnly != FALSE) || (syNetRbSnapSlotIsEmergencySentinel(slot) != FALSE))
		{
			const SYNetRbSnapEffectBlob *shield_blob;
			...
			if (shield_blob != NULL)
			{
				...
				else
				{
					...
					respawned = syNetRbSnapTryRespawnEffectFromBlob(slot, shield_blob);
					if (respawned != NULL)
					{
						syNetRbSnapPatchLiveShieldFromSlot(slot, fighter_gobj, fp, shield_blob->gobj_id,
						                                   respawned);
			...
			/*
			 * Verify-only: slot effect blobs + EnforceSlotAuthoritativeEffectSet own respawn — never
			 * fall through to synth mint here (guard @ T + escape leak @ T+1 double-shield class).
			 */
			continue;
		}
#endif
		...
		if ((eb != NULL) &&
		    ...
		    (syNetRbSnapEffectBlobAuthoritativeForSlot(slot, eb) != FALSE))
		{
			(void)syNetRbSnapTryRespawnEffectFromBlob(slot, eb);
		}
		else if (syNetRbSnapBlobInGuardScope(fb) != FALSE)
		{
			EFStruct synth_shield_vars;

			memset(&synth, 0, sizeof(synth));
			memset(&synth_shield_vars, 0, sizeof(synth_shield_vars));
			synth_shield_vars.effect_vars.shield.player = pi;
			memcpy(synth.effect_vars, &synth_shield_vars.effect_vars, sizeof(synth.effect_vars));
			synth.is_valid = TRUE;
			synth.gobj_id = eff_id;
			synth.fighter_gobj_id = (u32)fighter_gobj->id;
			synth.respawn_kind = (fp->fkind == nFTKindYoshi) ? SYNETRB_EFFECT_RESPAWN_YOSHI_SHIELD
			                                                  : SYNETRB_EFFECT_RESPAWN_SHIELD;
			if (syNetRbSnapEffectBlobAuthoritativeForSlot(slot, &synth) != FALSE)
			{
				(void)syNetRbSnapTryRespawnEffectFromBlob(slot, &synth);
			}
		}
	...
	for (pass = 0; pass < 2; pass++)
	{
		...
		for (gobj = link_head; gobj != NULL; gobj = next)
		{
...
#if defined(SSB64_NETMENU)
				if ((slot != NULL) && (s_syNetRbSnapRepairStageVerifyOnly != FALSE))
				{
					s32 shield_player;
					s32 ei;
					sb32 defer_no_fighter_eject;

					defer_no_fighter_eject = FALSE;
					shield_player = syNetRbSnapShieldPlayerFromEffectVars(ep);
					if ((shield_player >= 0) &&
					    (syNetRbSnapFindShieldEffectBlobForPlayer(slot, shield_player) != NULL))
					{
						defer_no_fighter_eject = TRUE;
					}
					else
					{
						for (ei = 0; ei < slot->effect_count; ei++)
						{
							const SYNetRbSnapEffectBlob *defer_blob = &slot->effects[ei];

							if ((defer_blob->is_valid != FALSE) &&
							    (syNetRbSnapEffectBlobIsShieldKind(defer_blob) != FALSE) &&
							    (defer_blob->gobj_id == gobj->id))
							{
								defer_no_fighter_eject = TRUE;
								break;
							}
					...
					if (defer_no_fighter_eject != FALSE)
					{
						continue;
					}
				}
			...
			if (shielding == FALSE)
			{
				if (syNetRbSnapFighterInYoshiShieldEscapePresentationScope(fp) != FALSE)
				{
					syNetRbSnapAuditLiveShieldEffectOwner(gobj, fp);
					syNetRbSnapApplyYoshiShieldPresentation(ep->fighter_gobj, fp);
					continue;
				}
				syNetRbSnapClearFighterEffectPointerIfMatch(fp, gobj);
				if (fp->is_effect_attach != FALSE)
...
static void syNetRbSnapReconcileFighterShieldCoupling(sb32 live_forward_policy)
{
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		if (live_shield == NULL)
		{
			live_shield = syNetRbSnapFindLiveShieldEffectByPlayerSlot(fp->player);
		}
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		if (log_tick != FALSE)
		{
			port_log(
			    ...
			    (int)ftStatusVarsGuard(fp)->is_release, (int)ftStatusVarsGuard(fp)->release_lag,
			    (int)syNetRbSnapFighterShieldInputHeld(fp),
			    (int)syNetRbSnapFighterShieldBubbleVisible(fighter_gobj, fp));
		}
...
static void syNetRbSnapDiagGuardShieldWithoutBubbleGap(const SYNetRbSnapshotSlot *slot)
{
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		fp = ftGetStruct(fighter_gobj);
		if ((fp == NULL) || (syNetRbSnapFighterInYoshiShieldEscapePresentationScope(fp) != FALSE))
		{
			continue;
		}
		player = fp->player;
		...
		if ((fp->is_shield != FALSE) && (z_auth != FALSE) &&
		    ...
		    (syNetRbSnapFighterShieldBubbleVisible(fighter_gobj, fp) == FALSE))
		{
...
static void syNetRbSnapDiagGuardShieldBubbleLinger(const SYNetRbSnapshotSlot *slot)
{
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		live_bubble = syNetRbSnapFindLiveShieldEffectForPlayer(player);
		if (live_bubble == NULL)
		{
...
static void syNetRbSnapReconcileGuardShieldEffectsCore(const SYNetRbSnapshotSlot *slot)
{
	...
	if (slot != NULL)
	{
		...
		syNetRbSnapPruneStaleYoshiEggLayShellsOutsideSpecialScope();
		syNetRbSnapReconcileYoshiShieldEscapePresentation(slot);
...
static void syNetRbSnapReconcileYoshiEggLayEffectsCore(const SYNetRbSnapshotSlot *slot)
{
	...
	syNetRbSnapPruneDuplicateLiveYoshiEggLayEffects(slot);
	syNetRbSnapSanitizeAllFightersCaptureYoshiEffectGobjs();
...
static sb32 syNetRbSnapEffectBlobYoshiEggLayHatchReady(const SYNetRbSnapEffectBlob *blob)
{
	const EFCommonEffectVarsYoshiEggLay *vars;
	...
	vars = (const EFCommonEffectVarsYoshiEggLay *)(const void *)blob->effect_vars;
	return ((vars->index == 1) && (blob->anim_frame <= 0.0F)) ? TRUE : FALSE;
}

static const SYNetRbSnapEffectBlob *syNetRbSnapFindYoshiEggLayHatchReadyBlobForPlayer(const SYNetRbSnapshotSlot *slot,
                                                                                    s32 player)
{
	const SYNetRbSnapFighterBlob *fb;
	u32 victim_gobj_id;
	...
	for (ei = 0; ei < slot->effect_count; ei++)
	{
		...
		if (blob->respawn_kind != SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_LAY)
		{
			continue;
		}
		if ((blob->fighter_gobj_id != victim_gobj_id) && (fb->captureyoshi_effect_gobj_id != blob->gobj_id))
		{
			continue;
		}
		if (syNetRbSnapEffectBlobYoshiEggLayHatchReady(blob) != FALSE)
		{
			return blob;
		}
	}
...
#if defined(SSB64_NETMENU)
static void syNetRbSnapMarkYoshiEggLayHatchReplayDone(s32 player, u32 tick)
{
#ifdef PORT
	if ((player >= 0) && (player < GMCOMMON_PLAYERS_MAX))
	{
...
static void syNetRbSnapTryReplayYoshiEggLayHatchCosmeticsForPlayer(const SYNetRbSnapshotSlot *slot, s32 pi,
                                                                 sb32 yoshi_egg_shell_break)
{
	const SYNetRbSnapFighterBlob *fb;
	GObj *fighter_gobj;
	FTStruct *fp;
	...
	{
		if ((fp->status_id != nFTCommonStatusYoshiEgg) && (fb->status_id != nFTCommonStatusYoshiEgg))
		{
			return;
		}
	}
	else
	{
		...
		if ((fb->status_id != nFTCommonStatusFall) || (fp->status_id != nFTCommonStatusFall) ||
		    (vel_y < (FTCOMMON_YOSHIEGG_ESCAPE_VEL_Y - 4.0F)) || (vel_y > (FTCOMMON_YOSHIEGG_ESCAPE_VEL_Y + 4.0F)))
		{
			return;
		}
	}
	...
	if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
	{
		...
		if (shell_gobj == NULL)
		{
			shell_gobj = syNetRbSnapFindLiveYoshiEggLayEffectForFighter(fighter_gobj);
		}
		port_log(
		    "SSB64 NetRbSnapshot: yoshi_egg_lay_hatch_replay tick=%u player=%d fighter_gobj_id=%u "
		    "vel_y=%.2f shell_break=%d deferred=%d shell_gobj_id=%u\n",
		    (unsigned int)slot->tick, (int)pi, (unsigned int)fighter_gobj->id, fp->physics.vel_air.y,
		    (int)yoshi_egg_shell_break, (int)deferred_replay,
		    (shell_gobj != NULL) ? (unsigned int)shell_gobj->id : 0U);
	}
}
#endif
...
static void syNetRbSnapReplayYoshiEggLayHatchCosmeticsFromSlot(const SYNetRbSnapshotSlot *slot)
{
	...
	for (pi = 0; pi < GMCOMMON_PLAYERS_MAX; pi++)
	{
		const SYNetRbSnapFighterBlob *fb;
#if defined(SSB64_NETMENU)
		const SYNetRbSnapEffectBlob *hatch_blob;
		sb32 yoshi_egg_shell_break;
#endif
...
#if defined(SSB64_NETMENU)
		hatch_blob = syNetRbSnapFindYoshiEggLayHatchReadyBlobForPlayer(slot, pi);
		yoshi_egg_shell_break =
		    ((fb->status_id == nFTCommonStatusYoshiEgg) && (hatch_blob != NULL)) ? TRUE : FALSE;
#ifdef PORT
		...
		{
			GObj *fighter_gobj = syNetRbSnapResolveFighterGobjByPlayer((s8)pi);
			FTStruct *fp = (fighter_gobj != NULL) ? ftGetStruct(fighter_gobj) : NULL;
			Vec3f pos;

			if ((fighter_gobj == NULL) || (fp == NULL) || (fp->status_id != nFTCommonStatusFall))
			{
				continue;
			}
			syNetRbSnapYoshiEggLayHatchPosFromCapture(fighter_gobj, fp, &pos);
...
static void syNetRbSnapReconcileSnapshotEffectsBeforeItems(const SYNetRbSnapshotSlot *slot)
{
	...
	syNetRbSnapPruneStaleYoshiEggEscapeEffects(slot);
	reconciled_count = 0;
	...
	for (ei = 0; ei < slot->effect_count; ei++)
	{
		...
		if (syNetRbSnapEffectBlobAuthoritativeForSlot(slot, blob) == FALSE)
		{
			continue;
		}
	 ...
	 * Verify-only: enforce handles missing blobs without pass-3 respawn spam (Phase 46 quake hyper-stack).
	 */
	if (s_syNetRbSnapRepairStageVerifyOnly == FALSE)
	{
		for (ei = 0; ei < slot->effect_count; ei++)
...
#if defined(SSB64_NETMENU)
	/*
	 * Verify-only: pass-3 respawn is disabled to avoid quake hyper-stack, but shield blobs still need
	 * one-shot restore when emergency live @ T+1 left the bubble detached (Whispy quake + GuardOff @2144).
	 */
	if (s_syNetRbSnapRepairStageVerifyOnly != FALSE)
	{
		for (ei = 0; ei < slot->effect_count; ei++)
		{
			...
			blob_applied[ei] = TRUE;
			syNetRbSnapTrackReconciledEffectGobjShieldAware(reconciled_ids, &reconciled_count,
			                                                &reconciled_shield_mask, reconciled_gobj_ptrs,
			                                                &reconciled_gobj_count, gobj, blob);
		}
	}
	...
	syNetRbSnapPruneExcessSlotMatchedQuakes(slot);
	syNetRbSnapPruneDuplicateYoshiEggEscapeEffects(slot);
...
/*
 * Synctest verify: eject every hashed effect GObj not in the canonical per-blob set so live eff fold
 * cardinality matches slot->effect_count (quake stack / orphan VFX class).
 */
static void syNetRbSnapEjectAllNonCanonicalEffectsForVerify(const SYNetRbSnapshotSlot *slot,
                                                            GObj * const *canonical_gobj_ptrs,
                                                            s32 canonical_gobj_count)
...
static void syNetRbSnapEnforceSlotAuthoritativeEffectSet(SYNetRbSnapshotSlot *slot)
{
	...
	syNetRbSnapSanitizeSlotEffectGobjIdCollisions(slot);
#if defined(SSB64_NETMENU)
	if (s_syNetRbSnapRepairStageVerifyOnly == FALSE)
#endif
	{
		...
		if (live == NULL)
		{
			...
			if (s_syNetRbSnapRepairStageVerifyOnly != FALSE)
			{
				if (syNetRbSnapEffectBlobIsShieldKind(blob) != FALSE)
				{
					...
					if (shield_player >= 0)
					{
						live = syNetRbSnapFindLiveShieldEffectForPlayer(shield_player);
						if (live == NULL)
						{
							live = syNetRbSnapFindLiveShieldEffectForFighter(
							    syNetRbSnapResolveFighterGobjByPlayer((s8)shield_player));
						}
						if (live != NULL)
						{
				...
				else if (blob->respawn_kind == SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE)
				{
					...
					if (escape_parent != NULL)
					{
						live = syNetRbSnapFindLiveYoshiEggEscapeEffectRelaxedForFighter(escape_parent);
						if (live == NULL)
						{
							live = syNetRbSnapFindLiveYoshiEggEscapeEffectForFighter(escape_parent);
						}
						if (live != NULL)
						{
							live = syNetRbSnapApplyEffectBlobToGObj(slot, live, blob);
...
static sb32 syNetRbSnapCaptureEffects(SYNetRbSnapshotSlot *slot)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		u32 respawn_kind_precheck;
		sb32 blob_append;

		gobj_iter = sorted[i];
		ep = efGetStruct(gobj_iter);
		respawn_kind_precheck = SYNETRB_EFFECT_RESPAWN_NONE;
		if (ep != NULL)
		{
			respawn_kind_precheck = syNetRbSnapEffectRespawnKindFromLive(gobj_iter, ep);
			if (respawn_kind_precheck == SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_LAY)
			{
				u32 victim_id = (ep->fighter_gobj != NULL) ? (u32)ep->fighter_gobj->id : 0U;
				s32 j;
				sb32 skip_dup;
				...
				if (skip_dup != FALSE)
				{
					...
					{
						port_log(
						    "SSB64 NetRbSnapshot: effect save skip tick=%u gobj_id=%u "
						    "reason=yoshi_egg_lay_victim_duplicate fighter_gobj_id=%u\n",
						    (unsigned int)slot->tick, (unsigned int)gobj_iter->id, victim_id);
					}
					continue;
				}
			...
			else if (respawn_kind_precheck == SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE)
			{
				...
				if (skip_dup != FALSE)
				{
					if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
					{
						port_log(
						    "SSB64 NetRbSnapshot: effect save skip tick=%u gobj_id=%u "
						    "reason=yoshi_egg_escape_duplicate fighter_gobj_id=%u\n",
						    (unsigned int)slot->tick, (unsigned int)gobj_iter->id, victim_id);
					}
					continue;
				}
				if ((victim_id != 0U) && (seen_yoshi_egg_escape_victim_count < SYNETRB_SNAPSHOT_MAX_EFFECTS))
				{
					seen_yoshi_egg_escape_victim_ids[seen_yoshi_egg_escape_victim_count++] = victim_id;
			         ...
			         (respawn_kind_precheck == SYNETRB_EFFECT_RESPAWN_YOSHI_SHIELD))
			{
				s32 shield_player;
				sb32 skip_dup;

				shield_player = syNetRbSnapShieldPlayerFromEffectVars(ep);
				if ((shield_player < 0) && (ep->fighter_gobj != NULL) && (ftGetStruct(ep->fighter_gobj) != NULL))
				{
					shield_player = ftGetStruct(ep->fighter_gobj)->player;
				}
				skip_dup = FALSE;
				if ((shield_player >= 0) && (shield_player < GMCOMMON_PLAYERS_MAX) &&
				    (seen_shield_players[shield_player] != 0U))
				{
					skip_dup = TRUE;
				}
				if (skip_dup != FALSE)
				{
					if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
					{
						port_log(
						    "SSB64 NetRbSnapshot: effect save skip tick=%u gobj_id=%u "
						    "reason=shield_player_duplicate shield_player=%d\n",
						    (unsigned int)slot->tick, (unsigned int)gobj_iter->id, shield_player);
					}
					continue;
				}
				if ((shield_player >= 0) && (shield_player < GMCOMMON_PLAYERS_MAX))
				{
					seen_shield_players[shield_player] = 1U;
				}
			}
		}
		{
			...
			blob_target = written;
			blob_append = TRUE;
			for (wi = 0; wi < written; wi++)
			{
				if (slot->effects[wi].gobj_id != gobj_iter->id)
				{
					continue;
				}
				...
				{
					port_log(
					    "SSB64 NetRbSnapshot: effect save skip tick=%u gobj_id=%u reason=gobj_id_duplicate\n",
					    (unsigned int)slot->tick, (unsigned int)gobj_iter->id);
				}
				blob_target = -1;
				break;
			}
		...
		if (ep != NULL)
		{
			...
			else if (blob->respawn_kind == SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_ESCAPE)
			{
				...
				fp_cap = (ep->fighter_gobj != NULL) ? ftGetStruct(ep->fighter_gobj) : NULL;
				if ((fp_cap != NULL) && (fp_cap->player >= 0) && (fp_cap->player < GMCOMMON_PLAYERS_MAX))
				{
					blob->quake_magnitude = (u8)fp_cap->player;
					if (slot->fighters[fp_cap->player].is_valid != FALSE)
					{
						blob->fighter_gobj_id = slot->fighters[fp_cap->player].gobj_id;
					}
				}
			}
			...
			else if (syNetRbSnapLiveEffectIsUserdataJointAttach(gobj_iter, ep) != FALSE)
			{
				...
				if (joint_idx < 0)
				{
					joint_idx = syNetRbSnapInferCaptainFalconPunchJointIndex(fp_joint);
				}
				if (joint_idx >= 0)
				{
					blob->snap_flags |= SYNETRB_EFFECT_SNAP_USERDATA_JOINT;
					blob->quake_magnitude = (u8)joint_idx;
					blob->respawn_kind = SYNETRB_EFFECT_RESPAWN_USERDATA_JOINT;
				}
				else if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
				{
					port_log(
					    "SSB64 NetRbSnapshot: effect save userdata_joint_skip tick=%u gobj_id=%u "
					    "fighter_gobj_id=%u reason=joint_idx_unresolved\n",
					    (unsigned int)slot->tick, (unsigned int)gobj_iter->id, blob->fighter_gobj_id);
				}
			}
		...
		if (blob_append != FALSE)
		{
			written++;
		}
	}
	slot->effect_count = written;
	syNetRbSnapSanitizeSlotEffectGobjIdCollisions(slot);
	if (syNetRbSnapSnapshotEffectDiagEnabled() != FALSE)
	{
...
/*
 * Guard @ probe_tick with shield bubble vs escape shell @ probe_tick+1 (emergency T+1 class): verify load
 * prunes escape then shield mint stomps figh/anim — skip probe on the boundary tick.
 */
static sb32 syNetRbSnapshotSynctestProbeGuardEscapeBoundaryFragile(u32 probe_tick)
{
	SYNetRbSnapshotSlot *probe_slot;
	SYNetRbSnapshotSlot *next_slot;
	s32 pidx;

	probe_slot = syNetRbSnapshotSlotForTick(probe_tick);
	next_slot = syNetRbSnapshotSlotForTick(probe_tick + 1U);
	if ((probe_slot == NULL) || (probe_slot->is_valid == FALSE) || (probe_slot->tick != probe_tick))
	{
		return FALSE;
	}
	if ((next_slot == NULL) || (next_slot->is_valid == FALSE) || (next_slot->tick != (probe_tick + 1U)))
	{
		return FALSE;
	}
	for (pidx = 0; pidx < GMCOMMON_PLAYERS_MAX; pidx++)
	{
		const SYNetRbSnapFighterBlob *probe_fb;

		if (syNetRbSnapSlotHasYoshiEggEscapeBlobForPlayer(next_slot, pidx) == FALSE)
		{
			continue;
		}
		if (syNetRbSnapSlotHasYoshiEggEscapeBlobForPlayer(probe_slot, pidx) != FALSE)
		{
			continue;
		}
		probe_fb = &probe_slot->fighters[pidx];
		if ((probe_fb->is_valid == FALSE) || (syNetRbSnapBlobInGuardScope(probe_fb) == FALSE))
		{
			continue;
		}
		if (syNetRbSnapFindShieldEffectBlobForPlayer(probe_slot, pidx) != NULL)
		{
			return TRUE;
		}
	}
...
static s32 syNetRbSnapGuardReleaseLagFromBlob(const SYNetRbSnapFighterBlob *blob)
{
	union FTStatusVars blob_sv;
	...
	memcpy(&blob_sv, blob->status_vars, sizeof(blob_sv));
	return blob_sv.common.guard.release_lag;
}

/*
 * Guard @ probe_tick with GuardOff/GuardSetOff @ probe_tick+1 (Kirby tap class @1085): verify load
 * cannot round-trip joint anim through the release frame while emergency live is already at T+1.
 * GuardOff with release_lag>0 (Whispy quake + shield @3602): dual-effect verify + guard coupling.
 */
static sb32 syNetRbSnapshotSynctestProbeGuardReleaseBoundaryFragile(u32 probe_tick)
{
	SYNetRbSnapshotSlot *probe_slot;
	SYNetRbSnapshotSlot *next_slot;
	s32 pidx;
	...
	/*
	 * GuardOff + release_lag on the probe slot (Whispy quake + shield @2144): verify cannot round-trip the
	 * dual-effect fold when emergency live is already at T+1 — skip without requiring next_slot.
	 */
	for (pidx = 0; pidx < GMCOMMON_PLAYERS_MAX; pidx++)
	{
		const SYNetRbSnapFighterBlob *probe_fb;
		...
		if ((probe_fb->status_id == nFTCommonStatusGuardOff) &&
		    (syNetRbSnapGuardReleaseLagFromBlob(probe_fb) > 0))
		{
			if (syNetRbSnapFindShieldEffectBlobForPlayer(probe_slot, pidx) != NULL)
			{
				return TRUE;
	...
	{
		const SYNetRbSnapFighterBlob *probe_fb;
		const SYNetRbSnapFighterBlob *next_fb;

		probe_fb = &probe_slot->fighters[pidx];
		next_fb = &next_slot->fighters[pidx];
		if ((probe_fb->is_valid == FALSE) || (next_fb->is_valid == FALSE))
		{
			continue;
		}
		if (syNetRbSnapBlobInGuardScope(probe_fb) == FALSE)
		{
			continue;
		}
		if ((probe_fb->status_id != nFTCommonStatusGuard) && (probe_fb->status_id != nFTCommonStatusGuardOn))
		{
			continue;
		}
		if ((next_fb->status_id == nFTCommonStatusGuardOff) || (next_fb->status_id == nFTCommonStatusGuardSetOff))
		{
			return TRUE;
		}
	}
...
sb32 syNetRbSnapshotSynctestShouldSkipProbeTick(u32 probe_tick, const char **reason_out)
{
	...
	if (syNetRbSnapshotSynctestProbeGuardReleaseBoundaryFragile(probe_tick) != FALSE)
	{
		if (reason_out != NULL)
		{
			*reason_out = "guard_release_boundary_probe";
		}
		return TRUE;
	}
	if (syNetRbSnapshotSynctestProbeGuardEscapeBoundaryFragile(probe_tick) != FALSE)
	{
		if (reason_out != NULL)
		{
			*reason_out = "yoshi_guard_escape_boundary_probe";
		}
		return TRUE;
	}
	if (syNetRbSnapshotSlotYoshiAerialLandingFragile(slot) != FALSE)
	{
		if (reason_out != NULL)
		{
			*reason_out = "yoshi_aerial_landing_probe";
		}
		return TRUE;
	}
	{
		...
		for (pidx = 0; pidx < GMCOMMON_PLAYERS_MAX; pidx++)
		{
			if (syNetRbSnapBlobInCliffMotionSynctestFragileScope(&slot->fighters[pidx]) != FALSE)
			{
				if (reason_out != NULL)
				{
					*reason_out = "cliff_climb_probe";
				}
				return TRUE;
			}
		}
	...
	if (syNetRbSnapshotSlotGuardEscapeOverlapFragile(slot) != FALSE)
	{
		if (reason_out != NULL)
		{
			*reason_out = "guard_escape_overlap_probe";
		}
		return TRUE;
	}
...
void syNetRbSnapshotResetSession(void)
{
	...
	sSYNetRbLoadFailRestoreVerifyValid = FALSE;
	memset(&sSYNetRbLoadFailRestoreVerifySlot, 0, sizeof(sSYNetRbLoadFailRestoreVerifySlot));
...
static void syNetRbSnapNormalizeCapturedYoshiEggLayIdentity(SYNetRbSnapshotSlot *slot)
{
	u32 seen_gobj_ids[SYNETRB_SNAPSHOT_MAX_EFFECTS];
	u32 seen_victim_ids[SYNETRB_SNAPSHOT_MAX_EFFECTS];
	s32 seen_gobj_count;
	s32 seen_victim_count;
	s32 ei;
	s32 j;
	...
	for (ei = 0; ei < slot->effect_count; ei++)
	{
		...
		blob = &slot->effects[ei];
		if ((blob->is_valid == FALSE) || (blob->respawn_kind != SYNETRB_EFFECT_RESPAWN_YOSHI_EGG_LAY))
		{
			continue;
		}
		if (blob->gobj_id != 0U)
		{
			for (j = 0; j < seen_gobj_count; j++)
			{
				if (seen_gobj_ids[j] == blob->gobj_id)
				{
					blob->is_valid = FALSE;
					break;
				}
			...
			seen_gobj_ids[seen_gobj_count++] = blob->gobj_id;
		}
		victim_id = blob->fighter_gobj_id;
		if (victim_id == 0U)
		{
			continue;
		}
		for (j = 0; j < seen_victim_count; j++)
		{
			if (seen_victim_ids[j] == victim_id)
			{
				blob->is_valid = FALSE;
				break;
			}
		...
		seen_victim_ids[seen_victim_count++] = victim_id;
	}
}
#endif
...
static sb32 syNetRbSnapFillSlotFromLive(SYNetRbSnapshotSlot *slot, u32 completed_sim_tick)
{
	...
	syNetRbSnapNormalizeCapturedYoshiEggLayIdentity(slot);
#endif
...
static void syNetRbSnapApplySlotToLive(const SYNetRbSnapshotSlot *slot)
{
	...
	syNetRbSnapRepairStageAfterParticleResetInternal(
	    slot, FALSE, (syNetRbSnapSlotIsEmergencySentinel(slot) == FALSE) ? TRUE : FALSE);
	syNetRbSnapEnsureFoxReflectorEffectsFromSlot(slot);
	syNetRbSnapBackfillGuardShieldEffectIdsFromEffects(slot);
	syNetRbSnapPruneStaleYoshiEggEscapeEffects(slot);
	syNetRbSnapReconcileGuardShieldEffectsCore(slot);
#if defined(SSB64_NETMENU)
	/* Replay hatch shell before egg reconcile can eject a break-complete blob (synctest probe @ T+1). */
...
#if defined(SSB64_NETMENU)
	if ((s_syNetRbSnapRepairStageVerifyOnly == FALSE) || (syNetRbSnapSlotHasQuakeEffectBlob(slot) != FALSE))
#else
	if (TRUE)
#endif
	{
...
void syNetRbSnapshotSanitizeLiveQuakeEffectsAfterEmergencyRestore(void)
{
#ifdef PORT
	s32 pass;
	GObj *gobj;
	...
	for (pass = 0; pass < 2; pass++)
	{
		...
		for (; gobj != NULL; gobj = next)
		{
			...
			eject = FALSE;
			if ((DObjGetStruct(gobj) == NULL) || (gobj->obj == NULL))
			{
				eject = TRUE;
			}
			else if (gobj->anim_frame <= 0.0F)
	...
	if (eject_count > 0)
	{
		port_log("SSB64 NetRbSnapshot: quake_sanitize ejected=%d tick=%u\n", eject_count,
		         syNetInputGetTick());
	}
#else
	/* Offline / non-PORT builds omit net rollback snapshot TU. */
#endif
}
...
#if defined(SSB64_NETMENU)
	memcpy(&sSYNetRbLoadFailRestoreVerifySlot, &sSYNetRbEmergencySlot, sizeof(sSYNetRbLoadFailRestoreVerifySlot));
	sSYNetRbLoadFailRestoreVerifySlot.tick = syNetInputGetTick();
	sSYNetRbLoadFailRestoreVerifySlot.is_valid = TRUE;
	sSYNetRbLoadFailRestoreVerifyValid = TRUE;
	if (syNetRbSnapshotAnyLiveFighterInIntroLoadFidelityScope() != FALSE)
	{
		syNetRbSnapIntroLoadFidelityPreSanityRepair(&sSYNetRbEmergencySlot);
	}
#endif
...
sb32 syNetRbSnapshotSaveMarked(u32 completed_sim_tick, sb32 is_load_safe)
{
	...
	{
		static u32 sLastHoldBlockedSaveLogTick = ~(u32)0;

		if (completed_sim_tick != sLastHoldBlockedSaveLogTick)
		{
			port_log("SSB64 NetRbSnapshot: SAVE blocked (BATTLE_SIM_HOLD) tick=%u\n", completed_sim_tick);
			sLastHoldBlockedSaveLogTick = completed_sim_tick;
		}
		return FALSE;
	}
...
sb32 syNetRbSnapshotLoad(u32 completed_sim_tick)
{
	...
	if (syNetRollbackIsBattleSimHoldActive() != FALSE)
	{
		static u32 sLastHoldBlockedLoadLogTick = ~(u32)0;

		if (completed_sim_tick != sLastHoldBlockedLoadLogTick)
		{
			port_log("SSB64 NetRbSnapshot: LOAD blocked (BATTLE_SIM_HOLD) tick=%u\n", completed_sim_tick);
			sLastHoldBlockedLoadLogTick = completed_sim_tick;
		}
		return FALSE;
	}
	slot = syNetRbSnapshotSlotForTick(completed_sim_tick);
 ...
 * clobbers per-joint AObj chains restored from the snapshot blob. Re-apply blob physics, MPColl, root
 * pose, and joint anim before load-hash verify so figh/anim partitions match ring slot digests.
 */
static void syNetRbSnapReapplyFighterJointAnimFromSlotEx(const SYNetRbSnapshotSlot *slot, sb32 apply_modelparts)
{
	GObj *fighter_gobj;
...
#if defined(SSB64_NETMENU)
		syNetRbSnapReapplyFighterJointAnimFromBlob(fighter_gobj, fp, blob, apply_modelparts);
#else
		fp->physics = blob->physics;
		{
			Vec3f *topn;
...
{
	syNetRbSnapReapplyFighterJointAnimFromSlotEx(slot, FALSE);
}

#if defined(SSB64_NETMENU)
static void syNetRbSnapApplyFighterModelPartsFromSlot(const SYNetRbSnapshotSlot *slot)
{
	GObj *fighter_gobj;
...
/*
 * Stale modelpart cursors on absent joints (Yoshi egg root j9, figatree-only slots) do not affect the
 * figh fold but block appear sanity and mislead modelpart push. Clear them when capture did not see
 * the joint.
 */
static void syNetRbSnapPruneStaleIntroModelpartCursorsFromBlob(FTStruct *fp, const SYNetRbSnapFighterBlob *blob)
{
	s32 ji;
	s32 modelpart_count;
	...
	for (ji = 0; ji < modelpart_count; ji++)
	{
		...
		joint_id = ji + nFTPartsJointCommonStart;
		if ((blob->joint_is_valid[joint_id] != FALSE) || (fp->joints[joint_id] != NULL))
		{
			continue;
		}
		if (fp->modelpart_status[ji].modelpart_id_curr >= 0)
...
static void syNetRbSnapPruneStaleIntroModelpartCursorsFromSlot(const SYNetRbSnapshotSlot *slot)
{
	GObj *fighter_gobj;
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		syNetRbSnapPruneStaleIntroModelpartCursorsFromBlob(fp, blob);
	}
}

static void syNetRbSnapEnsureHiddenPartRootPartsFromBlob(FTStruct *fp, const SYNetRbSnapFighterBlob *blob)
{
	FTHiddenPart *hp_table;
	s32 i;
	...
	for (i = 0; i < 32; i++)
	{
		...
		root_joint_id = hp_table[i].root_joint_id;
		if ((root_joint_id < 0) || (root_joint_id >= FTPARTS_JOINT_NUM_MAX))
		{
			continue;
		}
		if ((blob->joint_is_valid[root_joint_id] == FALSE) || (fp->joints[root_joint_id] == NULL))
		{
			continue;
		}
		syNetRbSnapEnsureFighterJointParts(fp, root_joint_id);
	}
}

static void syNetRbSnapEnsureHiddenPartRootPartsForSlot(const SYNetRbSnapshotSlot *slot)
{
	GObj *fighter_gobj;
...
/*
 * Re-pin MPColl after joint/gobj pose restore. pos_diff and *p_translate are in fhash_light; joint TRS
 * re-pin moves TopN without updating coll scalars left from reapply.
 */
static void syNetRbSnapRestoreFighterMPCollFromBlob(FTStruct *fp, const SYNetRbSnapFighterBlob *blob)
{
	if ((fp == NULL) || (blob == NULL))
	{
		return;
								...
								const SYNetRbSnapFighterBlob *blob)
{
	...
	syNetRbSnapApplyMPColl(&fp->coll_data, &blob->coll, syNetRbSnapFighterMPCollTranslatePtrGobj(fighter_gobj),
			       &fp->lr);
}

/*
 * Terminal hash pin: restore every syNetSyncFoldFighterSlotFullContribution field from the blob after
 * figatree/modelpart work. Joint pose restore is gated on joint_is_valid so phantom live joints never
 * pick up zeroed blob TRS (and missing capture joints stay ejected via reconcile). anim_vel, physics,
...
#if defined(SSB64_NETMENU)
	syNetplayQuantizeFighterPhysics(&fp->physics);
	syNetplayQuantizeVec3f(&fp->anim_vel);
#endif
	syNetRbSnapRestoreFighterJointPoseFromBlob(fp, blob);
	syNetRbSnapRestoreFighterGobjTransformFromBlob(fighter_gobj, blob);
...
/*
 * Final intro-load repair after figatree refresh: ensure FTParts + modelpart DL replay before appear
 * sanity (hidden-part topology already rebuilt via SetStatus in intro presentation sync).
 */
static void syNetRbSnapIntroLoadFidelityPreSanityRepair(const SYNetRbSnapshotSlot *slot)
{
	if (slot == NULL)
	...
	/*
	 * Topology and hidden-part materialization must precede modelpart push: pushing blob cursors
	 * before fp->joints[] matches blob->joint_is_valid[] leaves appear sanity failing on stale
	 * modelpart_id_curr on absent hidden roots.
	 */
	syNetRbSnapReconcileFighterJointPresenceFromSlot(slot);
	syNetRbSnapPruneStaleIntroModelpartCursorsFromSlot(slot);
	syNetRbSnapSyncFighterHiddenPartsForSlot(slot);
	syNetRbSnapEnsureHiddenPartRootPartsForSlot(slot);
	syNetRbSnapEnsureAllFighterJointPartsForSlot(slot);
	syNetRbSnapApplyFighterModelPartsFromSlot(slot);
	/* Modelpart push runs after RefreshFigatreePresentationFromSlot's final re-pin — restore blob AObj/pose. */
...
#if defined(SSB64_NETMENU)
static void syNetRbSnapRefreshIntroAppearCosmeticFromSlot(const SYNetRbSnapshotSlot *slot)
{
	GObj *fighter_gobj;
...
static void syNetRbSnapRefreshIntroAppearCosmeticLiveFromSim(void)
{
	GObj *fighter_gobj;
...
void syNetRbSnapshotRefreshLiveIntroPresentationAfterInterface(void)
{
	...
	{
		/*
		 * gcRunAll already advanced fighter anim; refresh Appear/Entry figatree from live sim only
		 * while those statuses are still active.
		 */
	 ...
	 * Keep integrating battle camera from live fighter interests through GO without slot CObj re-pin.
	 */
...
void syNetRbSnapshotCosmeticAppearPresentationAfterReplayGate(u32 load_tick)
{
	...
	if (syNetRbSnapshotAnyLiveFighterInIntroLoadFidelityScope() != FALSE)
	{
		syNetRbSnapRefreshIntroAppearCosmeticFromSlot(slot);
...
void syNetRbSnapshotRefreshIntroPresentationForDeferWait(u32 presentation_tick)
{
#ifdef PORT
#if defined(SSB64_NETMENU)
	SYNetRbSnapshotSlot *slot;
	...
	if (syNetRbSnapshotAnyLiveFighterInIntroLoadFidelityScope() != FALSE)
	{
		...
		{
			/*
			 * Seal-wait / preemptive cap skips ifCommonBattleUpdateInterfaceAll (gcRunAll). Re-pin Appear
			 * figatree from the ring anchor when still in Appear/Entry scope.
			 */
			syNetRbSnapRefreshIntroAppearCosmeticFromSlot(slot);
			syNetRbSnapLogAppearPresentationDiag(slot, presentation_tick, "defer_wait");
		}
	}
	/* Integrate live camera even when fighters have already entered Wait (post-Appear countdown). */
	syNetRbSnapRunIntroCameraIntegrateStep();
#else
...
void syNetRbSnapshotRefreshDeferredIntroPresentation(u32 sim_tick, u32 anchor_tick)
{
#ifdef PORT
#if defined(SSB64_NETMENU)
	u32 presentation_tick;
	u32 walk;
	u32 probe_figh;
	u32 probe_world;
	u32 probe_item;
	...
	presentation_tick = anchor_tick;
	if (sim_tick > 0U)
	{
		for (walk = sim_tick; (walk > 0U) && (walk >= anchor_tick); walk--)
		{
			if (syNetRbSnapshotGetStoredSubsystemHashes(walk, &probe_figh, &probe_world, &probe_item,
								    &probe_rng) != FALSE)
			{
				presentation_tick = walk;
				break;
			}
		}
	}
	if (presentation_tick == 0U)
	{
		return;
	}
	syNetRbSnapshotRefreshIntroPresentationForDeferWait(presentation_tick);
#else
	(void)sim_tick;
	(void)anchor_tick;
#endif
...
void syNetRbSnapshotRefreshIntroPresentationAfterForwardResimTick(u32 presentation_tick)
{
#ifdef PORT
#if defined(SSB64_NETMENU)
	if (syNetRbSnapshotAnyLiveFighterInIntroLoadFidelityScope() == FALSE)
	...
	(void)presentation_tick;
	/* Sim already at presentation_tick; refresh figatree + live camera integrate (no slot camera re-pin). */
	syNetRbSnapRefreshIntroAppearCosmeticLiveFromSim();
	syNetRbSnapRunIntroCameraIntegrateStep();
#else
	(void)presentation_tick;
...
/*
 * Intro Appear/Entry load: ftMainUpdateHiddenPartID alone skews DObj walk vs motion figatree.
 * Re-run ftMainSetStatus (preserve gameplay cursors) to rebuild hidden joints + figatree coherently,
 * then re-pin blob anim_desc before visual-only figatree bind.
 */
static void syNetRbSnapSyncFighterPresentationIntroLoadFidelity(const SYNetRbSnapshotSlot *slot)
{
	GObj *fighter_gobj;
	u32 preserve_flags;
	...
	preserve_flags = FTSTATUS_PRESERVE_HIT | FTSTATUS_PRESERVE_COLANIM | FTSTATUS_PRESERVE_EFFECT |
	     ...
	     fighter_gobj = fighter_gobj->link_next)
	{
		...
		blob_anim_desc = blob->anim_desc;
		ftMainSetStatus(fighter_gobj, fp->status_id, blob->gobj_anim_frame, 1.0F, preserve_flags);
		if (syNetRbSnapFighterInGrabThrowSynctestFragileScope(fp) == FALSE)
		{
			syNetRbSnapReconcileFighterHiddenPartsFromAnimDesc(fp, fp->anim_desc, blob_anim_desc);
		}
		fp->anim_desc = blob_anim_desc;
		/* SetStatus may clobber gobj/joint anim cursors; figatree bind uses fighter_gobj->anim_frame. */
		syNetRbSnapRestoreFighterAnimScalarsFromBlob(fighter_gobj, fp, blob);
		ftMainRefreshFigatreeVisual(fighter_gobj);
		syNetRbSnapReapplyFighterJointAnimFromBlob(fighter_gobj, fp, blob, FALSE);
	}
...
/*
 * lbCommonAddFighterPartsFigatree dereferences FTParts on every joint in the TopN subtree (no NULL
 * guard). Materialize parts on all live joints before figatree bind.
 */
static void syNetRbSnapSyncFighterPresentationAfterJointPrep(const SYNetRbSnapshotSlot *slot)
{
#if defined(SSB64_NETMENU)
	syNetRbSnapEnsureAllFighterJointPartsForSlot(slot);
	if ((slot != NULL) && (syNetRbSnapshotAnyLiveFighterInIntroLoadFidelityScope() != FALSE) &&
	    (syNetRbSnapFighterCleanupForceLegacySetStatus() == FALSE))
	{
		syNetRbSnapSyncFighterPresentationIntroLoadFidelity(slot);
		return;
	}
#endif
...
/*
 * Terminal verify prep (3-phase): pose/anim re-pin → figatree at restored anim_frame → modelpart push.
 * Modelpart before Sync left stale DLs; modelpart bundled in pre-Sync reapply left NULL FTParts and
 * SIGSEGV on first replay SetModelPartID (@0x38).
 */
static void syNetRbSnapRefreshFigatreePresentationFromSlot(const SYNetRbSnapshotSlot *slot)
{
	if (slot == NULL)
	{
		return;
	...
	syNetRbSnapReapplyFighterJointAnimFromSlot(slot);
	syNetRbSnapSyncFighterPresentationAfterJointPrep(slot);
#if defined(SSB64_NETMENU)
	syNetRbSnapApplyFighterModelPartsFromSlot(slot);
	/*
	 * Figatree bind clobbers per-joint anim_frame / AObj chains that load-hash and anchor probe compare
	 * against the ring blob. Re-pin pose + anim from blob after presentation without another figatree pass.
	 */
	syNetRbSnapReapplyFighterJointAnimFromSlot(slot);
#endif
}
...
void syNetRbSnapshotReapplyJointAnimAtTick(u32 completed_sim_tick)
{
	...
	/*
	 * Synctest verify on escape-roll / guard slots: figatree/joint re-stamp after presentation
	 * repair clobbers anim fields hashed at save time (tick 2318 / 1058 figh/anim drift).
	 */
	if ((s_syNetRbSnapRepairStageVerifyOnly != FALSE) &&
	    (syNetRbSnapSlotVerifyJointAnimFragile(slot) != FALSE))
	{
		return;
	}
	syNetRbSnapReapplyFighterJointAnimFromSlot(slot);
...
sb32 syNetRbSnapshotSlotAnyFighterInYoshiShieldEscapeScopeAtTick(u32 completed_sim_tick)
{
#ifdef PORT
	SYNetRbSnapshotSlot *slot;
...
sb32 syNetRbSnapshotSlotVerifyPresentationFragileAtTick(u32 completed_sim_tick)
{
#ifdef PORT
	SYNetRbSnapshotSlot *slot;
	...
	return syNetRbSnapSlotVerifyJointAnimFragile(slot);
#else
	(void)completed_sim_tick;
	return FALSE;
#endif
...
sb32 syNetRbSnapshotLoadHashEffSoftContinueBlocked(u32 tick)
{
#ifdef PORT
	SYNetRbSnapshotSlot *slot;
	s32 pi;
	...
	if ((syNetRbSnapSlotAnyFighterInYoshiShieldEscapeScope(slot) != FALSE) &&
	    (syNetRbSnapSlotAnyFighterInGuardScope(slot) != FALSE))
	{
		return TRUE;
	}
	for (pi = 0; pi < GMCOMMON_PLAYERS_MAX; pi++)
	{
		...
		blob_guard_id = syNetRbSnapGuardEffectIdFromBlob(fb);
		if (blob_guard_id == 0U)
		{
			continue;
		}
		...
		live_guard_id = syNetRbSnapGobjId(ftStatusVarsGuard(fp)->effect_gobj);
		if (live_guard_id == 0U)
		{
			{
				GObj *live_shield;
				...
				live_guard_id = syNetRbSnapGobjId(live_shield);
			}
		}
		if (live_guard_id != blob_guard_id)
		{
			return TRUE;
		}
	}
                                                ...
                                                sb32 refresh_coupled_weapon_geometry)
{
	 ...
	 */
	syNetRbSnapRepairStageAfterParticleResetInternal(slot, TRUE, FALSE);
	syNetRbSnapRebindFighterItemHoldCoupling();
	syNetRbSnapReconcileOrphanHeldItems(slot);
	if (s_syNetRbSnapRepairStageVerifyOnly == FALSE)
	{
...
void syNetRbSnapshotRefreshPresentationForLoadedTick(u32 completed_sim_tick)
{
#ifdef PORT
	SYNetRbSnapshotSlot *slot;
...
void syNetRbSnapshotResyncLiveFightersFromSlotForSim(u32 load_tick)
{
	...
	/*
	 * PrepareLoaded already ran RefreshFigatreePresentationFromSlot. Re-run the post-figatree repair
	 * tail (topology + hidden parts + modelpart + re-pin), then anchor-probe sim prep (AObj + entry_pos)
	 * before +1 sim.
	 */
	syNetRbSnapIntroLoadFidelityPreSanityRepair(slot);
...
static void syNetRbSnapshotFinalizeVerifyEffectStateInternal(u32 completed_sim_tick)
{
	...
	joint_anim_fragile = syNetRbSnapSlotVerifyJointAnimFragile(slot);
	...
	if ((s_syNetRbSnapRepairStageVerifyOnly == FALSE) || (joint_anim_fragile == FALSE))
	{
		syNetRbSnapReapplyFighterJointAnimFromSlot(slot);
	}
	syNetRbSnapRestoreRebirthFightersAfterFinalize(slot);
...
static sb32 syNetRbSnapFighterIsInPKFireSpecialNStatus(const FTStruct *fp)
{
	...
	if ((fp->fkind == nFTKindKirby) &&
	    ((fp->status_id == nFTKirbyStatusCopyNessSpecialN) ||
	     (fp->status_id == nFTKirbyStatusCopyNessSpecialAirN)))
	{
		return TRUE;
	}
	return FALSE;
...
static s32 syNetRbSnapPKFireSpawnJointForFighter(const FTStruct *fp)
{
	if ((fp != NULL) && (fp->fkind == nFTKindKirby))
	{
		return FTKIRBY_COPYNESS_PKFIRE_SPAWN_JOINT;
	}
	return FTNESS_PKFIRE_SPAWN_JOINT;
}
...
static void syNetRbSnapPKFireSpawnPoseForFighter(const FTStruct *fp, Vec3f *pos, Vec3f *vel, f32 *angle)
{
	...
	f32 spawn_off_y;
	f32 spark_angle_air;
	f32 spark_angle_ground;
	f32 spark_vel_air;
	f32 spark_vel_ground;
	...
	if ((fp != NULL) && (fp->fkind == nFTKindKirby))
	{
		spawn_joint = FTKIRBY_COPYNESS_PKFIRE_SPAWN_JOINT;
		spawn_off_x = FTKIRBY_COPYNESS_PKFIRE_SPAWN_OFF_X;
		spawn_off_y = FTKIRBY_COPYNESS_PKFIRE_SPAWN_OFF_Y;
		spark_angle_air = FTKIRBY_COPYNESS_PKFIRE_SPARK_ANGLE_AIR;
		spark_angle_ground = FTKIRBY_COPYNESS_PKFIRE_SPARK_ANGLE_GROUND;
		spark_vel_air = FTKIRBY_COPYNESS_PKFIRE_SPARK_VEL_AIR;
		spark_vel_ground = FTKIRBY_COPYNESS_PKFIRE_SPARK_VEL_GROUND;
	}
	else
	{
		spawn_joint = FTNESS_PKFIRE_SPAWN_JOINT;
		spawn_off_x = FTNESS_PKFIRE_SPAWN_OFF_X;
		spawn_off_y = FTNESS_PKFIRE_SPAWN_OFF_Y;
		spark_angle_air = FTNESS_PKFIRE_SPARK_ANGLE_AIR;
		spark_angle_ground = FTNESS_PKFIRE_SPARK_ANGLE_GROUND;
		spark_vel_air = FTNESS_PKFIRE_SPARK_VEL_AIR;
		spark_vel_ground = FTNESS_PKFIRE_SPARK_VEL_GROUND;
	}

	gmCollisionGetFighterPartsWorldPosition(fp->joints[spawn_joint], pos);
	pos->x += spawn_off_x * fp->lr;
	pos->y += spawn_off_y;
	pos->z = 0.0F;
	...
	{
		vel->z = 0.0F;
		*angle = spark_angle_air;
		vel->x = __cosf(spark_angle_air) * spark_vel_air * fp->lr;
		vel->y = __sinf(spark_angle_air) * spark_vel_air;
	}
	else
	{
		vel->z = 0.0F;
		*angle = spark_angle_ground;
		vel->x = __cosf(spark_angle_ground) * spark_vel_ground * fp->lr;
		vel->y = __sinf(spark_angle_ground) * spark_vel_ground;
	}
}
...
void syNetRbSnapTrySpawnPKFireFromAccessory(GObj *fighter_gobj)
{
	...
	if (fp->joints[syNetRbSnapPKFireSpawnJointForFighter(fp)] == NULL)
	{
...
#if defined(SSB64_NETMENU)
void syNetRbSnapshotCollectFighterSlotHashesAtTick(u32 tick, u32 *out_slot_hash)
{
	SYNetRbSnapshotSlot *slot;
	s32 si;
	s32 pidx;
...
static u32 syNetRbSnapshotHashFightersLightFromSlot(const SYNetRbSnapshotSlot *slot)
{
	u32 slot_hash[GMCOMMON_PLAYERS_MAX];
	u32 merged;
	s32 si;
...
u32 syNetRbSnapshotGetSlotHashFighterLight(u32 tick)
{
#if defined(SSB64_NETMENU)
	SYNetRbSnapshotSlot *slot;
	...
	return syNetRbSnapshotHashFightersLightFromSlot(slot);
#else
	(void)tick;
	return 2166136261U;
#endif
